...
# CPU metrics (5 second sample)
CPU_IDLE=$(top -l 2 -n 0 -s 1 | grep "CPU usage" | tail -1 | awk '{print $7}' | tr -d '%')
CPU_USER=$(top -l 2 -n 0 -s 1 | grep "CPU usage" | tail -1 | awk '{print $3}' | tr -d '%')
CPU_SYS=$(top -l 2 -n 0 -s 1 | grep "CPU usage" | tail -1 | awk '{print $5}' | tr -d '%')

# Memory
MEM_USED_GB=$(top -l 1 -n 0 | grep PhysMem | awk '{print $2}' | tr -d 'G')
MEM_WIRED_GB=$(top -l 1 -n 0 | grep PhysMem | awk '{print $6}' | tr -d 'G')

# Disk I/O (sample iostat)
DISK_READ_MB=$(iostat -d -c 2 disk0 | tail -1 | awk '{print $3}')
DISK_WRITE_MB=$(iostat -d -c 2 disk0 | tail -1 | awk '{print $4}')
