...
# shellcheck source=exit-codes-robust.sh
    ...
    if [ -f "$email_file" ]; then
        ...
        if grep -qE '(January|February|March|April|May|June|July|August|September|October|November|December) [0-9]{1,2}, [0-9]{4}' "$email_file"; then
            ...
            if [ "$date_unix" -gt 0 ] && [ "$date_unix" -lt "$today_unix" ]; then
                ...
                if echo "$combined_text" | grep -qiE '(case|26cv|arbitration|settlement|court|attorney|grimes|legal|plaintiff|defendant)'; then
                    email_type="legal"
                # Action keywords: move, mover, relocation, appointment, schedule, deadline
                elif echo "$combined_text" | grep -qiE '(move|mover|relocation|moving|appointment|schedule|deadline)'; then
                    email_type="action"
                fi
                
                case "$email_type" in
                    legal)
                        # Legal: allow historical dates, warn if >30 days old
                        local age_days=$(( (today_unix - date_unix) / 86400 ))
                        if [ "$age_days" -gt 30 ]; then
                            echo -e "${YELLOW}⚠ WARNING: Date $detect_date is ${age_days}d old (legal context: verify accuracy)${NC}"
                            WARNING_EMAILS+=("$email_file: Historical date ${detect_date} (${age_days}d old - legal context)")
                        else
