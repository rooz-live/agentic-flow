...
import type { AgentDBReasoningBankAdapter as AgentDBAdapter } from 'agentdb/reasoningbank/adapter/agentdb-adapter';
import type {
  ReasoningMemory,
  PatternData,
  TrajectoryData,
  RetrievalOptions,
} from 'agentdb/reasoningbank/adapter/types';
 ...
 * ```
 */
export async function createDefaultAgentDBAdapter(): Promise<AgentDBAdapter> {
...
export async function migrateToAgentDB(
  ...
  return migrateLegacyDatabase(
    sourcePath,
    destinationPath || '.agentdb/reasoningbank.db'
  );
}
...
// Re-export AgentDB adapter type for TypeScript users
