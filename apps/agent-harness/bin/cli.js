...
// Generated by metaharness — the `agent-harness` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx agent-harness`
...
// "bin" + "files" fields in package.json), so `npx agent-harness` works the moment
...
const HARNESS_NAME = 'agent-harness';

/** `agent-harness init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `agent-harness doctor` — verify the install end-to-end (kernel + host resolve). */
