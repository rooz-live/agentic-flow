...
export class AgentDBIntegration {
     ...
     */
    async storeAnalysisPattern(analysis) {
     ...
     */
    async recordFeedback(analysisId, accuracy, providerFeedback, corrections) {
        const feedback = {
            analysisId,
            accuracy,
            providerFeedback,
     ...
     */
    async findSimilarPatterns(symptoms, k = 5) {
        // In production, use AgentDB vector search
        // For now, simple matching
        const symptomText = symptoms.join(' ').toLowerCase();
        const matches = [];
        for (const pattern of this.patterns.values()) {
            const patternConditions = (pattern.metadata.conditions || []).join(' ').toLowerCase();
    ...
    async getLearningMetrics() {
        ...
        const avgAccuracy = feedbackArray.length > 0
            ? feedbackArray.reduce((sum, f) => sum + f.accuracy, 0) / feedbackArray.length
            : 0;
        // Count conditions
        const conditionCounts = new Map();
        for (const pattern of this.patterns.values()) {
            const conditions = pattern.metadata.conditions || [];
    ...
    async updatePattern(pattern) {
        this.patterns.set(pattern.metadata.analysisId, pattern);
