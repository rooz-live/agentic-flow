...
export class ConfidenceScoreTool {
    ...
    calculateCompositeScore(metrics) {
        ...
        const score = metrics.overall * weights.overall +
     ...
     */
    interpretConfidence(score, issues) {
        if (score >= 0.9) {
     ...
     */
    generateDetailedBreakdown(metrics, analysis) {
        return {
            overall: {
     ...
     */
    assessCitationQuality(analysis) {
    ...
    assessCompleteness(analysis) {
        ...
        if (analysis.conditions.every(c => c.icd10Code))
            score += 0.1;
        if (analysis.conditions.some(c => c.differential && c.differential.length > 0))
            score += 0.1;
     ...
     */
    interpretModelAgreement(score) {
        if (score >= 0.9)
            return 'Excellent agreement across models';
        if (score >= 0.75)
            return 'Good agreement with minor variations';
        if (score >= 0.6)
            return 'Moderate agreement, some disagreement exists';
        return 'Low agreement, significant model disagreement';
    }
    /**
     * Format confidence report
     */
    formatConfidenceReport(response) {
