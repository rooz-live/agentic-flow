...
export class EmergencyEscalationService {
    alerts;
    escalationChains; // specialization -> providerIds
    activeAlerts;
    constructor() {
        this.alerts = new Map();
     ...
     */
    async createAlert(query, severity, triggeredBy) {
        const alert = {
     ...
     */
    async resolveAlert(alertId, providerId, resolution) {
        const alert = this.alerts.get(alertId);
        if (!alert) {
            return false;
        }
     ...
     */
    setEscalationChain(specialization, providerIds) {
     ...
     */
    async assignToEscalationChain(alert) {
     ...
     */
    generateAlertDescription(query, severity) {
     ...
     */
    async checkUnacknowledgedAlerts(timeoutMinutes = 5) {
        const now = Date.now();
     ...
     */
    getAlertsForProvider(providerId) {
        return Array.from(this.alerts.values())
            .filter(alert => alert.assignedProviders.includes(providerId) &&
