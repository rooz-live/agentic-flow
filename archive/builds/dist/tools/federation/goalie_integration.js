...
export class GoalieIntegrationManager extends EventEmitter {
    ...
    async refreshAnalytics() {
        try {
            ...
            const analyticsSummary = await this.analytics.generateAnalytics(patternData, batchHistory, 30 // 30-day window
            );
            // Emit update event
     ...
     * Calculate WSJF scores for patterns
     */
    async calculatePatternWSJF(patternData) {
        return Promise.all(patternData.map(async (pattern) => {
        ...
        try {
            ...
            for (const plan of pendingPlans) {
                ...
                if (this.config.batching.autoApproveLowRisk) {
                    const lowRiskItems = plan.items.filter(item => item.riskLevel && item.riskLevel <= this.config.batching.riskThreshold);
...
//# sourceMappingURL=goalie_integration.js.map