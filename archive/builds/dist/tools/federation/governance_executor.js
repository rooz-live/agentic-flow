...
function getVerifiedRate(goalieDir) {
...
function getCpuPercent() {
    ...
    if (!avg || !Number.isFinite(avg) || cores <= 0)
        return null;
...
async function readGovernanceJsonFromStdin() {
...
async function applyProposals(proposals, opts) {
...
async function main() {
    ...
    const safePatterns = new Set(safePatternsEnv
        .split(',')
        .map((p) => p.trim())
        .filter((p) => p.length > 0));
    ...
    if (!autocommit)
        blockedReasons.push('AF_ALLOW_CODE_AUTOCOMMIT!=1');
    if (!autoApplyEnabled)
        blockedReasons.push('AF_GOVERNANCE_AUTO_APPLY_ENABLED!=1');
    if (verifiedThreshold > 0 && (verifiedRate ?? 0) < verifiedThreshold)
        blockedReasons.push('verifiedRate-below-threshold');
    if (cpuThreshold > 0 && (cpuPct ?? 0) >= cpuThreshold)
        blockedReasons.push('cpu-threshold-exceeded');
    if (minDiskGb > 0 && (freeDiskGb ?? 0) < minDiskGb)
        blockedReasons.push('disk-below-minimum');
    let summary;
...
//# sourceMappingURL=governance_executor.js.map