...
import { loadIrisConfig, watchIrisConfig, DEFAULT_IRIS_BRIDGE_CONFIG } from './iris_config.js';
// ============================================================================
...
export class EnterpriseIrisBridge extends EventEmitter {
    ...
    endDistributedTrace(trace, metrics, error) {
        ...
        if (metrics) {
            trace.tags = { ...trace.tags, ...Object.fromEntries(Object.entries(metrics).map(([k, v]) => [k, String(v)])) };
        }
        if (error) {
...
export async function captureIrisMetrics(command, args = [], options) {
    if (!defaultBridge) {
        const config = await loadIrisConfig();
...
export function __setCommandRunner(runner) {
...
export async function __resetIrisBridgeCache() {
    defaultBridge = null;
}
export function isIrisMetricsEnabled() {
