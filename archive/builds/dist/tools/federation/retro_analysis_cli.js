    ...
    try {
        ...
        if (retroData.forensic_action_analysis) {
            for (const [circle, data] of Object.entries(retroData.forensic_action_analysis)) {
                if (args.circle && circle !== args.circle)
                    continue;
                const analysis = data;
                insights.push({
        ...
        console.error(`[retro-analysis] Error: ${error.message}`);
        process.exit(1);
    }
}
function emitPatternEvent(goalieDir, insightCount) {
    const event = {
...
//# sourceMappingURL=retro_analysis_cli.js.map