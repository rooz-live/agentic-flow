...
import { getActionKeys, readJsonl, } from './shared_utils.js';
 ...
 */
function generateMultiDimensionalAnalytics(wsjfResults, patterns, insights) {
 ...
 */
function generateTrendAnalysis(wsjfResults, patterns) {
    // Analyze COD trends
    const codTrends = wsjfResults.map(result => {
        ...
        const recentAvg = recentPatterns.length > 0 ?
            recentPatterns.reduce((sum, p) => sum + (p.economic?.cod || 0), 0) / recentPatterns.length : 0;
        const olderAvg = olderPatterns.length > 0 ?
            olderPatterns.reduce((sum, p) => sum + (p.economic?.cod || 0), 0) / olderPatterns.length : 0;
    ...
    const wsjfTrends = wsjfResults.map(result => {
        ...
        const recentAvg = recentPatterns.length > 0 ?
            recentPatterns.reduce((sum, p) => sum + (p.economic?.wsjf_score || 0), 0) / recentPatterns.length : 0;
        const olderAvg = olderPatterns.length > 0 ?
            olderPatterns.reduce((sum, p) => sum + (p.economic?.wsjf_score || 0), 0) / olderPatterns.length : 0;
        const changePercent = olderAvg > 0 ? ((recentAvg - olderAvg) / olderAvg) * 100 : 0;
 ...
 */
function generateActionItemEffectiveness(insights) {
    const actionItems = insights.filter(i => i.type === 'action_item');
...
async function main() {
    ...
    const enhancedAnalyticsSummary = {
        wsjfAnalysis: {
            totalItems: wsjfResults.length,
            topPriorityItems: wsjfResults.slice(0, 10),
            batchRecommendations,
            riskSummary: {
                overallRiskLevel: Math.round(wsjfResults.reduce((sum, result) => sum + result.riskAssessment.riskLevel, 0) / wsjfResults.length),
                riskDistribution: {
                ...
                topRiskFactors: wsjfResults.length > 0 ?
                    Array.from(wsjfResults.reduce((acc, result) => {
                ...
                mitigationStrategies: wsjfResults.length > 0 ?
                    Array.from(new Set(wsjfResults.flatMap(result => result.riskAssessment.mitigationStrategies))).slice(0, 5) : [],
            },
            economicImpact: {
                totalCostOfDelay: wsjfResults.reduce((sum, result) => sum + result.costOfDelay, 0),
                potentialSavings: wsjfResults.reduce((sum, result) => sum + (result.wsjfScore * result.parameters.jobDuration), 0),
...
export { main, generateMultiDimensionalAnalytics, generateTrendAnalysis, generateActionItemEffectiveness };
//# sourceMappingURL=retro_coach_enhanced.js.map