...
class GoalieKanbanProviderEnhanced {
    ...
    WIP_LIMITS = { NOW: 5, NEXT: 8, LATER: 15 };
     ...
     */
    async moveKanbanEntry(element, targetSection) {
        const goalieDir = this.getGoalieDir();
        const kanbanDoc = this.loadKanbanDoc(goalieDir);
     ...
     * Add new Kanban item
     */
    async addKanbanItem(section, title, summary, filePath, wsjfScore, riskLevel = 5) {
        const goalieDir = this.getGoalieDir();
        const kanbanDoc = this.loadKanbanDoc(goalieDir);
        if (!kanbanDoc) {
            kanbanDoc[section] = [];
     ...
     * Update Kanban item
     */
    async updateKanbanEntry(element, updates) {
        const goalieDir = this.getGoalieDir();
        const kanbanDoc = this.loadKanbanDoc(goalieDir);
        if (!kanbanDoc || !element.section) {
            return;
     ...
     * Delete Kanban item
     */
    async deleteKanbanEntry(element) {
        const goalieDir = this.getGoalieDir();
        const kanbanDoc = this.loadKanbanDoc(goalieDir);
        if (!kanbanDoc || !element.section) {
            return;
...
class PatternMetricsProviderEnhanced {
    ...
    logger;
    _onDidChangeTreeData;
    analytics = new MultiDimensionalAnalytics('');
    constructor(workspaceRoot, logger) {
        this.workspaceRoot = workspaceRoot;
        this.logger = logger;
        this._onDidChangeTreeData = new vscode.EventEmitter();
...
class BatchExecutionProvider {
    ...
    logger;
    _extensionUri;
    _onDidChangeViewData = new vscode.EventEmitter();
    _executionState = {
        activePlans: new Map(),
        history: [],
        approvalQueue: []
    ...
    async showBatchExecution(planId) {
    ...
    async executeBatchPlan(planId, webview) {
    ...
    async approveBatchItem(itemId, approved, webview) {
    ...
    async rejectBatchItem(itemId, webview) {
        const plan = Array.from(this._executionState.activePlans.values())[0]; // Get first plan for demo
        if (!plan)
            return;
        const item = plan.items.find(item => item.id === itemId);
    ...
    async getSelectedPlanId(webview) {
...
class VisualizationProvider {
    ...
    async refreshAnalyticsView(webview) {
        const analytics = await this.analytics.generateAnalytics([], [], 30);
        webview.html = this.getAnalyticsHtml(analytics);
    }
