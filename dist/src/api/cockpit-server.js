...
app.post('/api/pivot', async (req, res) => {
    try {
        ...
        const { stdout, stderr } = await execAsync(`bash ${ROOT_DIR}/scripts/ay-yo-enhanced.sh pivot ${dimension} ${value || ''}`);
        res.json({
            success: true,
            dimension,
            value,
...
app.get('/api/episodes', async (req, res) => {
    try {
        ...
        const episodes = await getRecentEpisodes(parseInt(limit, 10), circle);
...
app.post('/api/episodes', async (req, res) => {
    try {
        ...
        const episodeFile = path.join(ROOT_DIR, '.episodes', `${circle}_${ceremony}_${Date.now()}.json`);
 ...
 */
app.get('/api/skills/:circle/:ceremony', async (req, res) => {
    try {
        const { circle, ceremony } = req.params;
        if (!CIRCLES.includes(circle)) {
            return res.status(400).json({ error: 'Invalid circle' });
        }
        const { stdout } = await execAsync(`bash ${ROOT_DIR}/scripts/ay-prod-skill-lookup.sh ${circle} ${ceremony}`).catch(() => ({ stdout: '', stderr: '' }));
 ...
 */
app.post('/api/ceremony', async (req, res) => {
    try {
 ...
 */
app.post('/api/ceremony/schedule', async (req, res) => {
    try {
        const schedule = req.body;
        if (!CIRCLES.includes(schedule.circle)) {
            return res.status(400).json({ error: 'Invalid circle' });
        }
 ...
 */
app.get('/api/ceremony/schedule', async (req, res) => {
    try {
        const { circle } = req.query;
        const schedules = scheduler.getAllSchedules(circle);
 ...
 */
app.post('/api/roam', async (req, res) => {
    try {
        ...
        if (!CIRCLES.includes(entity.owner_circle)) {
 ...
 */
app.get('/api/roam/all', async (req, res) => {
    try {
 ...
 */
app.post('/api/roam/mitigation-plan', async (req, res) => {
    try {
        const plan = req.body;
 ...
 */
app.get('/api/roam/mitigation-effectiveness', async (req, res) => {
    try {
...
async function getCircleEquity() {
    try {
...
async function getROAMExposure() {
    try {
...
async function getRecentEpisodes(limit, circle) {
    try {
...
export default app;
//# sourceMappingURL=cockpit-server.js.map