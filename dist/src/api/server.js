...
import { authenticate, optionalAuthenticate, requireCircleAccess, login } from './middleware/auth';
 ...
 */
async function calculateCircleEquityFromFiles() {
 ...
 */
async function readEquityCache() {
    const cacheFile = path.join(ROOT_DIR, '.equity', 'circle_equity.json');
    try {
        const data = await fs.readFile(cacheFile, 'utf-8');
        const cache = JSON.parse(data);
        // Convert to our response format
        const circles = Object.entries(cache.circles || {}).map(([name, data]) => ({
            name,
            color: CIRCLE_COLORS[name],
 ...
 */
app.get('/api/circles/equity', optionalAuthenticate, async (req, res) => {
    try {
        // Try cache first
 ...
 */
app.get('/api/roam/metrics', (req, res) => {
