...
app.get('/api/swarm/agents', async (req, res) => {
    try {
        ...
        const agents = await Promise.all(agentsList.agents.map(async (agent) => {
...
// Helper functions
function calculateSwarmHealth(swarmStatus, daemonStatus) {
...
async function calculateWsjfScore() {
    // Calculate aggregate WSJF from all tracked items
    try {
        const items = await fetchWsjfItems();
        const avgWsjf = items.reduce((sum, item) => sum + (item.wsjfScore || 0), 0) /
            (items.length || 1);
        return avgWsjf;
    }
    catch {
...
async function fetchAgentRoamData(agentId) {
...
function calculateMymScore(agent, roamData) {
...
function generateMemoryConnections(memoryData) {
...
export default app;
//# sourceMappingURL=swarm-api-server.js.map