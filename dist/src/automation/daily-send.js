...
export class DailySendAutomation {
    ...
    async syncTemplates(options) {
        if (options.source === 'git') {
            ...
            fs.writeFileSync(path.join(this.templateDir, 'synced.md'), '# Synced Template\n\nFrom: {{repository}}');
    ...
    async executeSchedule(scheduleId) {
        ...
        const rendered = await this.renderTemplate(schedule.template, schedule.variables || {});
        if (!rendered) {
            return { success: false };
        }
        // Record send
    ...
    async getStreakMetrics() {
        // Calculate streaks from send history
        const sortedSends = [...this.sendHistory].sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
        let currentStreak = 0;
        let longestStreak = 0;
        let streak = 0;
        let lastDate = null;
        for (const send of sortedSends) {
            if (!lastDate) {
                ...
                const dayDiff = Math.floor((send.timestamp.getTime() - lastDate.getTime()) / (1000 * 60 * 60 * 24));
                if (dayDiff === 1) {
                    streak++;
                }
                else {
    ...
    async sendDaily(options) {
        ...
        if (!templateExists && !options.dryRun) {
            ...
            fs.writeFileSync(path.join(this.templateDir, `${options.template}.md`), `# ${options.template}\n\nContent: {{content}}`);
        }
...
//# sourceMappingURL=daily-send.js.map