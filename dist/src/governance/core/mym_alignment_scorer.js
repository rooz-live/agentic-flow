...
export class MYMAlignmentScorer {
    ...
    async scoreYasna(context) {
        ...
        // Temporal Rank binding
        if (context.temporalRank) {
            const temporalValues = {
                'hour': 1.0, 'day': 0.95, 'week': 0.85, 'month': 0.7, 'season': 0.5,
                'year': 0.3, 'decade': 0.1, 'century': 0.05,
                'now': 1.0, 'next': 0.8, 'later': 0.4
            };
            const rankValue = temporalValues[context.temporalRank];
            const weight = 0.3;
            evidence.push({
                type: 'observation',
                description: `Temporal Capability Rank: ${context.temporalRank}`,
                value: rankValue,
                weight
            });
            weightedScore += rankValue * weight;
            totalWeight += weight;
        }
        const finalScore = totalWeight > 0 ? weightedScore / totalWeight : 0.5;
    ...
    async scoreMithra(context) {
        ...
        // Sprawl Cleanup Risk integration
        if (context.sprawlCleanupRisk !== undefined) {
            const sprawlScore = Math.max(0, 1 - context.sprawlCleanupRisk);
            const weight = 0.2;
            evidence.push({
                type: 'metric',
                description: `Sprawl Cleanup Readiness: ${(sprawlScore * 100).toFixed(1)}% (Risk: ${(context.sprawlCleanupRisk * 100).toFixed(1)}%)`,
                value: sprawlScore,
                weight
            });
            weightedScore += sprawlScore * weight;
            totalWeight += weight;
        }
        // Utilization metrics integration
        if (context.utilizationMetrics) {
            const utilScore = context.utilizationMetrics.mostCapable ? 1.0 : Math.min(1.0, context.utilizationMetrics.cycleExecutions / 10);
            const weight = 0.15;
            evidence.push({
                type: 'metric',
                description: `Cycle Utilization: ${context.utilizationMetrics.cycleExecutions} execs, Most Capable: ${context.utilizationMetrics.mostCapable}`,
                value: utilScore,
                weight
            });
            weightedScore += utilScore * weight;
            totalWeight += weight;
        }
        const finalScore = totalWeight > 0 ? weightedScore / totalWeight : 0.5;
        const confidence = totalWeight / 1.0;
    ...
    async calculateAlignment(context) {
        ...
        const overall = (manthra.score * manthra.confidence +
            yasna.score * yasna.confidence +
            mithra.score * mithra.confidence) / (manthra.confidence + yasna.confidence + mithra.confidence);
...
//# sourceMappingURL=mym_alignment_scorer.js.map