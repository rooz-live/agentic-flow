...
export class AdaptiveHealthChecker {
    ...
    calculateAnomalyRate(state) {
        ...
        const anomalyCount = recentWindow.filter((p) => !p.success).length;
        return {
            window: windowSize,
            anomalyCount,
    ...
    async getDynamicSleep(baseMs) {
        try {
            ...
            const result = execSync(`${scriptPath} calculate ${baseSec} auto`, { encoding: 'utf-8', timeout: 5000 }).toString().trim();
...
//# sourceMappingURL=adaptive-health-checker.js.map