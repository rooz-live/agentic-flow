...
export class GLMLocalLLM extends EventEmitter {
    ...
    validateConfig() {
        ...
        if (this.config.vramGB < requiredGB) {
            console.warn(`⚠️  Insufficient VRAM: ${this.config.vramGB}GB < ${requiredGB}GB required. ` +
    ...
    async infer(request) {
        ...
        if (this.isLoaded && !this.fallbackToAPI) {
            ...
            const newAvg = ((this.inferenceStats.avgLatencyLocal * (this.inferenceStats.localInferences - 1)) +
                response.latencyMs) / this.inferenceStats.localInferences;
            ...
            const newAvg = ((this.inferenceStats.avgLatencyAPI * (this.inferenceStats.apiInferences - 1)) +
                response.latencyMs) / this.inferenceStats.apiInferences;
    ...
    generateMockResponse(request) {
        ...
        return `Response to: "${request.prompt.substring(0, 50)}..." [Generated by ${this.isLoaded ? 'local GLM model' : 'API fallback'}]`;
    }
    getStats() {
        return {
            ...this.inferenceStats,
            localPercentage: ((this.inferenceStats.localInferences / this.inferenceStats.totalInferences) * 100).toFixed(1) + '%',
            apiPercentage: ((this.inferenceStats.apiInferences / this.inferenceStats.totalInferences) * 100).toFixed(1) + '%',
            avgLatencyDiff: (this.inferenceStats.avgLatencyAPI - this.inferenceStats.avgLatencyLocal).toFixed(0) + 'ms',
        };
    }
    getModelSpecs() {
...
// Usage example
export async function createGLMInstance(variant = 'small') {
...
//# sourceMappingURL=local-glm-integration.js.map