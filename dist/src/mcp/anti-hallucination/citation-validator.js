...
export class CitationValidator {
     ...
     */
    async verifyCitationSource(citation) {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                const isVerified = this.isTrustedSource(citation.source) &&
    ...
    isTrustedSource(source) {
        return Array.from(this.trustedSources).some(trusted => source.toLowerCase().includes(trusted.toLowerCase()));
    }
    /**
     * Calculate citation confidence
     */
    calculateCitationConfidence(citation, issues) {
        let confidence = 1.0;
        // Penalize for issues
     ...
     */
    calculateOverallConfidence(citations, issues) {
        if (citations.length === 0)
            return 0;
        const avgCitationScore = citations.reduce((sum, c) => sum + c.relevanceScore, 0) / citations.length;
