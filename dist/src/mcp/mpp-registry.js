...
export class MPPRegistry {
     ...
     */
    routeToModel(taskDescription, taskComplexity) {
     ...
     */
    verifyProtocol(serverVersion, clientVersion) {
     ...
     */
    createSession(sessionId, swarmId) {
     ...
     */
    updateSession(sessionId, context, tokensUsed) {
    ...
    saveToDisk() {
        try {
            ...
            writeFileSync(join(this.registryPath, 'registry.json'), JSON.stringify(data, null, 2), 'utf-8');
        }
        catch (error) {
    ...
    loadFromDisk() {
        ...
        try {
            ...
            data.models?.forEach((m) => this.models.set(m.name, m));
            data.patterns?.forEach((p) => this.patterns.set(p.id, p));
            data.protocols?.forEach((pr) => this.protocols.set(pr.version, pr));
            data.servers?.forEach((s) => this.servers.set(s.id, s));
            data.sessions?.forEach((se) => this.sessions.set(se.sessionId, se));
        }
        catch (error) {
...
//# sourceMappingURL=mpp-registry.js.map