...
export class KnowledgeSearchTool {
     ...
     */
    async searchKnowledgeBases(query) {
     ...
     */
    generateResults(queryText, source, filters) {
        const results = [];
...
This information is derived from peer-reviewed medical literature and clinical guidelines.
    ...
    getSourceType(source) {
        if (source.includes('Guideline'))
            return 'clinical_guideline';
        if (source.includes('Library') || source.includes('PubMed'))
            return 'research_paper';
        if (source.includes('UpToDate'))
            return 'textbook';
        return 'database';
    }
    /**
     ...
     */
    matchesSourceType(source, types) {
