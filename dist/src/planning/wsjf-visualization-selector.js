...
export class WSJFVisualizationSelector {
     ...
     */
    calculateWSJF(framework, requirements) {
        // Business Value (1-10)
        let businessValue = 5;
        ...
        const weeksToDeadline = requirements.business.productionDeadline
            ? (requirements.business.productionDeadline.getTime() - Date.now()) /
                (1000 * 60 * 60 * 24 * 7)
            : 999;
        if (weeksToDeadline < 4)
            timeCriticality += 3;
        else if (weeksToDeadline < 8)
            timeCriticality += 2;
        else if (weeksToDeadline < 12)
            timeCriticality += 1;
        if (framework.learningCurve === 'easy')
            timeCriticality += 2;
        else if (framework.learningCurve === 'steep')
            timeCriticality -= 1;
        ...
        if (framework.maturity === 'proven')
            riskReduction += 3;
        else if (framework.maturity === 'stable')
            riskReduction += 1;
        else if (framework.maturity === 'experimental')
            riskReduction -= 2;
        ...
        if (requirements.technical.reactStack && !framework.reactFriendly)
            jobSize += 3;
        // Package size impact
     ...
     */
    evaluateFrameworks(requirements) {
        ...
        let filtered = scored.filter((f) => {
            if (requirements.technical.requires3D && !f.use3D)
                return false;
            if (requirements.technical.requiresGeospatial && !f.geospatial)
                return false;
    ...
    generateRationale(framework, score, requirements) {
        const reasons = [];
        reasons.push(`**WSJF Score: ${score.wsjf.toFixed(2)}**`);
        reasons.push(`- Business Value: ${score.businessValue}/10 (${this.explainBusinessValue(framework, requirements)})`);
        reasons.push(`- Time Criticality: ${score.timeCriticality}/10 (${this.explainTimeCriticality(framework, requirements)})`);
        reasons.push(`- Risk Reduction: ${score.riskReduction}/10 (${this.explainRiskReduction(framework, requirements)})`);
    ...
    explainBusinessValue(framework, requirements) {
        const reasons = [];
        if (framework.maturity === 'proven')
            reasons.push('proven maturity');
        if (framework.reactFriendly && requirements.technical.reactStack)
            reasons.push('React fit');
        return reasons.join(', ') || 'baseline';
    }
    explainTimeCriticality(framework, requirements) {
        const reasons = [];
        if (framework.learningCurve === 'easy')
            reasons.push('easy learning curve');
        if (requirements.business.timeToMarket < 4)
            reasons.push('urgent deadline');
        return reasons.join(', ') || 'standard timeline';
    }
    explainRiskReduction(framework, requirements) {
        const reasons = [];
        if (framework.maturity === 'proven')
            reasons.push('production-proven');
        if (requirements.business.riskTolerance === 'low')
            reasons.push('low risk tolerance');
        return reasons.join(', ') || 'standard risk';
    }
    explainJobSize(framework, requirements) {
        const reasons = [];
        if (framework.learningCurve === 'easy')
 ...
 */
export function selectVisualizationForAgenticFlow() {
    ...
    recommendation.alternatives.forEach((alt, i) => {
        console.log(`   ${i + 1}. ${alt.name} (WSJF: ${alt.wsjfScore?.wsjf.toFixed(2)}) - ${alt.learningCurve} learning curve`);
    });
}
export function storeMCPDecision(decision) {
 ...
 */
export function trackMPPFactors(methodPattern, duration, velocity) {
...
//# sourceMappingURL=wsjf-visualization-selector.js.map