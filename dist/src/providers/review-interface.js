...
export class ReviewInterface {
    reviews;
    pendingReviews; // providerId -> queryIds
    constructor() {
     ...
     */
    async assignForReview(query, providerId) {
     ...
     * Submit review
     */
    async submitReview(review, query) {
        // Validate review
     ...
     */
    async approveQuery(queryId, providerId, options) {
        const review = {
     ...
     */
    async rejectQuery(queryId, providerId, reason) {
        const review = {
            id: `review-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            queryId,
            providerId,
     ...
     */
    async requestAdditionalInfo(queryId, providerId, requestedInfo) {
        const review = {
            id: `review-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            queryId,
            providerId,
     ...
     */
    async escalateQuery(queryId, providerId, reason, targetSpecialization) {
        const review = {
            id: `review-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            queryId,
            providerId,
     ...
     */
    getReviewsForQuery(queryId) {
        return this.reviews.get(queryId) || [];
    }
    /**
     * Get pending reviews for provider
     */
    getPendingReviewsForProvider(providerId) {
     ...
     */
    getProviderReviewHistory(providerId) {
        const history = [];
