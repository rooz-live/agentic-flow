...
export class EscalationRouter {
     ...
     */
    async route(query, availableProviders) {
        ...
        const estimatedResponseTime = this.calculateResponseTime(emergencyType, severityScore.level, bestMatch?.estimatedWaitTime || 60);
        // Generate reasoning
        const reasoning = this.generateReasoning(emergencyType, severityScore, matches.length, escalationRequired);
     ...
     * Determine if escalation is required
     */
    shouldEscalate(severity, emergencyType, matches) {
     ...
     * Calculate estimated response time
     */
    calculateResponseTime(emergencyType, severity, providerWaitTime) {
     ...
     */
    generateReasoning(emergencyType, severityScore, matchCount, escalationRequired) {
        const parts = [];
     ...
     */
    calculateConfidence(matches, severityScore) {
        let confidence = 0.5;
     ...
     */
    async escalateToNextLevel(query, currentProviderId, reason) {
...
//# sourceMappingURL=escalation-router.js.map