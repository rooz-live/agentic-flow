...
function scheduleBufferFlush() {
    if (!flushTimer) {
        flushTimer = setInterval(() => {
            flushIncidentBuffer().catch(err => console.warn('[ProcessGovernor] Flush error:', err));
...
async function waitForCapacity(count = 1) {
    ...
    while (true) {
        ...
        const pollInterval = Math.floor(AF_ADAPTIVE_POLL_MIN_MS +
        ...
        if (now - state.lastLoadCheck > pollInterval) {
            ...
            if (cpuLoad > AF_CPU_CRITICAL_THRESHOLD * 100) {
                ...
                state.currentBackoff = Math.min(state.currentBackoff * AF_ENHANCED_BACKOFF_FACTOR * 1.5, // Faster escalation for critical
                ...
                state.currentBackoff = Math.min(state.currentBackoff * AF_ENHANCED_BACKOFF_FACTOR, AF_ENHANCED_BACKOFF_CEILING_MS);
                state.metrics.backoff_ms = state.currentBackoff;
                continue;
            }
...
function calculateAdaptiveThrottlingLevel() {
    if (!AF_ADAPTIVE_THROTTLING_ENABLED)
        return 1.0;
...
function optimizeExecutionOrder(dependencies) {
    if (!AF_EXECUTION_ORDER_OPTIMIZATION)
        return dependencies;
...
function createOptimalBatches(items, dependencies) {
    if (!AF_BATCH_MAPPING_ENABLED) {
        return Array.from({ length: Math.ceil(items.length / AF_BATCH_SIZE) }, (_, i) => items.slice(i * AF_BATCH_SIZE, (i + 1) * AF_BATCH_SIZE));
    }
    const batches = [];
...
export async function runBatched(items, processor, options) {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        if (batchIndex < batches.length - 1) { // Not the last batch
            const interBatchDelay = Math.floor((1 - state.adaptiveThrottlingLevel) * AF_BACKOFF_MIN_MS);
...
export function reset() {
    ...
    flushIncidentBuffer().catch(err => console.warn('[ProcessGovernor] Final flush error:', err));
    state.activeWork = 0;
    state.queuedWork = 0;
