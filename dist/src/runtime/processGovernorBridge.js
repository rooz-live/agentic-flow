 ...
 */
function buildPatternState(metric) {
    const state = {};
 ...
 */
function calculateEconomicsFallback(metric) {
    ...
    if (metric.pattern === 'safe-degrade' && metric.degraded) {
        ...
        const requested = metric.details.requestedTokens || 0;
        const available = metric.details.availableTokens || 0;
        ...
        const level = metric.details.throttlingLevel || 0;
        cod = base.cod * level;
 ...
 */
function enrichWithEconomics(metric) {
 ...
 */
function getEventDurationMs(eventType) {
    const now = Date.now();
 ...
 */
async function appendToSink(sinkPath, records) {
    if (!ENABLED)
        return;
    try {
...
function scheduleFlush() {
    if (!flushTimer && ENABLED) {
        flushTimer = setInterval(() => {
            flushBuffer().catch(err => console.warn('[ProcessGovernorBridge] Flush error:', err));
 ...
 */
export function ingestGovernorEvent(event) {
    if (!ENABLED)
        return;
    ...
    if (event.type === 'CPU_OVERLOAD' || event.type === 'CIRCUIT_OPEN') {
        flushBuffer().catch(err => console.warn('[ProcessGovernorBridge] Immediate flush failed:', err));
    }
}
/**
...
//# sourceMappingURL=processGovernorBridge.js.map