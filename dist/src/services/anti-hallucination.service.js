...
export class AntiHallucinationService {
     ...
     */
    assessDiagnosisConfidence(diagnoses) {
        if (diagnoses.length === 0)
            return 0;
        const primaryDiagnosis = diagnoses[0];
        let score = primaryDiagnosis.confidence;
        // Reduce score if supporting evidence is weak
        const evidenceStrength = primaryDiagnosis.supportingEvidence.reduce((sum, ev) => sum + ev.reliability, 0) / Math.max(primaryDiagnosis.supportingEvidence.length, 1);
     ...
     */
    validateAgainstKnowledgeBase(analysis) {
        ...
        for (const diagnosis of analysis.diagnosis) {
            ...
            for (const criteria of knowledge.diagnosticCriteria) {
                ...
                const hasRequired = criteria.required.every(req => diagnosis.supportingEvidence.some(ev => ev.description.toLowerCase().includes(req.toLowerCase())));
                if (hasRequired)
                    criteriaMatch++;
            }
            validationCount++;
     ...
     */
    generateWarnings(analysis, confidenceScore) {
        const warnings = [];
     ...
     */
    requiresProviderReview(confidenceScore) {
        return confidenceScore.overall < this.CONFIDENCE_THRESHOLDS.REQUIRES_REVIEW;
    }
    // Helper methods
    generateConfidenceExplanation(score, factors) {
    ...
    areMutuallyExclusive(condition1, condition2) {
        ...
        return exclusivePairs.some(pair => (condition1.toLowerCase().includes(pair[0]) && condition2.toLowerCase().includes(pair[1])) ||
            (condition1.toLowerCase().includes(pair[1]) && condition2.toLowerCase().includes(pair[0])));
    }
    areContradictoryRecommendations(rec1, rec2) {
     ...
     */
    loadKnowledgeBase(entry) {
        this.knowledgeBase.set(entry.condition, entry);
    }
}
//# sourceMappingURL=anti-hallucination.service.js.map