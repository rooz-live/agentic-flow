...
export class MedicalAnalyzerService {
     ...
     */
    sanitizePatientData(patientData) {
        return {
            ...patientData,
    ...
    async generateDiagnosis(patientData, analysis) {
    ...
    async generateRecommendations(patientData, diagnosis) {
    ...
    async identifyRiskFactors(patientData, diagnosis) {
        const riskFactors = [];
        ...
        const matchedDiabetesSymptoms = patientData.symptoms.filter(s => diabetesSymptoms.some(ds => s.toLowerCase().includes(ds)));
    ...
    async runHallucinationChecks(analysis, diagnosis, citations) {
        const checks = [];
    ...
    calculateConfidence(checks, crossCheckCount, citations, diagnosis = []) {
...
//# sourceMappingURL=medical-analyzer.js.map