...
import { selectOffhostSyslogProvider, } from './providerSelection';
...
export async function probeDnsLookup(hostname, timeoutMs = 2000) {
    try {
...
export async function emitDriftEventToSyslog(event, options) {
    ...
    const spawnFn = options?.spawnFn ?? ((cmd, args) => spawn(cmd, args));
    try {
...
export function buildDefaultDriftChecksFromEnv() {
    const selected = selectOffhostSyslogProvider();
    const provider = selected?.candidate.providerId ?? 'aws_lightsail';
...
export async function runDriftChecks(checks, options) {
    const sourceHost = options?.sourceHost ?? os.hostname();
    const emit = options?.emit ?? ((event) => emitDriftEventToSyslog(event));
    ...
    for (const check of effectiveChecks) {
        ...
        if (check.type === 'dns') {
            ...
            const status = dnsRes.ok ? 'ok' : 'fail';
            const event = {
        ...
        const status = tcpRes.ok ? 'ok' : 'fail';
        const event = {
            ts: new Date().toISOString(),
            source: sourceHost,
            check: check.id,
...
export async function runDriftMonitorCli(argv = process.argv.slice(2)) {
    let noEmit = false;
...
//# sourceMappingURL=driftMonitor.js.map