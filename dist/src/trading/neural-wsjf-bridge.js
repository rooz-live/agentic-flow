...
function tryLoadNative() {
    if (_nativeLoadAttempted)
        return _native;
    _nativeLoadAttempted = true;
    try {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const nt = require('neural-trader');
        if (typeof nt.calculateKellyCriterion === 'function' &&
 ...
 */
function kellyFractionTS(winRate, avgWin, avgLoss) {
 ...
 */
function emaCriticalityDecay(data, horizon, alpha = 0.3) {
    if (data.length === 0) {
 ...
 */
function monteCarloAllocTS(initialValue, expectedReturn, volatility, simulations, periods) {
    const results = [];
...
export class NeuralWsjfBridge {
    ...
    scoreWsjfItem(item) {
        ...
        const kellyAdjustedSize = Math.max(1, item.job_size * (1 - kelly.fraction * 0.5));
        // --- Criticality decay forecast ---
        // Simulate a decaying time_criticality signal
        const criticalitySeries = Array.from({ length: 10 }, (_, i) => item.time_criticality * Math.exp(-0.05 * i));
        ...
        // --- Composite WSJF ---
        const numerator = item.business_value + item.time_criticality + item.risk_reduction;
        const wsjfScore = kellyAdjustedSize > 0 ? numerator / kellyAdjustedSize : 0;
        const horizon = wsjfScore >= 20 ? 'NOW' : wsjfScore >= 10 ? 'NEXT' : 'LATER';
        return {
            id: item.id,
            wsjfScore,
            kellyAdjustedSize,
     ...
     */
    predictCriticalityDecay(historicalValues, horizon = 5) {
        if (this.native) {
     ...
     */
    allocateMultiRisk(items, totalCapacity = 1.0, simulations = 1000) {
        if (items.length === 0)
            return [];
        const scored = items.map((item) => {
            ...
            const mc = this.native
                ...
                : monteCarloAllocTS(item.business_value, kelly.fraction, (1 - winRate) * 0.5, simulations, 10);
            return {
                item,
        ...
        const totalEv = scored.reduce((sum, s) => sum + Math.max(0.01, s.ev), 0);
        return scored.map((s) => ({
            itemId: s.item.id,
            allocation: totalCapacity * (Math.max(0.01, s.ev) / totalEv),
            expectedValue: s.ev,
...
//# sourceMappingURL=neural-wsjf-bridge.js.map