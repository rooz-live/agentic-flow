...
const COLLAPSE_PATTERN_CATEGORIES = {
    moral_detachment: {
        patterns: [
            { regex: /\b(we believe in|committed to|dedicated to)\b.*\b(without|but)\b/gi, severity: 'concerning' },
            { regex: /\b(values|principles|ethics)\b.*\b(however|although|despite)\b/gi, severity: 'concerning' },
        ],
        weight: 1.5,
    ...
    confidence_inflation: {
        patterns: [
            { regex: /\b(guaranteed|always|never)\b/gi, severity: 'early' },
            { regex: /\b(100%|zero risk|completely|absolutely)\b/gi, severity: 'concerning' },
            { regex: /\b(perfect|flawless|bulletproof)\b/gi, severity: 'critical' },
        ],
        weight: 2.0,
    ...
    dissent_suppression: {
        patterns: [
            { regex: /\b(concerns? (are|were) (noted|acknowledged) but)\b/gi, severity: 'concerning' },
            { regex: /\b(dissent|disagreement|objection).*\b(addressed|handled|managed)\b/gi, severity: 'concerning' },
            { regex: /\b(decided|resolved|closed).*\b(discussion|debate|conversation)\b/gi, severity: 'early' },
        ],
        weight: 1.8,
    ...
    suffering_narrative: {
        patterns: [
            { regex: /\b(learning experience|growth opportunity)\b.*\b(failure|loss|harm)\b/gi, severity: 'concerning' },
            { regex: /\b(necessary|acceptable|unavoidable)\b.*\b(cost|sacrifice|tradeoff)\b/gi, severity: 'early' },
        ],
        weight: 1.3,
    ...
    authority_drift: {
        patterns: [
            { regex: /\b(policy requires|mandate|compliance)\b.*\b(regardless|despite)\b/gi, severity: 'concerning' },
            { regex: /\b(decided|determined|ruled)\b.*\b(by|from)\b.*\b(leadership|management|exec)\b/gi, severity: 'early' },
        ],
        weight: 1.4,
...
export class CollapseResistanceMonitor {
    ...
    calculateDimensionalAlignment(signals) {
    ...
    generateRecommendations(signals, healthScore) {
        const recs = [];
        if (healthScore < 0.3) {
...
//# sourceMappingURL=collapse_resistance.js.map