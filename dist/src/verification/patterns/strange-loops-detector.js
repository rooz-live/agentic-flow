...
export class StrangeLoopsDetector {
     ...
     */
    async detectCircularReasoning(text, context) {
        const patterns = [];
     ...
     */
    detectSelfReferentialPatterns(text) {
        const patterns = [];
     ...
     */
    detectRelationship(claim1, claim2) {
    ...
    assessCycleSeverity(cycle) {
        ...
        if (cycle.length <= 2)
            return 'critical'; // Direct self-reference
        if (cycle.length === 3)
            return 'high';
     ...
     */
    async detectContradictions(text, context) {
        const patterns = [];
     ...
     */
    detectWithinSentenceContradictions(text) {
        const patterns = [];
        const lowerText = text.toLowerCase();
     ...
     */
    checkContradiction(claim1, claim2) {
        const text1 = claim1.claim.toLowerCase();
        const text2 = claim2.claim.toLowerCase();
     ...
     */
    async validateCausalChain(chain, context) {
        const issues = [];
        // Check for cycles
     ...
     */
    async detectRecursivePatterns(text, maxDepth = this.MAX_RECURSION_DEPTH) {
        const patterns = [];
...
//# sourceMappingURL=strange-loops-detector.js.map