...
export class VerificationPipeline {
    ...
    initializeHallucinationPatterns() {
        ...
        this.hallucinationPatterns.set('overconfident', /\b(always|never|absolutely|definitely|certainly|guaranteed|100%|impossible)\b/gi);
        // Unsupported quantitative claims
        this.hallucinationPatterns.set('unsupported-numbers', /\b(\d+%|\d+\s*times?(\s+(more|less|better|worse))?)\b/gi);
        // Temporal hallucinations
        this.hallucinationPatterns.set('temporal-vague', /\b(recent studies|latest research|modern medicine)\b/gi);
        // Medical jargon without support
        this.hallucinationPatterns.set('unsupported-medical', /\b(cures?|eliminates?|completely|permanently fixes?)\b/gi);
    }
    /**
     * Pre-output verification - check before generating output
     */
    async preOutputVerification(input) {
        ...
        const confidence = await this.confidenceScorer.calculateConfidence(input.claim, input.citations || [], input.context);
     ...
     */
    async detectHallucinations(text, context) {
        const hallucinations = [];
     ...
     */
    detectPatternHallucinations(text) {
        const hallucinations = [];
     ...
     */
    detectQuantitativeHallucinations(text) {
        const hallucinations = [];
     ...
     */
    detectTemporalHallucinations(text) {
        const hallucinations = [];
     ...
     */
    async postOutputValidation(output, originalInput) {
     ...
     */
    checkOutputFidelity(input, output) {
     ...
     */
    async addProviderReview(claimId, review) {
     ...
     */
    getProviderReviews(claimId) {
        return this.providerReviews.get(claimId) || [];
    }
    /**
     * Generate suggestions for improvement
     */
    generateSuggestions(confidence, hallucinations) {
        const suggestions = [];
