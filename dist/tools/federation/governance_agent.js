...
import { computeCodBaselineDeltas, getActionKeys, readJsonl, summarizePatterns, } from './shared_utils';
import { publishStreamEvent, resolveStreamSocket } from './streamPublisher';
import { WSJFCalculator } from './wsjf_calculator';
async function compareAgainstBaseline({ patterns, }) {
 ...
 */
function emitPatternMetric(pattern, mode, gate, reason, action, metrics, tags = ['Federation'], actionCompleted = true) {
...
function summarizeGovernance(metrics) {
    let total = 0;
    let ok = 0;
...
function calculateRelentlessMetrics(goalieDir, cycleLog = []) {
...
function proposeFix(missingPattern) {
...
function proposeCodeFix(pattern, context) {
    const baseProposal = {
        pattern,
...
function generateCodeFixProposals(patterns) {
    const proposals = [];
...
function loadRetroCoachContext(goalieDir) {
    ...
    try {
        ...
        const insights = data && typeof data.insightsSummary === 'object'
...
function ensureApproverRoleForHighRisk(pattern, existing) {
    if (existing)
        return existing;
    switch (pattern) {
        case 'ml-training-guardrail':
...
function lookupPatternMeta(pattern, patterns, topEconomicGaps) {
    ...
    const circle = ev && ev.circle ? String(ev.circle) : 'n/a';
    const depth = ev && typeof ev.depth === 'number' ? ev.depth : 0;
    return {
        circle,
        depth,
        totalImpactAvg: 0,
...
function classifyProposalRisk(proposal, meta) {
...
function applyAutoApplyPolicy(proposals, patterns, topEconomicGaps, retroCtx) {
    ...
    const normalizeHighRisk = (proposal) => {
        ...
        // eslint-disable-next-line no-console
        console.error(`[governance_agent] High-risk pattern "${proposal.pattern}" requires manual approval (${approverRole || 'approver role not set'})`);
        return {
            ...proposal,
            mode: 'dry-run',
            approvalRequired: true,
...
async function printGovernanceRecommendations(patterns, metrics, cycleLog) {
    ...
    console.log('Governance reviews: total=%d, ok=%d, failed=%d', govSummary.total, govSummary.ok, govSummary.failed);
    console.log(`Relentless Execution: Actions Done=${relentless.pctActionsDone.toFixed(1)}%, Avg Cycle Time=${relentless.avgCycleTimeSec.toFixed(1)}s`);
    console.log('\nKey patterns:');
    const keys = [
        'safe-degrade',
        'guardrail-lock',
    ...
    if ((patternCounts.get('observability-first') || 0) === 0) {
        if (isProdCycle()) {
            ...
            emitPatternMetric('observability-first', 'enforcement', 'prod-cycle-gate', 'observability-first pattern missing in prod-cycle', 'block-and-suggest-fix', {
                enforced: 1,
                missing_signals: 1,
                suggestion_made: 1,
            }, ['Federation', 'Observability'], false);
            ...
            emitPatternMetric('observability-first', 'advisory', 'governance-review', 'observability-first pattern recommended', 'suggest', {
                missing_signals: 1,
                suggestion_made: 1,
            }, ['Federation', 'Observability'], true);
...
async function summarizeObservabilityActions(goalieDir) {
    ...
    try {
        ...
        for (const [key, group] of Array.from(byKey.entries()).sort()) {
            ...
            console.log(`- circle=${circle}, depth=${depth} · actions=${group.length} · tags=[${Array.from(tags).join(', ')}]`);
        }
    }
    catch {
 ...
 */
async function printTopEconomicGaps(patterns, actionKeys) {
    ...
    for (const ev of patterns) {
        ...
        if (ev.fix_proposal)
            current.fixProposal = ev.fix_proposal;
        if (typeof cod === 'number' && !Number.isNaN(cod))
            current.codVals.push(cod);
        if (typeof compute === 'number' && !Number.isNaN(compute))
            current.computeCostVals.push(compute);
    ...
    for (const row of rows.slice(0, 5)) {
        ...
        console.log(`- ${row.pattern} · circle=${row.circle}, depth=${row.depth} · events=${row.count} · ${impactStr} ${breakdownStr}, ${wsjfStr}`);
        if (row.fixProposal) {
...
export function createRealTimeFeed(goalieDir) {
    return new RealTimeFeed(goalieDir);
}
function getDynamicThresholds(metrics) {
    const govSummary = summarizeGovernance(metrics);
...
function computeTopEconomicGapsForJson(patterns, actionKeys) {
    if (!patterns.length || !actionKeys.size) {
        return [];
    }
    const agg = new Map();
    // Expanded interestingPatterns set - includes all patterns from PATTERNS.yaml
    const interestingPatterns = new Set([
        // ML Patterns
        'ml-training-guardrail',
        'distributed-training-failure',
        'mixed-precision-check',
    ...
    for (const ev of patterns) {
        ...
        const isPlatformPattern = pattern.includes('mobile') ||
            pattern.includes('desktop') ||
            pattern.includes('web') ||
            ...
            tags.includes('Desktop') ||
            tags.includes('Web');
        if (!interestingPatterns.has(pattern) && !isPlatformPattern)
            continue;
        ...
        if (typeof cod === 'number' && !Number.isNaN(cod))
            current.codVals.push(cod);
        if (typeof economic.wsjf_score === 'number')
            current.wsjfVals.push(economic.wsjf_score);
        if (typeof economic.computeCost === 'number')
            current.computeCostVals.push(economic.computeCost);
        const p99 = ev.p99_latency_ms;
        if (typeof p99 === 'number')
            current.p99LatencyVals.push(p99);
        const nodes = ev.node_count;
        if (typeof nodes === 'number')
            current.nodeCountVals.push(nodes);
        const queueRaw = ev.queue_time_sec ?? ev.queue_time_s ?? ev.queue_time;
        if (typeof queueRaw === 'number' && !Number.isNaN(queueRaw)) {
            current.queueTimeVals.push(queueRaw);
        }
...
async function buildGovernanceJsonOutput(goalieDir, patterns, metrics, cycleLog) {
    const govSummary = summarizeGovernance(metrics);
    ...
    if (govSummary.failed > thresholds.failedThreshold) {
        suggested.push(`Governance: high failure rate (${govSummary.failed}/${govSummary.total}) above dynamic threshold ${thresholds.failedThreshold.toFixed(1)}. Tighten pre-merge checks and guardrail tests.`);
    }
    if (relentless.pctActionsDone < 60) {
        suggested.push(`Relentless execution: only ${relentless.pctActionsDone.toFixed(1)}% of actions done. Reduce WIP and close the loop on open governance actions.`);
    }
    if ((patternCounts.get('observability-first') || 0) === 0) {
        if (isProdCycle()) {
            suggested.push('[GOVERNANCE FAILURE] Prod-Cycle Enforcement: "observability-first" pattern is MISSING. In prod-cycle, you MUST enable observability-first to proceed.');
            ...
            suggested.push('Observability: consider enabling AF_PROD_OBSERVABILITY_FIRST for prod-cycle runs.');
            suggested.push(proposeFix('observability-first'));
        }
    }
    ...
    codeFixProposals = applyAutoApplyPolicy(codeFixProposals, patterns, topEconomicGaps, retroCtx);
    // Emit pattern event for WSJF enrichment and economic gap analysis
    const timestamp = new Date().toISOString().replace(/\.\d{3}Z$/, 'Z');
    const runKind = process.env.AF_RUN_KIND || 'governance-agent';
    const gapTags = new Set(['Federation']);
    ...
    if (codeFixProposals.length > 0) {
        const proposalTags = new Set(['Federation']);
    ...
    if (fs.existsSync(actionsPath)) {
        try {
            ...
            for (const it of items) {
                ...
                if (!bucket) {
                    bucket = { circle, depth, actions: 0, tags: new Set() };
                    grouped.set(key, bucket);
                }
                bucket.actions += 1;
                if (Array.isArray(it.tags)) {
                    for (const t of it.tags)
                        bucket.tags.add(String(t));
                }
            }
 ...
 */
function applyPatternAnalysisAdjustments(goalieDir, adjustments, dryRun = true) {
    ...
    for (const adj of adjustments) {
        ...
        if (dryRun) {
            ...
            emitPatternMetric('governance-tuning', 'mutate', 'parameter-adjustment', reason, `set-${param}`, {
                parameter: param,
                old_value: adj.current_value,
                new_value: suggestedValue,
                trigger: adj.pattern_trigger,
            }, ['Federation', 'Governance', 'Auto-Adjustment'], true);
        }
    }
}
async function main() {
    ...
    const cycleLog = fs.existsSync(cycleLogPath) ? await readJsonl(cycleLogPath) : [];
    ...
    if (wsjfMode || jsonMode) {
        ...
        const governanceDecisions = await generateAutomatedGovernanceDecisions(wsjfResults, batchRecommendations, patterns, metrics);
        const wsjfPayload = {
    ...
    const exampleState = { pattern: 'safe-degrade', failedReviews: 3, action: '' };
    ...
    if (patternAnalysis && patternAnalysis.governance_adjustments) {
        ...
        applyPatternAnalysisAdjustments(goalieDir, patternAnalysis.governance_adjustments, dryRun);
...
async function generateAutomatedGovernanceDecisions(wsjfResults, batchRecommendations, patterns, metrics) {
    ...
    for (const result of wsjfResults) {
        ...
        if (result.riskAssessment.riskLevel >= 8)
            highRiskCount++;
        else if (result.riskAssessment.riskLevel >= 5)
            mediumRiskCount++;
        else
            lowRiskCount++;
...
function generateRiskSummary(wsjfResults) {
    const riskDistribution = { critical: 0, high: 0, medium: 0, low: 0 };
    const riskFactors = new Map();
    for (const result of wsjfResults) {
        // Count risk levels
        if (result.riskAssessment.riskLevel >= 9)
            riskDistribution.critical++;
        else if (result.riskAssessment.riskLevel >= 7)
            riskDistribution.high++;
        else if (result.riskAssessment.riskLevel >= 4)
...
function generateMitigationStrategiesFromRiskFactors(riskFactors) {
    const strategies = [];
...
function calculateEconomicImpact(wsjfResults) {
    const totalCostOfDelay = wsjfResults.reduce((sum, result) => sum + result.costOfDelay, 0);
    const potentialSavings = wsjfResults.reduce((sum, result) => sum + (result.wsjfScore * result.parameters.jobDuration), 0);
...
async function applyAutomatedGovernanceActions(autoApplyItems, goalieDir) {
...
async function executeCodeFix(proposal) {
...
async function executeBatchRecommendations(batchRecommendations, goalieDir) {
...
export function simulateAnalyst(context, emit = true) {
    // Analyst focuses on data quality, lineage, and standards
    if (emit) {
        emitPatternMetric('analyst-data-quality-check', 'advisory', 'quality-gate', 'Checking data lineage and schema compliance', 'verify', {
            role: 'Analyst',
            completeness: 0.95,
...
export function simulateAssessor(context, emit = true) {
    // Assessor focuses on verification, audit, and compliance
    if (emit) {
        emitPatternMetric('assessor-verification', 'enforcement', 'security-gate', 'Verifying security controls and audit trails', 'audit', {
            role: 'Assessor',
            audit_pass: true,
            findings: []
        });
...
//# sourceMappingURL=governance_agent.js.map