...
import { computeCodBaselineDeltas, getActionKeys, readJsonl, summarizePatterns, } from './shared_utils.js';
 ...
 */
function initializeTimelineDeltaContext() {
 ...
 */
function createSignedTimelineDelta(deltaType, payload) {
 ...
 */
function verifyTimelineDelta(delta) {
    try {
 ...
 */
function verifyMerkleChain(chain, deltas) {
 ...
 */
function emitPatternMetric(pattern, mode, gate, reason, action, metrics, tags = ['Federation']) {
...
const PATTERN_EVIDENCE_SOURCES = {
...
const PRIORITY_VERIFICATION_THRESHOLDS = {
...
function validateVerificationThreshold(threshold, currentValue) {
    if (!threshold)
        return true;
...
async function analyzeRCATriggers(goalieDir, currentIteration) {
    ...
    const lines = await readJsonl(metricsPath);
    const patternLines = fs.existsSync(patternMetricsPath) ? await readJsonl(patternMetricsPath) : [];
    // Heuristics state
    let consecutiveFailures = 0;
    let lastFailureContext = '';
    const retryAttempts = {};
    const errorCounts = {};
    const circleStagnation = {};
...
function performSimpleForensicVerification(insights, metrics) {
    const actionCompletedEvents = metrics.filter(m => m.type === 'action_completed' || m.event === 'action_completed');
...
function performEnhancedForensicVerification(goalieDir, patterns) {
    ...
    for (const action of completedActions) {
        ...
        if (!completedTs)
            continue;
        const actionId = action.id || action.key || action.slug || 'unknown';
        const targetPattern = action.pattern || action.target_pattern || 'unknown';
        ...
        if (freqPre > 0 || freqPost > 0) {
            ...
            const wsjfImproved = wsjfPre !== undefined && wsjfPost !== undefined && wsjfPost >= wsjfPre * 1.1;
    ...
    if (!windows.length) {
        console.warn('[retro_coach] Completed actions found but none could be mapped to pattern events; falling back to simple verification');
        return { verifiedCount: 0, totalActions: 0 };
    }
    ...
    const avgCodDeltaPct = codDeltaPcts.length > 0
    ...
    const unverifiedHighPriorityActions = windows
        ...
        const codValues = patternEventsForAction
            .map(e => (e.economic && typeof e.economic.cod === 'number' ? e.economic.cod : undefined))
            .filter((v) => typeof v === 'number');
        const codAvg = codValues.length > 0 ? codValues.reduce((a, b) => a + b, 0) / codValues.length : 0;
        return {
            actionId: w.actionId,
            pattern: w.pattern,
            codAvg,
...
function extractMetricsFromPatterns(patterns) {
    let totalScore = 0;
    let scoreCount = 0;
    const riskCounts = {};
...
function workloadsForPattern(pattern, extraTags = []) {
    ...
    if (pattern === 'multiple-testing-correction' ||
    ...
    if (pattern === 'mobile-interaction-lag' ||
...
function loadCodThresholdConfig(goalieDir) {
    const defaults = {
    ...
    try {
        ...
        const patternThresholds = { ...defaults.patternThresholds };
...
async function computeTopEconomicGaps(patterns, actionKeys) {
    if (!patterns.length) {
        return [];
    }
    const goalieDir = getGoalieDirFromArgs();
    ...
    for (const ev of patterns) {
        ...
        if (!interesting.has(pattern))
            continue;
        const circle = ev.circle || '<none>';
        const depthVal = ev.depth;
        const depth = typeof depthVal === 'number' ? depthVal : 0;
        const econ = ev.economic || {};
        ...
        const frameworkRaw = ev.framework;
        ...
        const schedulerRaw = ev.scheduler;
    ...
    for (const a of agg.values()) {
        ...
        if (Object.prototype.hasOwnProperty.call(codConfig.patternThresholds, a.pattern)) {
            ...
            else if (workloads.includes('Device/Web') &&
...
async function loadBaselineMetrics(goalieDir) {
    ...
    try {
        ...
        const riskDist = obj.risk_distribution && typeof obj.risk_distribution === 'object'
            ? obj.risk_distribution
            : undefined;
        const ts = typeof obj.analysis_timestamp === 'string'
...
async function buildRetroJsonOutput(goalieDir, insights, patterns, gaps, baseline, current) {
    ...
    const observabilityRisks = insights.filter(i => i.type === 'roam_risk' && i.category === 'observability');
    ...
    const topEconomicGaps = gaps.slice(0, 5).map(row => ({
        pattern: row.pattern,
        circle: row.circle,
        depth: row.depth,
        count: row.count,
    ...
    const frameworkLabel = (hint) => {
        ...
        if (key === 'tf' || key === 'tensorflow')
            return 'TensorFlow';
        if (key === 'torch' || key === 'pytorch')
            return 'PyTorch';
        if (key === 'mixed')
            return 'mixed frameworks';
        return hint;
    };
    const schedulerLabel = (hint) => {
        if (!hint)
            return undefined;
        const key = hint.toLowerCase();
    ...
    const patternDeltas = computeCodBaselineDeltas(patterns);
    ...
    if (patternDeltas.length) {
        ...
        if (worst.length)
            topRegressions.push(...worst);
        if (best.length)
            topImprovements.push(...best);
        if (worst.length) {
            const w = worst[0];
            const tagsLabel = w.workloadTags && w.workloadTags.length ? ` (${w.workloadTags.join('/')})` : '';
            if (typeof w.deltaPct === 'number') {
                workloadPrompts.push(`Regression${tagsLabel}: Pattern "${w.pattern}" at circle=${w.circle}, depth=${w.depth} regressed by ${w.deltaPct.toFixed(1)}% (CoD from ${w.baselineScore?.toFixed(2)} to ${w.currentScore?.toFixed(2)}). What changed between baseline and now?`);
            }
        }
        if (best.length) {
            const b = best[0];
            ...
            if (typeof b.deltaPct === 'number') {
                workloadPrompts.push(`Improvement${tagsLabel}: Pattern "${b.pattern}" at circle=${b.circle}, depth=${b.depth} improved by ${b.deltaPct.toFixed(1)}% (CoD from ${b.baselineScore?.toFixed(2)} to ${b.currentScore?.toFixed(2)}). What governance or observability changes drove this?`);
            }
        }
    }
    if (baseline || current || topRegressions.length || topImprovements.length) {
        ...
        baselineComparison = {
            ...
            delta,
            deltaPct,
            regression: baselineScore !== undefined && currentScore !== undefined
                ? currentScore < baselineScore * 0.9
                : undefined,
            improvement: baselineScore !== undefined && currentScore !== undefined
    ...
    let observabilityGaps;
    if (observabilityRisks.length > 0) {
        const gapsList = observabilityRisks.map(risk => ({
            failure_id: risk.failure_id,
            failure_type: risk.failure_type,
            gap_severity: risk.gap_severity,
            risk_level: risk.risk_level,
            timestamp: risk.timestamp,
            suggested_action: risk.suggested_action,
            failure_context: {
                timestamp: risk.failure_context?.timestamp,
                source_log: risk.failure_context?.source_log,
                details: risk.failure_context?.details,
            },
        }));
...
async function loadCurrentMetrics(goalieDir) {
    const metricsPath = path.join(goalieDir, 'metrics_log.jsonl');
    if (!fs.existsSync(metricsPath)) {
        return null;
    }
...
async function printRetroRecommendations(insights, patterns, gaps, baseline, current) {
    const patternCounts = summarizePatterns(patterns);
    const recentInsights = insights.slice(-10);
    ...
    if (observabilityRisks.length > 0) {
        ...
        emitPatternMetric('observability-first', 'advisory', 'retro-analysis', `Detected ${highRisk} high-risk and ${mediumRisk} medium-risk observability gaps`, 'analyze-and-recommend', {
    ...
    const schedulerLabel = (hint) => {
        ...
        if (key === 'lsf')
            return 'LSF cluster';
        if (key === 'pbs' || key === 'torque')
            return 'batch scheduler cluster';
        if (key === 'k8s' || key === 'kubernetes')
            return 'Kubernetes cluster';
        if (key === 'local' || key === 'workstation')
            return 'local workstation';
        if (key === 'mixed')
            return 'mixed schedulers';
        return hint;
    };
    const describeEnv = (row) => {
    ...
    if (forensic.totalActions > 0) {
        ...
        emitPatternMetric('forensic-verification', 'advisory', 'retro-analysis', `Verified ${forensic.verifiedCount}/${forensic.totalActions} actions (${verificationRate.toFixed(1)}%)`, 'forensic-analysis', {
    ...
    console.log(`Forensic Verification: ${forensic.verifiedCount}/${forensic.totalActions} actions verified (actual)`);
    ...
    if (!gaps.length) {
        ...
        for (const row of gaps.slice(0, 5)) {
            ...
            console.log(`- ${row.pattern} · circle=${row.circle}, depth=${row.depth} · events=${row.count} · ${codStr}, ${wsjfStr}`);
        }
    }
    ...
    if (mlGap) {
        ...
        console.log(`- ML${mlEnv}: High-COD gap on "${mlGap.pattern}" at circle=${mlGap.circle}, depth=${mlGap.depth} (cod≈${mlGap.codAvg.toFixed(2)}). Where did training or evaluation observability fall short for this environment?`);
    }
    const hpcGap = gaps.find(row => (row.workloadTags || workloadsForPattern(row.pattern)).includes('HPC'));
    if (hpcGap) {
        const hpcEnv = describeEnv(hpcGap);
        console.log(`- HPC / batch workloads${hpcEnv}: High-COD gap on "${hpcGap.pattern}" at circle=${hpcGap.circle}, depth=${hpcGap.depth} (cod≈${hpcGap.codAvg.toFixed(2)}). Did degradation and batch window handling match failure and restart expectations for this environment?`);
    }
    ...
    if (statsDeviceGap) {
        ...
        console.log(`- Stats / device / web${statsEnv}: High-COD gap on "${statsDeviceGap.pattern}" at circle=${statsDeviceGap.circle}, depth=${statsDeviceGap.depth} (cod≈${statsDeviceGap.codAvg.toFixed(2)}). Were statistical checks and device/browser coverage sufficient for this environment when things went wrong?`);
    }
    if (!mlGap && !hpcGap && !statsDeviceGap) {
        console.log('- No ML/HPC/Stats/Device-Web-specific economic gaps detected; focus retro on the broader pattern prompts below.');
    }
    ...
    if (!baseline && !current) {
        ...
        console.log(`- Baseline average_score=${baseline.averageScore.toFixed(2)}; current run did not emit comparable scoring metrics. Consider wiring average_score + risk_distribution into .goalie/metrics_log.jsonl.`);
    }
    else if (!baseline && current) {
        console.log(`- Current average_score=${current.averageScore.toFixed(2)}, but no baseline.json found. Consider capturing this run as the initial baseline in investing/agentic-flow/metrics/baseline.json.`);
    }
    else if (baseline && current) {
        const baselineScore = baseline.averageScore;
        const currentScore = current.averageScore;
        ...
        console.log(`- Baseline=${baselineScore.toFixed(2)}, current=${currentScore.toFixed(2)} (Δ=${delta.toFixed(2)}, ${deltaPct.toFixed(1)}% vs baseline).`);
        const prodCycle = isProdCycle();
        if (currentScore < baselineScore * 0.9) {
            const contextMsg = prodCycle ? 'during prod-cycle' : '';
            console.log(`- Performance Regression: Score dropped significantly below baseline (more than 10%) ${contextMsg}. Focus retro on what changed in flows, governance, and workload mixes.`);
        }
        else if (currentScore > baselineScore * 1.05) {
            console.log('- Performance Improvement: Score is more than 5% above baseline. Consider updating baseline.json to reflect the new standard and locking in the practices that led here.');
        }
        const baselineP0 = baseline.riskDistribution?.P0 ?? 0;
        const currentP0 = current.riskDistribution?.P0 ?? 0;
        if (currentP0 > baselineP0) {
            console.log(`- Critical Risk Spike: P0 incidents increased from ${baselineP0} to ${currentP0} vs baseline. Treat this as an explicit retro topic with owners and follow-ups.`);
        }
    }
}
function ensureDspyIntegration(goalieDir) {
    try {
        const repoRoot = path.resolve(goalieDir, '..');
        const integrationPath = path.join(repoRoot, 'src', 'integrations', 'dspy.ts');
        if (!fs.existsSync(integrationPath)) {
            // eslint-disable-next-line no-console
            console.warn('[retro_coach] DSPy integration not found at src/integrations/dspy.ts; continuing with rule-based retro prompts only.');
        }
        const dspyModulePath = path.join(repoRoot, 'node_modules', 'dspy.ts');
        if (!fs.existsSync(dspyModulePath)) {
            // eslint-disable-next-line no-console
            console.warn('[retro_coach] DSPy library not installed (node_modules/dspy.ts missing). To enable DSPy-assisted retros, run: npm install dspy.ts');
        }
    }
    catch {
...
async function generateIntelligentInsights(gaps, insights, baseline, current) {
    try {
...
async function emitIterativeRCARecommendation(goalieDir, recommendation, runId) {
    const patternMetricsPath = path.join(goalieDir, 'pattern_metrics.jsonl');
    const timestamp = new Date().toISOString();
    const event = {
        ts: timestamp,
...
async function updateConsolidatedActionsFromRCA(goalieDir, recommendations) {
    const consolidatedPath = path.join(goalieDir, 'CONSOLIDATED_ACTIONS.yaml');
    if (!fs.existsSync(consolidatedPath)) {
    ...
    try {
        ...
        for (const rec of recommendations) {
            ...
            const existingIdx = doc.items.findIndex((item) => item.pattern === rec.detected_pattern && item.rca_method === rec.rca_method);
            if (existingIdx >= 0) {
                // Update priority if higher
                const existing = doc.items[existingIdx];
                const priorityRank = { critical: 4, urgent: 3, important: 2, normal: 1, low: 0 };
...
async function updateActionsWithForensicVerification(goalieDir, forensicWindows) {
    const consolidatedPath = path.join(goalieDir, 'CONSOLIDATED_ACTIONS.yaml');
    if (!fs.existsSync(consolidatedPath)) {
    ...
    try {
        ...
        for (const window of forensicWindows) {
            ...
            const matchingIdx = doc.items.findIndex((item) => item.id === window.actionId || item.pattern === window.pattern);
            if (matchingIdx >= 0) {
...
async function generateCeremonyOutputs(goalieDir, rcaTriggers, forensicResult, runId, iteration) {
    const timestamp = new Date().toISOString();
    const outputs = {};
 ...
 */
function loadPatternAnalysisReport(goalieDir) {
...
async function main() {
    ...
    if (iterativeMode) {
        ...
        if (rcaTriggers.iterativeRecommendations && rcaTriggers.iterativeRecommendations.length > 0) {
            const { added, updated } = await updateConsolidatedActionsFromRCA(goalieDir, rcaTriggers.iterativeRecommendations);
            if (!jsonMode) {
        ...
        if (ceremonyMode || process.env.AF_CEREMONY_OUTPUTS === '1') {
            ...
            if (forensicResult.windows && forensicResult.windows.length > 0) {
                const { verified, highImpact } = await updateActionsWithForensicVerification(goalieDir, forensicResult.windows);
            ...
            const ceremonyOutputs = await generateCeremonyOutputs(goalieDir, rcaTriggers, forensicResult, runId, iteration);
            if (!jsonMode) {
