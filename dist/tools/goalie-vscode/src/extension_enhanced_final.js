...
class GoalieKanbanProvider {
    ...
    dropMimeTypes = ['application/vnd.code.tree.goalieKanbanItem'];
    dragMimeTypes = ['application/vnd.code.tree.goalieKanbanItem'];
        ...
        try {
            ...
            for (const line of lines) {
                try {
                    ...
                    const matches = event?.id === patternId ||
...
export function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalie.promoteToKanban', async (item) => {
        if (!item?.metric)
            return;
        ...
        // Prompt for section
        const section = await vscode.window.showQuickPick(['NOW', 'NEXT', 'LATER'], {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalieKanban.editItem', async (item) => {
        if (!item?.payload?.id)
            return;
        const goalieDir = getGoalieDir(workspaceRoot);
        if (!goalieDir)
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalieKanban.removeItem', async (item) => {
        if (!item?.payload?.id)
            return;
        const confirm = await vscode.window.showWarningMessage(`Are you sure you want to remove '${item.label}'?`, { modal: true }, 'Yes');
        if (confirm === 'Yes') {
            const goalieDir = getGoalieDir(workspaceRoot);
            if (goalieDir) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalieKanban.moveItem', async (item) => {
        if (!item?.payload)
            return;
        const targetSection = await vscode.window.showQuickPick(['NOW', 'NEXT', 'LATER'], {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalie.copyItemValue', async (item) => {
