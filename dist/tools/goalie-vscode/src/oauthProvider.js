 ...
 */
export class GoalieOAuthProvider {
    context;
    static SESSION_KEY = "goalie.oauth.sessions";
    static CALLBACK_PORT = 54321;
    static STATE_TIMEOUT_MS = 300000; // 5 minutes
    ...
    async getSession(domain) {
        ...
        if (session && session.expiresAt <= Date.now()) {
            ...
            if (session.refreshToken) {
                try {
                    const refreshedSession = await this.refreshToken(targetDomain, session.refreshToken);
    ...
    async saveSession(session) {
        ...
        await this.context.globalState.update(GoalieOAuthProvider.SESSION_KEY, sessions);
    ...
    async login(domain) {
        ...
        const session = await this.exchangeCodeForToken(targetDomain, authCode, codeVerifier);
     ...
     */
    async loginWithDomainSelection() {
        const domains = this.getSupportedDomains();
     ...
     */
    async exchangeCodeForToken(domain, code, codeVerifier) {
        ...
        // Fetch user info
        const userInfo = await this.fetchUserInfo(domain, tokenResponse.access_token);
        return {
            domain,
     ...
     */
    async refreshToken(domain, refreshToken) {
        const config = this.domainConfigs[domain];
        const body = new URLSearchParams({
     ...
     */
    async fetchUserInfo(domain, accessToken) {
        const config = this.domainConfigs[domain];
        try {
 ...
 */
export function registerOAuthCommands(context, provider) {
    context.subscriptions.push(vscode.commands.registerCommand("goalie.oauth.login", async () => {
        try {
    ...
    }), vscode.commands.registerCommand("goalie.oauth.loginWithDomain", async () => {
        try {
        ...
        if (isAuthenticated && session) {
            ...
            vscode.window.showInformationMessage(`Authenticated with ${config.displayName}${session.userInfo?.name ? ` as ${session.userInfo.name}` : ""}. Token expires in ${expiresIn} minutes.`);
        }
        else {
            vscode.window
