...
**Canonical public FQDN:** [`FQDN_CANONICAL.md`](FQDN_CANONICAL.md) (`billing.bhopti.com`; CI alias `api.billing.o-gov.com` until cutover).
...
| **invoice-engine** | ✅ N/A (VERIFY-first) | — | ✅ verify | — | — | ✅ COMPLETE |

**All 9 legacy billing domains: VERIFY gap closed. Invoice Engine (domain 10): VERIFY at `tests/invoice-engine-verify.e2e.spec.ts`.**
...
| Protobuf | `/docs/api/billing.proto` | **CANONICAL** (510+ LOC) | Yes |
...
| Public edge FQDN VERIFY spec | CRITICAL | Medium | 9.00 | NEAR | 🟡 DOC (`FQDN_CANONICAL.md` + `public_synthetic_check.sh`) |
| HostBill/Oro/Stripe FQDN config | CRITICAL | Medium | 8.80 | NEAR | 🟡 Caddy `billing.bhopti.com`; CI o-gov pending cutover |
...
| Artifact | Canonical Path | Status |
|----------|---------------|--------|
| Domain schemas (proto) | `docs/api/billing.proto` | ✅ EXISTS — 10 domains incl. Invoice |
...
| Invoice Engine VERIFY | `tests/invoice-engine-verify.e2e.spec.ts` | ✅ EXISTS |
| Python schemas | `src/validation/schema_engine.py` | ✅ EXISTS |
| Invoice domain impl | `src/billing/invoice_engine.py` | ✅ EXISTS |
...
### Public edge blocker (2026-05-26)

Synthetic check `public_synthetic_check.sh billing.bhopti.com` wrote evidence with **exit 1** (see `.goalie/evidence/public-edge/public_*.json`):

| Run | Symptom | curl code |
|-----|---------|-----------|
| `public_20260526T004344Z-53275.json` | SSL: unable to get local issuer certificate | 60 |
| `public_20260526T033933Z-47516.json` | DNS resolve timed out (~15s) | 28 |
