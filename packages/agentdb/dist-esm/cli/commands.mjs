...
import { getAvailableTemplates } from './templates.mjs';
export async function listPlugins(verbose = false) {
        ...
        for (const pluginName of plugins) {
            ...
            try {
                ...
                if (verbose) {
                    ...
                    if (description)
                        console.log(chalk.gray(`    ${description.split(':')[1].trim()}`));
                    if (version)
                        console.log(chalk.gray(`    Version: ${version.split(':')[1].trim()}`));
                    if (algorithm)
                        console.log(chalk.gray(`    Algorithm: ${algorithm.split(':')[1].trim()}`));
                    console.log();
                }
            }
            catch (error) {
...
export async function listTemplates(detailed = false) {
...
export async function getPluginInfo(name, asJson = false) {
