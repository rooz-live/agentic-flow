...
export async function generatePlugin(
  config: PluginConfig | { name: string; template: string; useDefaults?: boolean },
...
async function createDirectoryStructure(pluginDir: string): Promise<void> {
  await fs.mkdir(path.join(pluginDir, 'src'), { recursive: true });
  await fs.mkdir(path.join(pluginDir, 'tests'), { recursive: true });
}

async function saveConfiguration(pluginDir: string, config: PluginConfig): Promise<void> {
  const yamlStr = YAML.stringify(config);
  await fs.writeFile(path.join(pluginDir, 'plugin.yaml'), yamlStr, 'utf-8');
}

async function generateIndexFile(pluginDir: string, config: PluginConfig): Promise<void> {
  const content = `/**
 * ${config.name}
 * ${config.description}
 *
...
async function generateAgentFile(pluginDir: string, config: PluginConfig): Promise<void> {
...
async function generatePolicyFile(pluginDir: string, config: PluginConfig): Promise<void> {
  const content = `/**
...
async function generateTestFile(pluginDir: string, config: PluginConfig): Promise<void> {
  const content = `/**
...
async function generateReadme(pluginDir: string, config: PluginConfig): Promise<void> {
...
async function generatePackageJson(pluginDir: string, config: PluginConfig): Promise<void> {
  const pkg = {
    name: config.name,
    version: config.version,
...
async function generateTsConfig(pluginDir: string): Promise<void> {
  const tsconfig = {
    compilerOptions: {
      target: 'ES2022',
      module: 'ESNext',
...
function getPluginClassName(config: PluginConfig): string {
  return config.name
    .split('-')
...
function getAgentClassName(config: PluginConfig): string {
  return config.name
    .split('-')
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join('') + 'Agent';
...
function generateAlgorithmImplementation(config: PluginConfig) {
