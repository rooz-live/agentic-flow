...
imports_to_add = "from dataclasses import dataclass\nfrom typing import Dict, Any, List, Optional, Protocol, Tuple\nimport random\n"
content = re.sub(r'from typing import Dict, Any, List, Optional', imports_to_add, content)

refactored_code = """class SystemLoadSensor(Protocol):
...
# replace from class AdmissionController to get_metrics
pattern = r"class AdmissionController:[\s\S]*?def get_metrics\(self\) -> dict:"
content = re.sub(pattern, refactored_code, content)

with open("scripts/policy/governance.py", "w") as f:
    f.write(content)
