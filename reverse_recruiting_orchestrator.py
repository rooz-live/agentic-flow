...
class ReverseRecruitingPipeline:
    def __init__(self):
        # CSQBM Governance Constraint: Enforce truth matrix
        csqbm_path = os.path.join(os.path.dirname(__file__), "scripts/validators/project/check-csqbm.sh")
        if os.path.exists(csqbm_path):
            try:
                subprocess.run([csqbm_path], check=True, capture_output=True)
            except subprocess.CalledProcessError:
                print("❌ CSQBM Constraint Failed. Reality matrix bounds breached.")
                exit(1)
