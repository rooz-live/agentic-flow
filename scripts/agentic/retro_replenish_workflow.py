...
def main():
    ...
    # CSQBM Governance Constraint: Retro Replenish execution bounds
    try:
        proj_root = subprocess.run(["git", "rev-parse", "--show-toplevel"], capture_output=True, text=True, check=True).stdout.strip()
        csqbm_path = os.path.join(proj_root, "scripts", "validators", "project", "check-csqbm.sh")
        if os.path.exists(csqbm_path):
            subprocess.run(["bash", csqbm_path], check=True)
    except Exception as e:
        print(f"⚠️ CSQBM boundary check skipped or failed: {e}")
