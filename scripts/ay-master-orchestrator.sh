...
initialize_swarm() {
  ...
  if command -v Task >/dev/null 2>&1; then
    echo "Using Task tool for swarm initialization..."
    # Task tool invocation would go here when available
    # Task({
    #   prompt: "Initialize hierarchical-mesh swarm with 15 specialized agents",
    #   subagent_type: "swarm-initializer",
    #   run_in_background: true,
    #   description: "Persistent swarm initialization and coordination"
    # })
    exit_code=0  # EXIT_SUCCESS
  else
    echo "Task tool unavailable, falling back to CLI..."
