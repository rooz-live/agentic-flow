#!/usr/bin/env bash
# unstage_scope_creep.sh — Keep gate-allowlisted + active P1-INDEX-02 manifest paths staged.
set -euo pipefail
...
allow = {
    "code/tooling/scripts/dod-gate.sh",
    "code/tooling/scripts/agent_session_dor.sh",
    "code/tooling/scripts/lib/evidence_json.sh",
    "code/tooling/scripts/public_synthetic_check.sh",
    "scripts/cicd/continuous_learning_swarm.sh",
    "scripts/cicd/perceive_tick.sh",
    "scripts/cicd/index_slice_allowlist.sh",
    "scripts/cicd/index_slice_substrate.sh",
    "scripts/cicd/trust_path_owner.sh",
    "scripts/cicd/hold_unblock_w3.sh",
    "scripts/cicd/run_loop_tick.sh",
    "scripts/cicd/unstage_scope_creep.sh",
    ...
    "tests/load/k6_billing_smoke.js",
    "scripts/cicd/emit_perf_baseline.sh",
...
manifest = os.path.join(root, ".goalie/evidence/learning/index_substrate_manifest.json")
if os.path.isfile(manifest):
    try:
        allow.update(json.load(open(manifest)).get("staged", []))
    except (json.JSONDecodeError, OSError):
        pass
staged = subprocess.check_output(["git", "diff", "--cached", "--name-only"], cwd=root, text=True).splitlines()
unstage = [p for p in staged if p not in allow]
if unstage:
    subprocess.run(["git", "restore", "--staged", "--"] + unstage, cwd=root, check=False)
print(f"unstaged={len(unstage)} kept={len(staged) - len(unstage)}")
PY
git diff --cached --stat 2>/dev/null | head -15 || true
