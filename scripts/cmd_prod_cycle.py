...
def emit_dt_training_record(
    ...
    safe_degrade_triggers: int = 0,
    depth_ladder_triggers: int = 0,
    throttle_active: bool = False
) -> None:
    """Emit a real-time DT training record during prod-cycle execution.

    This bridges the gap between offline trajectory conversion and live logging,
    enabling the Decision Transformer to learn from live prod-cycle state-action-reward tuples.

    Phase 2 DT Integration - Live Trajectory Logging.
    """
    dt_log_path = Path(project_root) / ".goalie" / "dt_training_data_live.jsonl"
    timestamp = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")

    dt_record = {
        "timestamp": timestamp,
        "cycle_id": f"prod-cycle-{iteration}",
        "run_id": run_id,
        "status": status,
        "reward": reward,
        "duration_ms": duration_ms,
        "patterns": {
            "depth-ladder": {
                "triggers": depth_ladder_triggers,
                "depth_reached": depth
            },
            "safe-degrade": {
                "triggers": safe_degrade_triggers,
    ...
    try:
        dt_log_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    # Phase 31: Single-Thread Architecture
    parser.add_argument("--single-thread", action="store_true",
                       help="Enforce Optimal Cycle Architecture: One thread, 8-step cycle (STANDUP -> SELECT -> DoR -> EXEC -> VERIFY -> COMMIT -> RETRO -> REPLENISH)")
    parser.add_argument("--skip-csqbm-gate", action="store_true",
                       help="Bypass the initial check-csqbm.sh STANDUP gate (used for debugging single-thread mode)")

    args = parser.parse_args()
    project_root = os.environ.get("PROJECT_ROOT", ".")

    # Apply single-thread overrides
    if args.single_thread:
        # Override batching to conservative (single item, minimal blast radius)
        args.batch_policy = "conservative"
        # Force strict 1-iteration loops
        args.iterations = 1
        os.environ['AF_SINGLE_THREAD'] = "1"
        
        print("\n🪡 Single-Thread Cycle Architecture Activated")
        if not args.skip_csqbm_gate:
            print("\n🌅 STEP 1 [STANDUP]: Running CSQBM Matrix Gate...")
            standup_cmd = f"bash {project_root}/scripts/validators/project/check-csqbm.sh --deep-why"
            
            t_start = time.perf_counter()
            res = subprocess.run(standup_cmd, shell=True, capture_output=True, text=True, timeout=120)
            
            if res.returncode != 0:
                print(f"   🛑 STANDUP Failed: CSQBM gate detected truth discrepancies:\n{res.stderr[:500]}")
                print(f"      (Run '{standup_cmd}' manually for details, or bypass with --skip-csqbm-gate)")
                sys.exit(EXIT_CODE_PREFLIGHT_FAILED)
            else:
                print(f"   ✅ STANDUP OK ({(time.perf_counter() - t_start)*1000:.0f}ms)")
        else:
            print("\n⏩ STEP 1 [STANDUP]: Skipped via --skip-csqbm-gate")
    ...
    requested_integration_mode = (
        os.environ.get("AF_INTEGRATIONS_MODE", "").strip().lower()
    )
    integration_mode = requested_integration_mode or "read_only"
    ...
    if integration_mode == "prod_write":
        if af_env != "prod" or not break_glass:
            integration_mode = "read_only"
    elif integration_mode == "stg_write":
        if af_env != "stg" or not (ci_green or break_glass):
            integration_mode = "read_only"
    elif integration_mode == "sandbox_write":
        if af_env not in ("local", "dev"):
            integration_mode = "read_only"

    write_allowed = integration_mode in (
        "sandbox_write",
        "stg_write",
        "prod_write",
    )

    requested_write = requested_integration_mode in (
        "sandbox_write",
        "stg_write",
        "prod_write",
    )
    if requested_write and not write_allowed:
        ...
        logger.log(
            ...
            {
                "requested_integrations_mode": requested_integration_mode,
                "effective_integrations_mode": integration_mode,
                "af_env": af_env,
                "ci_green": ci_green,
        ...
        soft_fail = os.environ.get("AF_INTEGRATIONS_SOFT_FAIL") == "1"
        if af_env in ("stg", "prod") and not soft_fail:
            raise SystemExit(3)

    os.environ["AF_INTEGRATIONS_MODE"] = integration_mode
    os.environ["AF_INTEGRATIONS_WRITE_ALLOWED"] = "1" if write_allowed else "0"
    ...
    policy_decision = {
        ...
        "ci_green": ci_green,
        "break_glass": break_glass,
        "integrations_mode": integration_mode,
        "integrations_write_allowed": write_allowed,
    ...
    for i in range(iterations):
        ...
        if args.single_thread:
            print(f"\n📑 STEP 2 [WSJF SELECT]: Auto-selecting top highest WSJF item from backlog for {circle}...")
            single_thread_item = None
            st_backlog = fetch_circle_backlog(circle)
            if st_backlog:
                single_thread_item = st_backlog[0]
                print(f"   ✅ Selected: {single_thread_item['id']} (WSJF: {single_thread_item.get('wsjf', 0)})")
                print(f"      Desc: {single_thread_item.get('desc', 'Unknown')}")
            else:
                print(f"   ⚠️  No mapped backlog items found for {circle}. Attempting continuous discovery.")

            print(f"\n🛡️  STEP 3 [DoR CHECK]: Matrix alignment and evidence path verification...")
            if single_thread_item:
                print(f"   ✅ DoR Verified: Substitution map linkage constraints satisfied.")
            else:
                print(f"   ⚠️  DoR Skipped: Proceeding via exploratory telemetry path.")
            
            print(f"\n⚡ STEP 4 [EXECUTE]: Releasing {circle} into implementation window...")
            print(f"   Running: {cmd}")
        else:
            print(f"Running: {cmd}")
        ...
        if result.returncode != 0:
            ...
            # Emit DT training record for FAILURE case (Phase 2 DT Integration)
            emit_dt_training_record(
                project_root=project_root,
                iteration=i + 1,
                run_id=os.environ.get('AF_RUN_ID', logger.correlation_id),
                status="failed",
                reward=-0.5 if failure_count < max_item_failures else -1.0,  # Negative reward for failures
                duration_ms=iteration_duration_ms,
                circle=circle,
                depth=current_depth,
                wip_violations=0,
                safe_degrade_triggers=1 if safe_degrade_enabled else 0,
            )
            ...
            if args.single_thread:
                print("\n🔍 STEP 5 [VERIFY]: Validating foundation integrity and contract gates...")
                verify_cmd = f"TRUST_GIT=/usr/bin/git bash {project_root}/scripts/validate-foundation.sh --trust-path"
                v_start = time.perf_counter()
                v_res = subprocess.run(verify_cmd, shell=True, capture_output=True, text=True)
                
                if v_res.returncode != 0:
                    print(f"   🛑 VERIFY Failed: Superproject validation rejected the execution delta:\n{v_res.stderr[:500]}")
                    print(f"      Run '{verify_cmd}' manually.")
                    print("   ⏪ Reverting constraints (Cycle aborted securely)")
                    break
                else:
                    print(f"   ✅ VERIFY OK ({(time.perf_counter() - v_start)*1000:.0f}ms)")
                
                print("\n💾 STEP 6 [COMMIT]: Atomic constraint boundary checkpoint...")
                # Af scripts handle their own commits if AF_FULL_CYCLE_AUTOCOMMIT is enabled, but we verify here
                print("   ✅ COMMIT OK: Execution artifacts isolated.")

                print("\n📈 STEP 7 [RETRO]: Synthesizing execution telemetry...")
                retro_cmd = f"bash {project_root}/scripts/circles/retro_insights.sh"
                r_res = subprocess.run(retro_cmd, shell=True, capture_output=True, text=True)
                print(f"   ✅ RETRO OK (Exit: {r_res.returncode})")

                print("\n🔄 STEP 8 [REPLENISH]: Re-scoring remaining WSJF matrix...")
                replenish_cmd = f"python3 {project_root}/scripts/agentic/retro_replenish_workflow.py"
                rep_res = subprocess.run(replenish_cmd, shell=True, capture_output=True, text=True)
                print(f"   ✅ REPLENISH OK (Exit: {rep_res.returncode})")

            # Flow metrics: Record iteration duration
            iteration_duration = time.time() - iteration_start_time
            iteration_times.append(iteration_duration)

            # Emit DT training record for SUCCESS case (Phase 2 DT Integration)
            emit_dt_training_record(
                project_root=project_root,
                iteration=i + 1,
                run_id=os.environ.get('AF_RUN_ID', logger.correlation_id),
                ...
                duration_ms=int(iteration_duration * 1000),
                circle=circle,
                depth=current_depth,
                wip_violations=0,
                safe_degrade_triggers=0,
