...
# CROSS-REF: VALIDATOR_INVENTORY.md | ADR-019 | CASE_REGISTRY.yaml | CONSOLIDATION-TRUTH-INDEX.md
# SYMLINK: scripts/validate.sh -> this file
# MPP: method=meta_aggregation | pattern=contrastive_ensemble | protocol=md_json_report
# ROLE: Anti-fragile contrastive meta-validator — disagreement between validators = signal, not noise
# CLT BRIDGE: email-server.js /validate-full runs JS+bash+runner; this script runs ALL including project-level
#
...
# CSQBM Governance Constraint: Trace root validation cross-tree ensemble orchestrator
...
# --- RCA/Deep-Why aggregation (discover/consolidate layer, no new files)
RCA_RESULTS_FILE="$RESULTS_DIR/rca_results.txt"
if [[ "$RCA_AGG" == "true" ]]; then
    RCA_CHECKS=(
        "gate_csQBM_deep_why|CSQBM_DEEP_WHY=true bash \"$PROJECT_ROOT/scripts/validators/project/check-csqbm.sh\" --deep-why"
        "gate_strict_validation_workflow|test -f \"$PROJECT_ROOT/.github/workflows/strict-validation.yml\" && grep -q \"CSQBM_DEEP_WHY=true\" \"$PROJECT_ROOT/.github/workflows/strict-validation.yml\""
        "causal_governance_rca_fields|grep -q \"rca.dt_consecutive_failures\" \"$PROJECT_ROOT/scripts/policy/governance.py\""
        "causal_emit_metrics_rca_fields|grep -q \"rca.dt_consecutive_failures\" \"$PROJECT_ROOT/scripts/emit_metrics.py\""
        "causal_metrics_anchor_present|test -f \"$PROJECT_ROOT/.goalie/metrics_log.jsonl\" && grep -q \"rca.dt_consecutive_failures\" \"$PROJECT_ROOT/.goalie/metrics_log.jsonl\""
        "retro_feedback_loop_analyzer|bash -n \"$PROJECT_ROOT/scripts/feedback-loop-analyzer.sh\""
        "retro_link_metrics_to_retro|bash -n \"$PROJECT_ROOT/scripts/link_metrics_to_retro.sh\""
        "retro_cmd_retro_health|python3 \"$PROJECT_ROOT/scripts/cmd_retro.py\" list >/dev/null"
    )

    for entry in "${RCA_CHECKS[@]}"; do
        name="${entry%%|*}"
        cmd="${entry#*|}"
        res="$(run_validator "$name" "$cmd")"
        ec="${res%%|*}"; rest="${res#*|}"
        sm="${rest%%|*}"; note="${rest#*|}"
        echo "RCA|${name}|${ec}|${sm}|${note}" >> "$RCA_RESULTS_FILE"
    done
fi
...
# --- Build report
{
    ...
    echo ""
    if [[ -f "$RCA_RESULTS_FILE" ]]; then
        rca_pass=$(grep -c "|PASS|" "$RCA_RESULTS_FILE" 2>/dev/null || true); rca_pass=${rca_pass:-0}
        rca_fail=$(grep -c "|FAIL|" "$RCA_RESULTS_FILE" 2>/dev/null || true); rca_fail=${rca_fail:-0}
        rca_skip=$(grep -c "|SKIP|" "$RCA_RESULTS_FILE" 2>/dev/null || true); rca_skip=${rca_skip:-0}
        rca_total=$((rca_pass + rca_fail + rca_skip))
        echo "## RCA / Deep-Why Consolidation"
        echo "- **Model:** Gate evidence -> Causal metrics -> Retro synthesis"
        echo "- **Status:** $rca_pass/$rca_total PASS, $rca_fail FAIL, $rca_skip SKIP"
        echo ""
        echo "| RCA check | Exit | Result | Notes |"
        echo "|-----------|------|--------|-------|"
        while IFS= read -r line; do
            rest="${line#*|}"; name="${rest%%|*}"; rest="${rest#*|}"
            exit_code="${rest%%|*}"; rest="${rest#*|}"; summary="${rest%%|*}"; note="${rest#*|}"
            echo "| $name | $exit_code | $summary | ${note:0:80} |"
        done < "$RCA_RESULTS_FILE" 2>/dev/null || true
        echo ""
    fi
...
BASELINE_FILE="/tmp/.validation-baseline-\$(whoami).json"
    ...
    cat <<EOF > "$JSON_FILE"
{
  ...
  "metrics": {
    ...
    "rca_metrics": {
      "enabled": $([[ "$RCA_AGG" == "true" ]] && echo "true" || echo "false"),
      "pass": $(grep -c "|PASS|" "$RCA_RESULTS_FILE" 2>/dev/null || echo "0"),
      "fail": $(grep -c "|FAIL|" "$RCA_RESULTS_FILE" 2>/dev/null || echo "0"),
      "skip": $(grep -c "|SKIP|" "$RCA_RESULTS_FILE" 2>/dev/null || echo "0")
    }
  },
  "green_validators": {
...
EOF
    ...
    loc=$(wc -l < "$0" | tr -d ' ')
    echo "compare-all-validators.sh (${coverage_pct}%/${total_runs} | +0.0%/min | ${loc}L | 100.0%) — meta-orchestrator for cross-tree validator consolidation"
fi
