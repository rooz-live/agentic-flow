...
if [[ "${MODE}" == "--full" ]]; then
  ...
  npx claude-flow@v3alpha agent spawn -t coder --name typescript-fixer || true
  npx claude-flow@v3alpha agent spawn -t tester --name coverage-agent || true
  npx claude-flow@v3alpha agent spawn -t reviewer --name quality-agent || true
  npx claude-flow@v3alpha agent spawn -t devops --name deployment-agent || true
