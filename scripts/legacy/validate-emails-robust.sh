...
# Resolve X-Vector from file or CLI args
VECTOR=$(grep -i "^X-Vector:" "$EMAIL_FILE" | awk '{print $2}' | tr -d '\r')
if [ -z "$VECTOR" ]; then
    VECTOR="shared" # fallback
fi

# Check 7: Contextual validation per X-Vector
if [ "$VECTOR" = "clt" ] || [ "$VECTOR" = "clt-career" ]; then
    # Manufacturing/Logistics generic validation rules
    if ! grep -q "^X-Scenario" "$EMAIL_FILE"; then
        log_result "error" $EXIT_MISSING_REQUIRED_FIELD "Missing X-Scenario header" "Add X-Scenario telemetry header to the EML."
        exit $EXIT_MISSING_REQUIRED_FIELD
    fi
else
    # Legacy moving timeline validation
