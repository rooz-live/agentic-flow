...
OUTPUT_FILE=".goalie/retro_summary.md"  # Per governance: output to .goalie for Retro Synthesis
BASELINE_DIR="logs"
INCIDENTS_LOG="logs/governor_incidents.jsonl"
METRICS_LOG=".goalie/metrics_log.jsonl"
RCA_FINDINGS=".goalie/rca_findings.md"
...
GIT_COMMITS_TODAY=$(git --no-pager log --since="00:00:00" --oneline 2>/dev/null | wc -l | xargs || echo "0")
GIT_COMMITS_WEEK=$(git --no-pager log --since="7 days ago" --oneline 2>/dev/null | wc -l | xargs || echo "0")

# Parse commits with WSJF tags
TAGGED_COMMITS=$(git --no-pager log -n 100 --grep="\[QW-ID\|\[RETRO-ID\|\[WSJF" --oneline 2>/dev/null || echo "")
...
if [ "$TAGGED_COMMIT_COUNT" -gt 0 ]; then
    git --no-pager log -n 100 --grep="\[QW-ID\|\[RETRO-ID\|\[WSJF" --format="%h|%s" 2>/dev/null | head -20 | while IFS='|' read -r hash subject; do
        if [ -n "$hash" ]; then
        ...
        if [ -n "$TIMESTAMP" ]; then
            ...
            if [ "$TIMESTAMP_UNIX" != "0" ]; then
                ...
                if [ -n "$BEFORE" ] && [ -n "$AFTER" ]; then
                    COMMITS=$(git --no-pager log --since="$AFTER" --until="$BEFORE" --oneline --no-merges 2>/dev/null || echo "")
...
\`\`\`text
$(if [ -f "scripts/cmd_retro.py" ]; then
    python3 scripts/cmd_retro.py list 2>/dev/null || echo "No approvals currently mapped."
else
    echo "No approval management script found."
fi)
\`\`\`
...
# Extract RCA Findings from Causal Metrics explicitly
echo -e "${GREEN}🔍 Synthesizing Deep-Why RCA Findings from Causal Metrics...${NC}"
cat > "$RCA_FINDINGS" <<RCAEOF
# Deep-Why RCA Findings

**Generated:** $(date -u +%Y-%m-%dT%H:%M:%SZ)  
**Single Source of Truth** - Active RCA Failures

---

## Causal Traces (\`metrics_log.jsonl\`)
RCAEOF

if [ -f "$METRICS_LOG" ]; then
    grep '"type": "retro_coach_run"' "$METRICS_LOG" | tail -10 | while IFS= read -r log_line; do
        if [ -n "$log_line" ]; then
            REASON=$(echo "$log_line" | grep -o '"retro_event_prototype": "[^"]*"' | cut -d'"' -f4 || echo "N/A")
            WHY=$(echo "$log_line" | grep -o '"retro_rca_why": "[^"]*"' | cut -d'"' -f4 || echo "N/A")
            CYCLE=$(echo "$log_line" | grep -o '"cycle_index": [0-9]*' | cut -d' ' -f2 || echo "N/A")
            echo "- **Cycle $CYCLE** [$REASON]: $WHY" >> "$RCA_FINDINGS"
        fi
    done
    echo "" >> "$RCA_FINDINGS"
    echo "Synthesized $(grep -c '"type": "retro_coach_run"' "$METRICS_LOG" || echo "0") historical RCA failures matching constraints organically." >> "$RCA_FINDINGS"
else
    echo "*No active metrics log found. Awaiting execution bounds.*" >> "$RCA_FINDINGS"
fi
echo "  Output: $RCA_FINDINGS"
echo ""
