#!/usr/bin/env python3
"""
WSJF Failure Tracker Tool
Tracks failure rates and trends for wsjf-enrichment patterns
"""
...
def track_wsjf_failure_rates(events: List[Dict[str, Any]], time_window_hours: int = 24) -> Dict[str, Any]:
    """Track WSJF-enrichment failure rates over time windows"""
    wsjf_events = [e for e in events if is_wsjf_enrichment_event(e)]
    
    if not wsjf_events:
        return {"error": "No wsjf-enrichment events found"}
    
    # Sort events by time
    wsjf_events.sort(key=lambda e: parse_event_time(e) or datetime.min)
    ...
    for hours in windows:
        ...
        window_events = [e for e in wsjf_events if parse_event_time(e) and parse_event_time(e) >= cutoff]
    ...
    return {
        ...
        "trend": {
            ...
            "assessment": "significant" if trend_magnitude > 5 else "minor"
        }
    }

def analyze_wsjf_failure_patterns(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Analyze patterns in WSJF-enrichment failures"""
    wsjf_events = [e for e in events if is_wsjf_enrichment_event(e)]
    ...
    # Analyze by circle and enrichment type
    failures_by_circle = defaultdict(int)
    failures_by_enrichment_type = defaultdict(int)
    ...
    for event in failed_events:
        ...
        if any("user-value" in tag for tag in tags):
            failures_by_enrichment_type["user-value"] += 1
        elif any("business-value" in tag for tag in tags):
            failures_by_enrichment_type["business-value"] += 1
        elif any("time-criticality" in tag for tag in tags):
            failures_by_enrichment_type["time-criticality"] += 1
        elif any("risk-reduction" in tag for tag in tags):
            failures_by_enrichment_type["risk-reduction"] += 1
        else:
            failures_by_enrichment_type["general"] += 1
    ...
    return {
        ...
        "organizational_patterns": {
            ...
            "failures_by_enrichment_type": dict(failures_by_enrichment_type)
        }
    }

def calculate_wsjf_failure_trends(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Calculate WSJF-enrichment failure trends over time"""
    wsjf_events = [e for e in events if is_wsjf_enrichment_event(e)]
    
    if len(wsjf_events) < 10:
...
def generate_wsjf_failure_alerts(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Generate alerts based on WSJF-enrichment failure patterns"""
    wsjf_events = [e for e in events if is_wsjf_enrichment_event(e)]
    failed_events = [e for e in wsjf_events if e.get("status") == "failed"]
    ...
    if len(recent_failures) >= 2:
        alerts.append({
            "severity": "high",
            "type": "failure_spike",
            "message": f"{len(recent_failures)} WSJF-enrichment failures in last hour",
    ...
    if high_impact_failures:
        alerts.append({
            ...
            "message": f"{len(high_impact_failures)} high-impact WSJF-enrichment failures detected",
    ...
    # Enrichment type-specific alert
    enrichment_failures = defaultdict(int)
    for event in failed_events:
        tags = [tag.lower() for tag in event.get("tags", [])]
        
        if any("user-value" in tag for tag in tags):
            enrichment_failures["user-value"] += 1
        elif any("business-value" in tag for tag in tags):
            enrichment_failures["business-value"] += 1
        elif any("time-criticality" in tag for tag in tags):
            enrichment_failures["time-criticality"] += 1
        elif any("risk-reduction" in tag for tag in tags):
            enrichment_failures["risk-reduction"] += 1
        else:
            enrichment_failures["general"] += 1
    
    # Check for enrichment types with high failure rates
    total_by_enrichment = defaultdict(int)
    for event in wsjf_events:
        tags = [tag.lower() for tag in event.get("tags", [])]
        
        if any("user-value" in tag for tag in tags):
            total_by_enrichment["user-value"] += 1
        elif any("business-value" in tag for tag in tags):
            total_by_enrichment["business-value"] += 1
        elif any("time-criticality" in tag for tag in tags):
            total_by_enrichment["time-criticality"] += 1
        elif any("risk-reduction" in tag for tag in tags):
            total_by_enrichment["risk-reduction"] += 1
        else:
            total_by_enrichment["general"] += 1
    
    for enrichment_type, failure_count in enrichment_failures.items():
        total = total_by_enrichment.get(enrichment_type, 0)
        if total >= 3:  # Only analyze enrichment types with sufficient data
            failure_rate = (failure_count / total) * 100
            if failure_rate > 50:  # More than 50% failure rate
                alerts.append({
                    "severity": "medium",
                    "type": "enrichment_failure_rate",
                    "enrichment_type": enrichment_type,
                    "failure_rate": failure_rate,
                    "message": f"WSJF-enrichment type '{enrichment_type}' has {failure_rate:.1f}% failure rate",
                    "recommendation": f"Review and improve '{enrichment_type}' enrichment process"
                })
...
def main():
    ...
    parser = argparse.ArgumentParser(description="WSJF Failure Tracker Tool")
    parser.add_argument("--input-file", help="Input pattern metrics file")
    parser.add_argument("--pattern", default="wsjf-enrichment", help="Pattern to track")
    ...
    # Track failure rates
    failure_rates = track_wsjf_failure_rates(events, args.hours)
    result["failure_rates"] = failure_rates
    
    # Analyze failure patterns
    patterns = analyze_wsjf_failure_patterns(events)
    result["failure_patterns"] = patterns
    
    # Calculate trends
    trends = calculate_wsjf_failure_trends(events)
    result["failure_trends"] = trends
    
    # Generate alerts if requested
    if args.alerts:
        alerts = generate_wsjf_failure_alerts(events)
    ...
    if args.json:
        ...
        print("=" * 70)
        print("WSJF FAILURE TRACKING REPORT")
        print("=" * 70)
