#!/usr/bin/env python3
"""
WSJF Pattern Filter Tool
Advanced filtering and analysis capabilities for wsjf-enrichment patterns
"""
...
def filter_wsjf_by_pattern(events: List[Dict[str, Any]], patterns: List[str]) -> List[Dict[str, Any]]:
    """Filter WSJF-enrichment events by pattern names"""
    if not patterns:
        return events
    
    pattern_set = set(p.lower() for p in patterns)
    return [e for e in events if is_wsjf_enrichment_event(e) and e.get("pattern", "").lower() in pattern_set]

def filter_wsjf_by_circle(events: List[Dict[str, Any]], circles: List[str]) -> List[Dict[str, Any]]:
    """Filter WSJF-enrichment events by circle names"""
    if not circles:
        return events
    
    circle_set = set(c.lower() for c in circles)
    return [e for e in events if is_wsjf_enrichment_event(e) and e.get("circle", "").lower() in circle_set]

def filter_wsjf_by_time_range(events: List[Dict[str, Any]], hours: Optional[int] = None, 
                          start_date: Optional[str] = None, end_date: Optional[str] = None) -> List[Dict[str, Any]]:
    """Filter WSJF-enrichment events by time range"""
...
def filter_wsjf_by_economic_metrics(events: List[Dict[str, Any]], 
    ...
    """Filter WSJF-enrichment events by economic metrics"""
    filtered_events = []
...
def filter_wsjf_by_status(events: List[Dict[str, Any]], status_values: List[str]) -> List[Dict[str, Any]]:
    """Filter WSJF-enrichment events by status values"""
    if not status_values:
        return events
    
    status_set = set(s.lower() for s in status_values)
    return [e for e in events if is_wsjf_enrichment_event(e) and e.get("status", "").lower() in status_set]

def filter_wsjf_by_depth(events: List[Dict[str, Any]], min_depth: Optional[int] = None, 
                      max_depth: Optional[int] = None) -> List[Dict[str, Any]]:
    """Filter WSJF-enrichment events by depth range"""
...
def filter_wsjf_by_tags(events: List[Dict[str, Any]], include_tags: List[str], 
                      exclude_tags: List[str] = None) -> List[Dict[str, Any]]:
    """Filter WSJF-enrichment events by tags (include/exclude)"""
...
def filter_wsjf_by_enrichment_type(events: List[Dict[str, Any]], 
                                   enrichment_types: List[str]) -> List[Dict[str, Any]]:
    """Filter WSJF-enrichment events by enrichment type"""
    if not enrichment_types:
        return events
    
    filtered_events = []
    enrichment_type_set = set(t.lower() for t in enrichment_types)
    ...
    for event in events:
        ...
        # Check for enrichment type matches
        if any(enrichment_type in tags for enrichment_type in enrichment_type_set):
            filtered_events.append(event)
    
    return filtered_events

def analyze_filtered_wsjf_events(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Analyze filtered WSJF-enrichment events and provide insights"""
    if not events:
    ...
    for event in events:
        ...
        if any("user-value" in tag for tag in tags):
            enrichment_type_counts["user-value"] = enrichment_type_counts.get("user-value", 0) + 1
        elif any("business-value" in tag for tag in tags):
            enrichment_type_counts["business-value"] = enrichment_type_counts.get("business-value", 0) + 1
        elif any("time-criticality" in tag for tag in tags):
            enrichment_type_counts["time-criticality"] = enrichment_type_counts.get("time-criticality", 0) + 1
        elif any("risk-reduction" in tag for tag in tags):
            enrichment_type_counts["risk-reduction"] = enrichment_type_counts.get("risk-reduction", 0) + 1
        else:
            enrichment_type_counts["general"] = enrichment_type_counts.get("general", 0) + 1
    ...
    return {
        ...
        "enrichment_type_distribution": dict(sorted(enrichment_type_counts.items(), key=lambda x: x[1], reverse=True)[:10]),
        "top_patterns": sorted(pattern_counts.items(), key=lambda x: x[1], reverse=True)[:5],
        "top_circles": sorted(circle_counts.items(), key=lambda x: x[1], reverse=True)[:5],
        "top_enrichment_types": sorted(enrichment_type_counts.items(), key=lambda x: x[1], reverse=True)[:5]
    }

def export_filtered_wsjf_events(events: List[Dict[str, Any]], output_file: str, format_type: str = "json") -> bool:
    """Export filtered WSJF-enrichment events to file"""
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="WSJF Pattern Filter Tool")
    ...
    parser.add_argument("--enrichment-types", help="Comma-separated list of enrichment types")
    
    # Analysis options
    parser.add_argument("--analyze", action="store_true", help="Analyze filtered events")
    parser.add_argument("--limit", type=int, help="Limit number of results")
    parser.add_argument("--sort-by", choices=["time", "wsjf", "cod", "pattern", "circle", "enrichment-type"], default="time", help="Sort results by field")
    ...
    # Apply filters
    if args.pattern:
        events = filter_wsjf_by_pattern(events, [args.pattern])
    elif args.patterns:
        patterns = [p.strip() for p in args.patterns.split(',') if p.strip()]
        events = filter_wsjf_by_pattern(events, patterns)
    
    if args.circle:
        events = filter_wsjf_by_circle(events, [args.circle])
    elif args.circles:
        circles = [c.strip() for c in args.circles.split(',') if c.strip()]
        events = filter_wsjf_by_circle(events, circles)
    
    events = filter_wsjf_by_time_range(events, args.hours, args.start_date, args.end_date)
    events = filter_wsjf_by_economic_metrics(events, args.min_wsjf, args.max_wsjf, args.min_cod, args.max_cod)
    
    if args.status:
        status_values = [s.strip() for s in args.status.split(',') if s.strip()]
        events = filter_wsjf_by_status(events, status_values)
    
    events = filter_wsjf_by_depth(events, args.min_depth, args.max_depth)
    ...
    if args.include_tags or args.exclude_tags:
        ...
        events = filter_wsjf_by_tags(events, include_tags, exclude_tags)
    
    if args.enrichment_types:
        enrichment_types = [t.strip() for t in args.enrichment_types.split(',') if t.strip()]
        events = filter_wsjf_by_enrichment_type(events, enrichment_types)
    ...
    elif args.sort_by == "enrichment-type":
        events.sort(key=lambda e: (e.get("tags", [""])[0] if e.get("tags") else ""))
    ...
    if args.analyze:
        analysis = analyze_filtered_wsjf_events(events)
        result["analysis"] = analysis
    
    # Export if requested
    if args.output_file:
        success = export_filtered_wsjf_events(events, args.output_file, args.export_format)
        result["export"] = {
    ...
    if args.json:
        ...
        print("=" * 70)
        print("WSJF PATTERN FILTER REPORT")
        print("=" * 70)
        print(f"Events Filtered: {len(events)}")
        print(f"Filter Applied: {args.pattern or 'all wsjf-enrichment patterns'}")
        ...
        if args.analyze and "analysis" in result:
            ...
            top_enrichment_types = analysis.get("top_enrichment_types", [])
            if top_enrichment_types:
                print(f"\n🔧 Top Enrichment Types:")
                for enrich_type, count in top_enrichment_types:
                    print(f"   {enrich_type}: {count}")
