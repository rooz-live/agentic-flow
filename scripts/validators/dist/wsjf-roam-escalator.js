...
exports.EXTRA_WATCH_DIRS = void 0;
exports.calculateWsjf = calculateWsjf;
exports.categorizeRoam = categorizeRoam;
...
const SENT_DIRS = [
    (0, path_1.join)(process.env.HOME, 'Documents/Personal/CLT/MAA/Uptown/BHOPTI-LEGAL/01-ACTIVE-CRITICAL/MAA-26CV005596-590/CORRESPONDENCE/OUTBOUND/01-OPPOSING-COUNSEL/SENT'),
    (0, path_1.join)(process.env.HOME, 'Documents/Personal/CLT/MAA/Uptown/BHOPTI-LEGAL/01-ACTIVE-CRITICAL/MAA-26CV005596-590/CORRESPONDENCE/OUTBOUND/04-SETTLEMENT-OFFERS/SENT'),
    (0, path_1.join)(process.env.HOME, 'Documents/Personal/CLT/MAA/Uptown/BHOPTI-LEGAL/11-ADVOCACY-PIPELINE/TIER-5-DIGITAL/Email/SENT')
];
const SCAN_INTERVAL_MS = 3000; // Scan every 3 seconds (increased from 10s)
const VALIDATE_SH_PATH = (0, path_1.join)(process.env.HOME, 'Documents/Personal/CLT/MAA/Uptown/BHOPTI-LEGAL/_SYSTEM/_AUTOMATION/validate-email.sh');
...
function calculateWsjf(content, keywords) {
...
function categorizeRoam(content) {
    const lowerContent = content.toLowerCase();
...
async function extractTextWithPaperclip(filePath) {
    try {
        return await new Promise((resolve, reject) => {
            const paperclipProcess = (0, child_process_1.spawn)('paperclip', ['extract', '--file', filePath, '--ocr-enabled']);
            let output = '';
        ...
        return new Promise((resolve) => {
            const pdftotextProcess = (0, child_process_1.spawn)('pdftotext', [filePath, '-']);
            let output = '';
...
async function routeToSwarm(task, context, wsjf, roam) {
...
async function storeInMemory(key, value, namespace) {
    return new Promise((resolve, reject) => {
        const process = (0, child_process_1.spawn)('npx', [
            'ruflo', 'memory', 'store',
            '--key', key,
            '--value', value,
            '--namespace', namespace
...
async function processFile(filePath) {
    ...
    try {
        ...
        if (filePath.endsWith('.pdf')) {
            ...
            const jsonContent = (0, fs_1.readFileSync)(filePath, 'utf-8');
            try {
...
async function generateWsjfHtml(allFiles) {
...
async function validateEmailPreSend(filePath) {
    if (!filePath.endsWith('.eml'))
        return true;
    console.log(`🔍 Pre-send validation: ${filePath}`);
    return new Promise((resolve) => {
        const process = (0, child_process_1.spawn)('bash', [VALIDATE_SH_PATH, filePath]);
...
function initWatcher() {
    ...
    console.log(`📁 Extra watch dirs (covered by main depth=10): ${exports.EXTRA_WATCH_DIRS.length}`);
    exports.EXTRA_WATCH_DIRS.forEach(d => console.log(`   ↳ ${d}`));
    ...
    const mainWatcher = (0, chokidar_1.watch)(WATCH_DIR, {
        persistent: true,
        ignoreInitial: false,
        depth: 10,
        awaitWriteFinish: {
    ...
    const processAndUpdate = async (path) => {
