...
#   1 = Validation PASSED with warnings (review recommended)
#   11 = File not found
#   12 = Invalid format (not .eml)
#   21 = Missing required headers
#   50 = Network unavailable (Mail.app API)
#   60 = Tool missing (Python, vibesthinker)
#   111 = Placeholder detected
#   150 = Legal citation malformed
#   151 = Recipient blacklisted
#   210 = Permission denied
#   220 = Mail.app daemon not available
# ═══════════════════════════════════════════════════════════════════════════════
...
# Source robust exit codes from validation-core.sh
...
detect_doc_type() {
    ...
    # FIRST: Check if procedural/administrative email to court officials
    # These are NOT legal filings, just administrative coordination (no Pro Se required)
    if grep -qiE '^To:.*@nccourts\.org' "$file" 2>/dev/null; then
        echo "procedural"
        return 0
    fi
