...
# 3. Vector Provisioning TDD Check (ADR-006)
AGENTDB_PATH="$PROJECT_ROOT/agentdb.db"
if [[ -f "$AGENTDB_PATH" ]]; then
    if [[ -z $(find "$AGENTDB_PATH" -mmin -120 2>/dev/null) ]]; then
        if [[ "${ALLOW_CSQBM_BYPASS:-false}" == "true" ]]; then
            echo "WARN: Vector Graph (agentdb.db) is stale (> 120m), but ALLOW_CSQBM_BYPASS=true. Proceeding."
        else
            echo "FAIL: Vector Provisioning TDD Check (ADR-006)."
            echo "The semantic graph (agentdb.db) lacks mathematical hydration traces (stale > 120m). Graph Paralysis (R-2026-021) risk flagged."
    ...
    echo "WARN: agentdb.db not localized. Vector Graph is structurally non-existent. ADR-006 checks bypassed."
fi
