...
const DEFAULT_CONFIG: Required<AffiliateStateTrackerConfig> = {
...
export class AffiliateStateTracker extends EventEmitter {
  ...
  private config: Required<AffiliateStateTrackerConfig>;
  private eventHandlers: Map<string, AffiliateEventHandler[]> = new Map();
  ...
  transitionStatus(affiliateId: string, newStatus: AffiliateStatus): { success: boolean; newStatus?: AffiliateStatus; error?: string } {
  ...
  updateAffiliate(affiliateId: string, input: UpdateAffiliateInput): AffiliateState | null {
    ...
    if (input.tier !== undefined) { updates.push('tier = ?'); values.push(input.tier); }
  ...
  private mapRowToAffiliateState(row: any): AffiliateState {
    return {
      id: row.id,
  ...
  logActivity(input: CreateActivityInput): AffiliateActivity {
    ...
    const activity = this.getActivityById(result.lastInsertRowid as number)!;
  ...
  getActivitiesByAffiliateId(affiliateId: string, limit = 100): AffiliateActivity[] {
    const rows = this.db.prepare(`
      SELECT id, affiliate_id, activity_type, source, payload, timestamp, created_at
  ...
  private mapRowToActivity(row: any): AffiliateActivity {
    return {
      id: row.id,
      affiliateId: row.affiliate_id,
  ...
  createRisk(input: CreateRiskInput): AffiliateRisk {
    ...
    const risk = this.getRiskById(result.lastInsertRowid as number)!;
  ...
  getRisksByRoamStatus(roamStatus: string): AffiliateRisk[] {
    ...
    `).all(roamStatus) as any[];

    return rows.map(row => this.mapRowToRisk(row));
  }

  private mapRowToRisk(row: any): AffiliateRisk {
    return {
      id: row.id,
      affiliateId: row.affiliate_id,
  ...
  createAffinity(input: CreateAffinityInput): AffiliateAffinity {
    ...
    const result = stmt.run(
      input.affiliateId1,
      input.affiliateId2,
      input.affinityScore ?? 0.0,
      input.confidence ?? 0.5,
      input.relationshipType || 'peer',
      input.metadata ? JSON.stringify(input.metadata) : null
    );

    const affinity = this.getAffinityById(result.lastInsertRowid as number)!;
  ...
  getAffinitiesForAffiliate(affiliateId: string): AffiliateAffinity[] {
    ...
    `).all(affiliateId, affiliateId) as any[];
  ...
  private mapRowToAffinity(row: any): AffiliateAffinity {
    return {
      id: row.id,
  ...
  private emitEvent(type: AffiliateEvent['type'], affiliateId: string, data: Record<string, unknown>): void {
  ...
  private logLearningEvent(eventType: string, affiliateId: string, context: Record<string, unknown>): void {
    if (!this.agentDb || !this.config.enableLearning) return;
    ...
    try {
      ...
      stmt.run(
        'affiliate_tracker',
        eventType,
        JSON.stringify({ affiliateId, ...context }),
        'pending',
        0.5,
        JSON.stringify(['affiliate', 'state_tracking'])
      );
    } catch (error) {
