...
const CIRCLE_PRIORS: Record<string, number> = {
  innovator: 5000,
  analyst: 3500,
  orchestrator: 2500,
  assessor: 2000,
...
export class WSJFAnalytics {
  ...
  calculateWSJF(
    title: string,
    description: string,
    manualOverrides?: Partial<WSJFScore>
  ): WSJFScore {
   ...
   */
  rankByWSJF<T extends { id: string; title: string; description: string }>(
    items: T[]
  ): Array<T & { wsjf: WSJFScore; rank: number }> {
