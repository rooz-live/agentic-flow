...
export interface Skill {
  ...
  impact: {
    health?: number; // Expected health improvement
    roam?: {
      reach?: number;
      optimize?: number;
      automate?: number;
...
export class SkillScanner {
  ...
  private async scanScripts(): Promise<void> {
    ...
    for (const file of files) {
      ...
      let type: Skill['type'] = 'script';
      let impact: Skill['impact'] = {};
   ...
   */
  private async scanAgentDBSkills(): Promise<void> {
    ...
    try {
      ...
      const { stdout } = await execAsync(
        `sqlite3 "${this.agentDBPath}" "SELECT id, name, description, confidence, metadata FROM skills LIMIT 100"`
      );
   ...
   */
  private async scanDeploymentTargets(): Promise<void> {
   ...
   * Scan test suites
   */
  private async scanTestSuites(): Promise<void> {
   ...
   */
  selectOptimalSkills(context: SkillContext, maxSkills: number = 5): Skill[] {
   ...
   */
  private calculateSkillScore(skill: Skill, context: SkillContext): number {
    let score = 0;
    ...
    if (skill.impact.roam) {
      const roamImprovement =
        (skill.impact.roam.reach || 0) +
    ...
    if (skill.metadata?.successCount) {
      const successRate =
        skill.metadata.successCount / (skill.metadata.successCount + (skill.metadata.failureCount || 0));
 ...
 */
export async function integrateSkillsIntoAy(projectRoot: string, context: SkillContext): Promise<void> {
