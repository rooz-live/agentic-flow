...
export class SkillManager {
  ...
  constructor(baseDir: string = '.goalie') {
  ...
  public registerSkill(name: string, confidence: number = 0.5, description: string = ''): void {
    const now = Date.now();
  ...
  public updateOutcome(name: string, success: boolean, runId: string = 'unknown'): void {
  ...
  public importSkills(path: string): void {
    ...
    const importTx = this.db.transaction((skills: any[]) => {
