...
interface WsjfItem {
  ...
  jobSize: number;            // 1-10 (effort/duration)
  wsjfScore: number;          // Calculated: (BV + TC + RR) / JobSize
  position: [number, number, number];
  mcpFactor?: number;         // Model Context Protocol alignment (0-1)
  mppFactor?: number;         // Method Pattern Protocol adherence (0-1)
  selected: boolean;
...
function calculateWsjf(bv: number, tc: number, rr: number, size: number): number {
  return (bv + tc + rr) / Math.max(size, 1);
}

function generatePositions(items: WsjfItem[]): WsjfItem[] {
...
function createMeshTopology(items: WsjfItem[]): Array<{ path: number[][]; color: number[] }> {
  const paths: Array<{ path: number[][]; color: number[] }> = [];
...
export default function WsjfVisualization({
  items,
  onItemSelect,
  autoSelectTop = 5,
  showMeshTopology = true
}: WsjfVisualizationProps) {
  ...
  const processedItems = useMemo(() => {
    const withScores = items.map(item => ({
      ...item,
      wsjfScore: item.wsjfScore || calculateWsjf(
        item.businessValue,
        item.timeCriticality,
        item.riskReduction,
        item.jobSize
      )
    }));
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '600px' }}>
      ...
      <div style={{
        ...
        <div style={{ fontWeight: 'bold', marginBottom: '10px' }}>WSJF Visualization</div>
        <div>🔵 Feature</div>
        <div>🔴 Bug</div>
        <div>🟡 Tech Debt</div>
        <div>🟣 Spike</div>
        <div>🟢 Auto-Selected (Top {autoSelectTop})</div>
        <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #444' }}>
          <div>Height = WSJF Score</div>
          <div>X = Value + Criticality</div>
          <div>Y = Risk Reduction</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div>Total Items: {processedItems.length}</div>
        <div>Auto-Selected: {processedItems.filter(i => i.selected).length}</div>
        <div>Avg WSJF: {(processedItems.reduce((sum, i) => sum + i.wsjfScore, 0) / processedItems.length).toFixed(2)}</div>
        <div>Max WSJF: {Math.max(...processedItems.map(i => i.wsjfScore)).toFixed(2)}</div>
      </div>
    </div>
  );
}
...
export function generateSampleWsjfData(): WsjfItem[] {
  const types: WsjfItem['type'][] = ['feature', 'bug', 'tech-debt', 'spike'];
  const titles = [
