...
export class CPanelAPIClient {
   ...
   */
  async uploadFile(localPath: string, remotePath: string): Promise<CPanelResponse> {
    const fs = require('fs');
   ...
   */
  async deployApplication(files: string[], targetDir: string): Promise<CPanelResponse> {
...
// Factory function
export function createCPanelClient(config?: Partial<CPanelConfig>): CPanelAPIClient {
  const defaultConfig: CPanelConfig = {
...
// Usage example
export async function deployCPanel(): Promise<void> {
