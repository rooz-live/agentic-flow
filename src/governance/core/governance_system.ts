...
export class GovernanceSystem {
  ...
  private auditLogger: DecisionAuditLogger;
   ...
   */
  async checkDimensionalCompliance(): Promise<DimensionalViolation[]> {
  ...
  private async checkTruthDimension(): Promise<DimensionalViolation[]> {
    const violations: DimensionalViolation[] = [];

    // Check 1: Direct measurement coverage (health checks using direct DB queries vs proxies)
    try {
      const events = this.loadPatternEvents();
      const healthCheckEvents = events.filter(e =>
        e.pattern?.includes('health') || e.gate === 'health'
      );
      ...
      if (healthCheckEvents.length > 0) {
        ...
        const directMeasurements = healthCheckEvents.filter(e =>
          e.economic?.wsjf_score !== undefined || // Has economic data = direct measurement
          e.economic?.cod !== undefined || // Cost of delay quantified
          (e as any).semantic_context?.rationale !== undefined || // Semantic reasoning present
          (e as any).semantic_context?.decision_factors !== undefined || // Decision factors documented
          e.rationale?.includes('direct') || // Mentions direct measurement
          (e as any).measurement_type === 'direct' || // Explicit measurement type
  ...
  private async checkTimeDimension(): Promise<DimensionalViolation[]> {
    const violations: DimensionalViolation[] = [];
  ...
  private async checkLiveDimension(): Promise<DimensionalViolation[]> {
    const violations: DimensionalViolation[] = [];
    ...
    try {
      ...
      const adaptiveEvents = recentEvents.filter(e =>
        e.pattern?.includes('adaptive') ||
        ...
        (e as any).semantic_context?.trigger?.type === 'adaptive_learning' ||
        (e as any).semantic_context?.trigger?.type === 'threshold_exceeded'
      );
  ...
  private findROAMFiles(): Array<{ path: string; ageHours: number }> {
    const { execSync } = require('child_process');
    try {
      // Find all ROAM files in project
      const output = execSync(
        'find . -type f \\( -name "ROAM-*.md" -o -name "*-roam.md" \\) -not -path "*/node_modules/*" -not -path "*/.git/*"',
        { cwd: this.goalieDir + '/..', encoding: 'utf-8' }
      );
    ...
    if (this.config.autoLogDecisions) {
      ...
      this.auditLogger.logDecision({
        ...
        context: {
          ...
          recentEventCount: this.loadPatternEvents().filter(e =>
            new Date(e.ts) > new Date(Date.now() - 60 * 60 * 1000)
          ).length,
          checks: checks.map(c => ({
            area: c.area,
            status: c.status,
