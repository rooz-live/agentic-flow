...
export class CausalLearningIntegration {
   ...
   */
  async recordObservation(episode: CompletionEpisode): Promise<void> {
   ...
   */
  async analyzeExperiments(minSampleSize: number = 30): Promise<void> {
...
// @ts-expect-error - Type incompatibility requires refactoring
   ...
   */
  async explainCompletion(circle: string, ceremony: string, currentRate: number): Promise<string> {
...
async function main() {
  ...
  switch (command) {
    case 'record': {
      ...
      const episode: CompletionEpisode = {
        circle: data.circle,
