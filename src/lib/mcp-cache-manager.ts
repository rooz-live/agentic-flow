...
export class MCPCacheManager {
  ...
  private getCacheKey(provider: string, tool: string, args: any): string {
   ...
   */
  async loadFromCache(provider: string, tool: string, args: any): Promise<any | null> {
    try {
      const cacheKey = this.getCacheKey(provider, tool, args);
      const cachePath = this.getCachePath(cacheKey);
  ...
  async enforceSizeLimit(): Promise<number> {
    ...
    let deletedCount = 0;
    const providers = fs.readdirSync(this.cacheDir);
    const allEntries: { path: string; mtime: number }[] = [];
