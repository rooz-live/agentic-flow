...
class PiHardwareInterface {
   ...
   */
  private validatePiEnvironment(): void {
    try {
   ...
   */
  private checkCriticalThresholds(temperature: number, cpuUsage: number): void {
    if (temperature > 85) {
  ...
  private extractModel(cpuInfo: string): string {
    const match = cpuInfo.match(/Model\s*:\s*(.+)/);
    return match ? match[1].trim() : 'Unknown Raspberry Pi';
  }

  private extractSerial(cpuInfo: string): string {
  ...
  private calculateMemoryUsage(memInfo: string): number {
  ...
  private parseMemValue(line?: string): number {
    if (!line) return 0;
  ...
  private determineStatus(sensorId: string, value: number): 'ok' | 'warning' | 'critical' {
  ...
  private getSensorUnit(sensorId: string): string {
   ...
   */
  async controlGPIO(pin: number, state: 'high' | 'low' | 'input'): Promise<void> {
   ...
   */
  validateSystemHealth(): { status: 'healthy' | 'warning' | 'critical'; issues: string[] } {
  ...
  try {
    switch (name) {
      ...
      case 'pi_read_sensor':
        ...
        const sensorId = String(args.sensor_id) as 'temperature' | 'cpu' | 'memory' | 'disk';
      ...
      case 'pi_control_gpio':
        ...
        const state = String(args.state) as 'high' | 'low' | 'input';
