...
import { SecurityMonitoring, SecurityEvent } from './security-monitoring';
...
export class AutomationSelfHealing {
  ...
  private securityMonitor: SecurityMonitoring;
  private activeHealing: Map<string, any> = new Map();
  ...
  constructor(
    ...
    securityMonitor: SecurityMonitoring
  ) {
    this.config = {
  ...
  private setupEventListeners(): void {
    ...
    this.eventBus.on('security:event', (event: SecurityEvent) => {
      this.handleSecurityEvent(event);
    });

    // Listen for metric alerts
    this.eventBus.on('metric:alert', (alert: any) => {
    ...
    this.eventBus.on('system:event', (event: any) => {
  ...
  private async performHealthCheck(healthCheck: HealthCheck): Promise<void> {
    ...
    try {
      ...
      // Log health check
      this.logger.logHealthCheck(healthCheck.name, isHealthy ? 'pass' : 'fail', duration, 
        isHealthy ? null : `Status code: ${response.status}`
      );
      ...
      this.logger.logHealthCheck(healthCheck.name, 'fail', duration, 
  ...
  private triggerHealthCheckAutomation(healthCheck: HealthCheck): void {
    ...
    const relevantRules = this.config.automationRules.filter(rule => 
      rule.enabled && rule.trigger.metricName === 'health_check_status'
    );
  ...
  private evaluateAutomationRules(): void {
  ...
  private getHealthCheckMetric(labels?: Record<string, string>): number {
  ...
  private async scaleService(serviceName: string, direction: 'up' | 'down', instances: number): Promise<void> {
  ...
  private async sendAlert(message: string, severity: string, channels: string[]): Promise<void> {
  ...
  private async enableMaintenanceMode(duration: number): Promise<void> {
  ...
  private handleSecurityEvent(event: SecurityEvent): void {
    ...
    if (event.severity === 'critical') {
      ...
      const securityRules = this.config.automationRules.filter(rule => 
        rule.enabled && rule.trigger.metricName === 'security_events_total'
      );
  ...
  private handleMetricAlert(alert: any): void {
    ...
    // Find relevant automation rules
    const metricRules = this.config.automationRules.filter(rule => 
      rule.enabled && rule.trigger.metricName === alert.metricName
    );
  ...
  private cleanupAutomationData(): void {
  ...
  // Public API methods
  getActiveHealing(): Array<{ id: string; startTime: number; action: string }> {
    const healing = [];
