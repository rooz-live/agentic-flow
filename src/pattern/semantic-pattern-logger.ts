...
export interface SemanticPatternRationale {
  ...
  outcome_actual?: string;              // Actual outcome (filled later)
}
...
export interface SemanticPatternMetric {
  ...
  embeddings?: number[];                // Vector representation (384-dim)
...
export class SemanticPatternLogger {
   ...
   */
  async logPattern(metric: SemanticPatternMetric): Promise<string> {
  ...
  private prepareTextForEmbedding(metric: SemanticPatternMetric): string {
  ...
  private logToDatabase(metric: SemanticPatternMetric): string {
    ...
    try {
      ...
      stmt.run(
        ...
        metric.rationale.context,
        metric.rationale.decision_logic,
        JSON.stringify(metric.rationale.alternatives_considered),
        metric.rationale.outcome_expected,
        metric.rationale.outcome_actual || null,
  ...
  private logToJsonl(metric: SemanticPatternMetric): string {
    const id = `pattern-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
  ...
  private async findSimilarInDatabase(
  ...
  private findSimilarInJsonl(
    ...
    try {
      ...
      for (const line of lines) {
        const metric = JSON.parse(line) as SemanticPatternMetric;
        if (metric.embeddings) {
  ...
  private rowToMetric(row: any, embeddings: number[]): SemanticPatternMetric {
    return {
   ...
   */
  async getRecentPatterns(limit: number = 100): Promise<SemanticPatternMetric[]> {
    if (this.db && !this.useFallback) {
  ...
  private getFromDatabase(limit: number): SemanticPatternMetric[] {
    try {
      const stmt = this.db!.prepare(`
        SELECT * FROM semantic_patterns
  ...
  private getFromJsonl(limit: number): SemanticPatternMetric[] {
    const logPath = join(this.goalieDir, 'semantic_patterns.jsonl');
    if (!existsSync(logPath)) {
      return [];
    }
    ...
    try {
      ...
      const patterns = lines
        .map(line => JSON.parse(line) as SemanticPatternMetric)
        .slice(-limit)
        .reverse();
