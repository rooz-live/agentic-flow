...
export class LearnedCircuitBreaker {
  ...
  private loadConfigsFromDatabase(): void {
    try {
  ...
  private loadConfigsFromJson(): void {
    ...
    try {
      ...
      const configs = JSON.parse(content) as CircuitBreakerConfig[];
   ...
   */
  async getFailureHistory(service: string, days: number = 30): Promise<FailureHistory[]> {
    if (this.db && !this.useFallback) {
  ...
  private getFailureHistoryFromDatabase(service: string, days: number): FailureHistory[] {
    try {
      ...
      const rows = stmt.all(service, cutoff) as any[];
   ...
   */
  async updateAllThresholds(days: number = 30): Promise<ThresholdUpdate[]> {
   ...
   */
  async updateServiceThreshold(service: string, days: number = 30): Promise<ThresholdUpdate | null> {
  ...
  private saveConfig(config: CircuitBreakerConfig): void {
    if (this.db && !this.useFallback) {
      try {
        const stmt = this.db.prepare(`
        ...
        stmt.run(
          ...
          config.lastLearned.toISOString(),
          config.learningEnabled ? 1 : 0,
          config.manualOverride ? 1 : 0,
          config.overrideReason || null
        );
      } catch (error) {
  ...
  private saveConfigsToJson(): void {
    const configPath = join(this.goalieDir, 'circuit_breaker_configs.json');
  ...
  private logThresholdUpdate(update: ThresholdUpdate): void {
    if (this.db && !this.useFallback) {
      try {
        const stmt = this.db.prepare(`
  ...
  private getDefaultConfig(service: string): CircuitBreakerConfig {
    return {
      service,
      failureThreshold: 10,
      timeoutMs: 30000,
   ...
   */
  getRecentUpdates(limit: number = 10): ThresholdUpdate[] {
    if (!this.db || this.useFallback) {
      return [];
    }
