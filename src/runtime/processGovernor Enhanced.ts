 ...
 */
export async function refreshDynamicThresholds(
  state: any,
...
function getDefaultThresholds(): DynamicThresholds {
  return {
    circuitBreaker: {
      threshold: 0.7,
      confidence: 'NO_DATA',
...
// ═══════════════════════════════════════════════════════════════════════════
// DEGRADATION DETECTION (95% CI Method)
// ═══════════════════════════════════════════════════════════════════════════
...
export interface DegradationCheckResult {
  ...
  currentMean: number;
  threshold: number;
  degradationScore: number; // 0-1, where 1 = severe degradation
  method: string;
  confidence: string;
}
...
export function checkDegradation(
  ...
  const successfulEpisodes = recentPerf.filter((p: any) => p.success);
  ...
  const currentMean = successfulEpisodes.reduce((sum: number, p: any) => sum + p.reward, 0) / successfulEpisodes.length;
 ...
 */
export function recordEpisodePerformance(
  state: any,
  reward: number,
  success: boolean
): void {
...
export interface CascadeFailureCheckResult {
  ...
  failureVelocity: number; // failures per minute
  method: string;
}
 ...
 */
export function checkCascadeFailure(state: any, taskId?: string): CascadeFailureCheckResult {
  if (!state.dynamicThresholds) {
    return {
      cascading: false,
      failureCount: 0,
  ...
  state.cascadeFailureWindow = state.cascadeFailureWindow.filter(
    (f: any) => (now - f.timestamp) <= windowMs
  );
 ...
 */
export function recordFailureForCascade(state: any, taskId: string): void {
...
function logCascadeFailureIncident(state: any, result: CascadeFailureCheckResult): void {
  const incident = {
 ...
 */
export function getDivergenceRateStatus(state: any, recentEpisodeCount: number = 20): DivergenceRateStatus {
  if (!state.dynamicThresholds) {
    return {
      currentRate: 0.05,
  ...
  const successRate = recentPerf.length > 0
    ? recentPerf.filter((p: any) => p.success).length / recentPerf.length
    : 0;
 ...
 */
export function applyDivergenceRate(state: any): number {
...
function logDivergenceIncident(state: any, status: DivergenceRateStatus): void {
  const incident = {
    timestamp: new Date().toISOString(),
    type: 'DIVERGENCE_HIGH' as const,
    details: {
 ...
 */
export function getAdaptiveCheckFrequency(state: any): number {
  if (!state.dynamicThresholds) {
 ...
 */
export function shouldPerformCheck(state: any, episodeCount: number): boolean {
 ...
 */
export function getQuantileThreshold(state: any): number {
  if (!state.dynamicThresholds) {
    return 0.75;
  }
 ...
 */
export function isPerformanceOutlier(state: any, reward: number): boolean {
