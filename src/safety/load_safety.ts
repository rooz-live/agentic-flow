...
const DEFAULT_OBS_GUARDRAIL: ObservabilityGuardrail = {
...
export class LoadSafetyController {
  private state: LoadSafetyState;
  private tokenConfig: TokenBucketConfig;
  private backoffConfig: BackoffConfig;
  private obsGuardrail: ObservabilityGuardrail;

  constructor(
    tokenConfig?: Partial<TokenBucketConfig>,
    backoffConfig?: Partial<BackoffConfig>,
    obsGuardrail?: Partial<ObservabilityGuardrail>
  ) {
   ...
   */
  requestTokens(depth: number = 1): { allowed: boolean; waitMs: number } {
    this.refillTokens();
  ...
  private refillTokens(): void {
    ...
    this.state.tokens = Math.min(
      this.tokenConfig.burstCapacity,
   ...
   */
  recordFailure(): { shouldBackoff: boolean; sleepMs: number } {
    ...
    if (this.state.failureTimestamps.length >= this.backoffConfig.failThreshold) {
      ...
      const sleepMs = Math.min(
        this.backoffConfig.sleepSeconds * 1000 * backoffMultiplier,
        this.backoffConfig.maxSleepSeconds * 1000
      );
  ...
  checkObservabilityGrowth(bytesWritten: number): {
    ...
    if (this.state.bytesThisMinute > this.obsGuardrail.maxBytesPerMinute) {
      ...
      const newRate = Math.min(
        this.obsGuardrail.currentSampleRate * 2,
        this.obsGuardrail.maxSampleRate
      );
