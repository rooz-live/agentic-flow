...
export interface GLMREAPConfig {
  ...
  model?: string; // Default: '0xSero/GLM-4.7-REAP-50-W4A16'
  apiKey?: string; // Optional for local deployment
...
export class GLMREAPClient {
  private config: Required<GLMREAPConfig>;
   ...
   */
  async complete(request: GLMREAPCompletionRequest): Promise<GLMREAPCompletionResponse> {
    const startTime = Date.now();

    try {
      const observatory = getLLMObservability();
      return await observatory.trackInference(
        `${this.config.model}`,
        async () => {
   ...
   */
  async chat(request: GLMREAPChatRequest): Promise<GLMREAPCompletionResponse> {
    const startTime = Date.now();
   ...
   */
  private formatChatPrompt(messages: GLMREAPChatMessage[]): string {
    return messages
      .map((msg) => {
        switch (msg.role) {
          case 'system':
   ...
   */
  private async callVLLMAPI(endpoint: string, body: any): Promise<any> {
        ...
        res.on('end', () => {
          try {
            ...
            if (res.statusCode >= 200 && res.statusCode < 300) {
              ...
              reject(
                new Error(
                  `vLLM API error (${res.statusCode}): ${data.error?.message || responseBody}`
                )
              );
            }
          } catch (err) {
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      return response.data?.some((model: any) => model.id === this.config.model);
    } catch (error) {
 ...
 * Factory function
 */
export function createGLMREAPClient(config?: GLMREAPConfig): GLMREAPClient {
