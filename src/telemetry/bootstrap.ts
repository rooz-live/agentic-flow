...
import { Metrics, NoopMetrics } from '../notifications/metrics';
...
export function startTelemetry(options?: {
  ...
  const metrics: Metrics = (options?.useProm !== false) ? new PromMetrics() : new NoopMetrics();
  ...
  let otelStop: (() => Promise<void>) | undefined;
  (async () => {
    try {
  ...
  const stop = async () => {
    try { sink.stop(); } catch {}
    if (otelStop) { try { await otelStop(); } catch {} }
  };
...
export function getTelemetry(): TelemetryRuntime | undefined {
  return runtime;
}
