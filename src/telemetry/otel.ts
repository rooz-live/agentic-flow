...
export async function startOtel(): Promise<OtelRuntime> {
  ...
  try {
    ...
    if (extra) Object.assign(resourceAttrs, extra);
    ...
    await sdk.start();
    runtime = { sdk, started: true, stop: async () => { try { await sdk.shutdown(); } catch {} } };
    return runtime;
  } catch {
