...
export class PerformanceAnalytics extends EventEmitter {
  ...
  updatePortfolioValue(value: number, timestamp?: string): void {
    this.portfolioValue.push({ value, timestamp: timestamp || new Date().toISOString() } as any);
  ...
  private calculateReturns(): number[] {
    ...
    for (let i = 1; i < this.portfolioValue.length; i++) {
      const currentValue = typeof this.portfolioValue[i] === 'number' ? this.portfolioValue[i] : (this.portfolioValue[i] as any).value;
      const previousValue = typeof this.portfolioValue[i - 1] === 'number' ? this.portfolioValue[i - 1] : (this.portfolioValue[i - 1] as any).value;
  ...
  private calculateTotalReturn(): number {
    if (this.portfolioValue.length < 2) return 0;
    
    const lastVal = this.portfolioValue[this.portfolioValue.length - 1];
    const firstVal = this.portfolioValue[0];
    const currentValue = typeof lastVal === 'number' ? lastVal : (lastVal as any).value;
    const initialValue = typeof firstVal === 'number' ? firstVal : (firstVal as any).value;
  ...
  private calculateMonthlyReturn(): number {
    if (this.portfolioValue.length < 30) return 0;
    
    const lastVal = this.portfolioValue[this.portfolioValue.length - 1];
    const monthVal = this.portfolioValue[this.portfolioValue.length - 30];
    const currentValue = typeof lastVal === 'number' ? lastVal : (lastVal as any).value;
    const monthAgoValue = typeof monthVal === 'number' ? monthVal : (monthVal as any).value;
   ...
   */
  private calculateDailyReturn(returns: number[]): number {
    if (returns.length === 0) return 0;
  ...
  private calculateMaxDrawdown(): number {
    ...
    let maxDrawdown = 0;
    const firstVal = this.portfolioValue[0];
    let peak = typeof firstVal === 'number' ? firstVal : (firstVal as any).value;
    
    for (let i = 1; i < this.portfolioValue.length; i++) {
      const val = this.portfolioValue[i];
      const currentValue = typeof val === 'number' ? val : (val as any).value;
  ...
  private calculateBeta(returns: number[]): number {
    const benchmarkReturns = this.getBenchmarkReturns('SPY');
    if (benchmarkReturns.length === 0 || returns.length === 0) return 1;
  ...
  private calculateTrackingError(returns: number[]): number {
    const benchmarkReturns = this.getBenchmarkReturns('SPY');
    if (benchmarkReturns.length === 0 || returns.length === 0) return 0;
   ...
   */
  private getBenchmarkReturns(symbol: string): number[] {
   ...
   */
  private getCompletedTrades(): TradeRecord[] {
   ...
   */
  private calculateAverageTradeDuration(trades: TradeRecord[]): number {
   ...
   */
  private calculateTotalCommission(trades: TradeRecord[]): number {
   ...
   */
  private calculateCommissionRate(trades: TradeRecord[], totalValue: number): number {
    const totalCommission = this.calculateTotalCommission(trades);
   ...
   */
  private calculateExecutionQuality(trades: TradeRecord[]): number {
    const completedTrades = this.getCompletedTrades();
  ...
  private getCurrentPortfolioValue(): number {
    if (this.portfolioValue.length === 0) return 0;
    const lastVal = this.portfolioValue[this.portfolioValue.length - 1];
    return typeof lastVal === 'number' ? lastVal : (lastVal as any).value;
  }
   ...
   */
  private getCurrentSectorAttribution(): Record<string, number> {
   ...
   */
  private getCurrentStrategyAttribution(): Record<string, number> {
   ...
   */
  private getCurrentFactorExposure(): Record<string, number> {
   ...
   */
  private calculatePeriodReturn(startDate: string, endDate: string): number {
   ...
   */
  private calculateBenchmarkReturn(startDate: string, endDate: string): number {
   ...
   * Calculate factor attribution
   */
  private calculateFactorAttribution(startDate: string, endDate: string): any {
   ...
   * Calculate sector attribution
   */
  private calculateSectorAttribution(startDate: string, endDate: string): Record<string, any> {
    // Simplified sector attribution
    return {
   ...
   * Calculate strategy attribution
   */
  private calculateStrategyAttribution(startDate: string, endDate: string): Record<string, any> {
    // Simplified strategy attribution
    return {
   ...
   */
  private getPortfolioValueAtDate(date: string): number {
    const targetDate = new Date(date);
    
    for (let i = this.portfolioValue.length - 1; i >= 0; i--) {
      const val = this.portfolioValue[i];
      const valueDate = new Date(typeof val === 'number' ? Date.now() : (val as any).timestamp);
      if (valueDate <= targetDate) {
        return typeof val === 'number' ? val : (val as any).value;
      }
    }
  ...
  private checkPerformanceAlerts(): void {
    ...
    if (metrics.currentDrawdown > this.config.alertThresholds.maxDrawdown) {
      this.createAlert('DRAWDOWN', 'HIGH', 
        `Portfolio drawdown of ${(metrics.currentDrawdown * 100).toFixed(1)}% exceeds threshold`,
        metrics.currentDrawdown);
    }
    
    // Check volatility alert
    if (metrics.volatility > this.config.alertThresholds.maxVolatility) {
      this.createAlert('VOLATILITY', 'MEDIUM',
        `Portfolio volatility of ${(metrics.volatility * 100).toFixed(1)}% exceeds threshold`,
        metrics.volatility);
    }
    ...
    if (metrics.sharpeRatio < this.config.alertThresholds.minSharpe) {
      this.createAlert('PERFORMANCE', 'LOW',
        `Sharpe ratio of ${metrics.sharpeRatio.toFixed(2)} below threshold`,
        metrics.sharpeRatio);
    }
  }
   ...
   */
  private getAlertRecommendations(type: PerformanceAlert['type']): string[] {
    switch (type) {
      case 'DRAWDOWN':
