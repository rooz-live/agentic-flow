...
export class DimensionalCoherenceValidator {
  private driftHistory: MisalignmentSignal[] = [];
  ...
  private assessManthra(intention: string): DimensionalState['manthra'] {
  ...
  private assessYasna(intention: string, documentation: string): DimensionalState['yasna'] {
    const intentionConcepts = this.extractConcepts(intention);
    const docConcepts = this.extractConcepts(documentation);

    const overlap = intentionConcepts.filter(c =>
      docConcepts.some(d => d.toLowerCase().includes(c.toLowerCase()))
    ).length;
  ...
  private assessMithra(
    ...
    documentation: string,
    implementation: string
  ): DimensionalState['mithra'] {
    const intentionConcepts = this.extractConcepts(intention);
    const implConcepts = this.extractConcepts(implementation);

    const binding = intentionConcepts.filter(c =>
      implConcepts.some(i => i.toLowerCase().includes(c.toLowerCase()))
    ).length;
    ...
    const docConcepts = this.extractConcepts(documentation);
    const docImplOverlap = docConcepts.filter(d =>
      implConcepts.some(i => i.toLowerCase().includes(d.toLowerCase()))
    ).length;
  ...
  private detectMisalignments(
    ...
    const signals: MisalignmentSignal[] = [];
  ...
  private categoryToDimension(category: string): 'manthra' | 'yasna' | 'mithra' {
    if (category.includes('confidence') || category.includes('moral')) return 'yasna';
    if (category.includes('dissent') || category.includes('suffering')) return 'mithra';
    return 'manthra';
  }

  private getRemediationPath(category: string): string {
  ...
  private calculateCollapseRisk(
    misalignments: MisalignmentSignal[],
    manthra: DimensionalState['manthra'],
    yasna: DimensionalState['yasna'],
    mithra: DimensionalState['mithra']
  ): number {
