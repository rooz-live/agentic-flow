...
# 2. each hard-gate override in isolation
...
def test_hard_gate_rev0_br15_without_sign_off():
    ...
    sc["impact"]["tail_risks"] = []
    res = evaluate(sc)
    assert "REV0 x BR1.5 requires sign_off" in res["errors"]
    assert res["decision"] == "DROP"
    ...
    sc["impact"]["sign_off"] = True
    res = evaluate(sc)
    assert "REV0 x BR1.5 requires sign_off" not in res["errors"]
    assert res["decision"] == "SHIP"
...
@pytest.mark.parametrize("disp", ["invalid", "none", "", None])
...
def test_invalid_enums():
    ...
    with pytest.raises(ValueError, match="cod_weight must be in"):
        evaluate(sc)

    sc = make_valid_scorecard()
    sc["impact"]["blast_radius"] = 0.1
    with pytest.raises(ValueError, match="blast_radius must be in"):
        evaluate(sc)

    sc = make_valid_scorecard()
    sc["impact"]["reversibility"] = 2
    with pytest.raises(ValueError, match="reversibility must be 0 or 1"):
        evaluate(sc)

def test_missing_fields():
    sc = make_valid_scorecard()
    del sc["originality"]["coherence"]
    with pytest.raises(ValueError, match="Missing required originality field"):
        evaluate(sc)

    sc = make_valid_scorecard()
    del sc["impact"]["gate_integrity"]
    with pytest.raises(ValueError, match="Missing required impact field"):
        evaluate(sc)
...
# 6. multiple/malformed fenced blocks in --pr-body
...
def test_extract_no_block():
    assert extract_from_text("No block") == {}

def test_extract_malformed_block():
    body = "```json\n{\n  \"coherence\": # invalid\n}\n```"
    with pytest.raises(ValueError, match="Malformed JSON in fenced block"):
...
def test_derive_coherence_artifact_ingestion(tmp_path, monkeypatch):
    ...
    # Matching head but with force_dynamic=True
    artifact_file.write_text(json.dumps({
        "coherence": "PASS",
        "git_head": "mocked_sha12345"
    }))
...
def test_evaluate_derive_mode_ingest_only(tmp_path, monkeypatch):
    monkeypatch.setattr("scripts.gates.scorecard_gate.derive_gate_integrity", lambda: "PASS")
    sc = make_valid_scorecard()
    # No artifact exists in default test environment root path (tmp_path), so ingest_only should return FAIL
