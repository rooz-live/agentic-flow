...
exports.GovernanceSystem = void 0;
const fs_1 = require("fs");
const path_1 = require("path");
const decision_audit_logger_1 = require("./decision_audit_logger");
class GovernanceSystem {
    constructor(config) {
        ...
        this.auditLogger = new decision_audit_logger_1.DecisionAuditLogger(this.goalieDir);
        this.initializeDefaultPolicies();
    }
    async initialize() {
        // Load any custom policies from filesystem
        const policiesPath = (0, path_1.join)(this.goalieDir, 'governance_policies.json');
        if ((0, fs_1.existsSync)(policiesPath)) {
            try {
                const customPolicies = JSON.parse((0, fs_1.readFileSync)(policiesPath, 'utf-8'));
    ...
    async checkLiveDimension() {
        ...
        try {
            const events = this.loadPatternEvents();
            const recentEvents = events.filter(e => new Date(e.ts) > new Date(Date.now() - 24 * 60 * 60 * 1000));
            // Look for evidence of adaptive behavior (changing thresholds, learning)
            ...
            const adaptivityRate = recentEvents.length > 0
    ...
    loadPatternEvents() {
        const logPath = (0, path_1.join)(this.goalieDir, 'pattern_metrics.jsonl');
...
exports.GovernanceSystem = GovernanceSystem;
exports.default = GovernanceSystem;
