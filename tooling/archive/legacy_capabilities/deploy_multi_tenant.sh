...
echo "🛡️ TRIGGERING CI-GATE (QUALITY ENGINEERING BOUNDARY)..."
if ! bash tooling/scripts/ci-gate.sh; then
    echo "❌ [DEPLOYMENT ABORTED] CI-Gate structural failure. Rollback mandated."
    exit 1
fi
echo "🟢 [DEPLOYMENT CONTINUING] Physics metrics verified."
...
case "$ENV" in
  ...
  prod)
    ...
    if [[ -z "$KUBECONFIG" ]]; then
      echo "⚠️ KUBECONFIG_PROD bypass enabled. Automatically sourcing from OpenStack STX Bare-Metal Edge (23.92.79.2)..."
      STX_HOST="23.92.79.2"
      STX_USER="${STX_USER:-sysadmin}"
      # Attempt to securely scrape the Kubeconfig from the StarlingX environment
      scp -o StrictHostKeyChecking=no -q "$STX_USER@$STX_HOST:~/.kube/config" /tmp/kubeconfig_stx_prod 2>/dev/null || true
      if [[ -s /tmp/kubeconfig_stx_prod ]]; then
         KUBECONFIG="/tmp/kubeconfig_stx_prod"
         export KUBECONFIG
         echo "✅ STX Context Bound Successfully."
      else
         echo "❌ CRITICAL: Could not bypass locally. StarlingX SSH target missed. Fallback to KUBECONFIG_PROD environment variable required."
         exit 1
      fi
    fi
    if [[ $DRY_RUN == true ]]; then
