 ...
 */
export function loadAutoApplyPolicy(goalieDir: string): AutoApplyPolicy {
  const policyPath = path.join(goalieDir, 'autocommit_policy.yaml');
  
  // Default policy (conservative)
  const defaultPolicy: AutoApplyPolicy = {
    enabled: false,
...
export function filterByDecision(
  ...
  targetDecision: 'apply' | 'dry-run' | 'manual'
): PolicyDecision[] {
...
export async function applyCodeFix(
  ...
  policy: AutoApplyPolicy,
  goalieDir: string
): Promise<CodeFixApplicationResult> {
  const startTime = Date.now();
  const result: CodeFixApplicationResult = {
 ...
 */
async function runTests(testFiles: string[]): Promise<{ passed: number; failed: number; skipped: number }> {
