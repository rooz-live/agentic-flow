...
function classifyProposalRisk(
  ...
  let level: RiskLevel = hasCode ? 'medium' : 'low';
...
function applyAutoApplyPolicy(
  ...
  const mode =
    (process.env.AF_GOVERNANCE_AUTO_APPLY_MODE || 'none') as
      | 'none'
      | 'low-risk'
      | 'economic'
      | 'all';
  ...
  return proposals.map((proposal) => {
    ...
    const canAutoApply =
      allowAutocommit && !highRisk && riskOk && economicOk && circleOk && depthOk;

    if (canAutoApply) {
      // eslint-disable-next-line no-console
      console.error(
        `[governance_agent] Auto-apply eligible code fix for pattern="${proposal.pattern}", circle=${meta.circle}, depth=${meta.depth}, risk=${risk}, impact=${meta.totalImpactAvg}, wsjf=${meta.wsjfAvg ?? 0}`,
      );
      return {
        ...proposal,
        mode: 'apply',
