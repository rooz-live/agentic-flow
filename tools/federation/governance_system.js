...
function checkCompliance() {
  ...
  try {
    const policiesPath = path.join(GOALIE_DIR, 'governance_policies.json');
    if (fs.existsSync(policiesPath)) {
      const raw = fs.readFileSync(policiesPath, 'utf-8');
      const policies = JSON.parse(raw);
      if (Array.isArray(policies)) {
        metrics.audit.totalPolicies = policies.filter(p => (p?.status || 'active') === 'active').length;
      }
    }
  } catch (_err) {
    // ignore
  ...
  try {
    ...
    if (fs.existsSync(dbPath)) {
      const q = `SELECT DISTINCT policy_id FROM governance_decisions WHERE policy_id IS NOT NULL AND timestamp >= ${cutoffSeconds};`;
    ...
    metrics.audit.mirrorCoverage = metrics.audit.totalPolicies > 0
      ? metrics.audit.auditedPolicies / metrics.audit.totalPolicies
      : 0;
  } catch (_err) {
