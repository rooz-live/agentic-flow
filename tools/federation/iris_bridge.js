...
exports.EnterpriseIrisBridge = void 0;
exports.captureIrisMetrics = captureIrisMetrics;
exports.__setCommandRunner = __setCommandRunner;
exports.__resetIrisBridgeCache = __resetIrisBridgeCache;
exports.isIrisMetricsEnabled = isIrisMetricsEnabled;
...
class EnterpriseIrisBridge extends events_1.EventEmitter {
    ...
    reinitializeEnterpriseFeatures() {
        if (!this.config.enableEnterpriseFeatures)
            return;
        this.circuitBreaker = this.createCircuitBreaker();
        this.retryStrategy = this.createRetryStrategy();
    ...
    // Public API
    // ============================================================================
    async captureIrisMetrics(command, args = [], options) {
        ...
        try {
            ...
            // Apply middleware
            const irisCommand = { command, args };
    ...
    // ============================================================================
    async executeWithEnterpriseFeatures(command, options) {
    ...
    async executeWithRetry(command, retryConfig, operation) {
        let lastError;
        const attempts = [];
    ...
    async executeWithCircuitBreaker(command, operation) {
    ...
    async executeCommandWithMonitoring(command) {
        const startTime = Date.now();
    ...
    async executeIrisCommand(command, args) {
        return new Promise((resolve, reject) => {
            ...
            const child = (0, child_process_1.spawn)(executable, cmdArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
            let stdout = '';
            let stderr = '';
            ...
            child.on('error', (error) => {
                // Graceful fallback for missing IRIS CLI
                if (error.message.includes('ENOENT') || stderr.includes('command not found')) {
                    console.warn('[iris_bridge] IRIS CLI not found, using mock data');
                    resolve(JSON.stringify({
                        command,
                        status: 'mock',
                        message: 'IRIS CLI not installed - using fallback',
                        timestamp: new Date().toISOString()
                    }));
                } else {
                    reject(error);
            ...
            child.on('close', (code) => {
                ...
                else if (stderr.includes('command not found') || stderr.includes('IRIS command not found')) {
                    // Fallback for when IRIS isn't installed
                    console.warn('[iris_bridge] IRIS CLI not found, using mock data');
                    resolve(JSON.stringify({
                        command,
                        status: 'mock',
    ...
    // ============================================================================
    async createMetricsEvent(command, result, correlationId, executionId, options) {
    ...
    async createErrorMetricsEvent(command, args, error, correlationId, executionId, options) {
        const config = await (0, iris_config_js_1.loadIrisConfig)();
        return {
            // Basic event fields
            type: 'iris_evaluation',
    ...
    generateCorrelationId() {
        return `iris-${(0, crypto_1.randomUUID)()}`;
    }
    generateExecutionId() {
        return `exec-${this.executionCount++}-${Date.now()}`;
    }
    generateTraceId() {
        return `trace-${(0, crypto_1.randomUUID)()}`;
    }
    ...
    canExecuteCommand(command) {
        return this.circuitBreaker.state !== 'open' ||
            (this.circuitBreaker.state === 'half-open' && this.shouldAllowHalfOpenCall());
    }
    shouldAllowHalfOpenCall() {
    ...
    updateCircuitBreakerMetrics(success) {
        if (success) {
            this.circuitBreaker.metrics.successCount++;
        }
        else {
    ...
    async applyMiddlewareItem(middleware, command) {
        return new Promise((resolve, reject) => {
            const next = async () => {
                try {
    ...
    startDistributedTrace(traceId, command) {
        return {
            traceId,
    ...
    endDistributedTrace(trace, metrics, error) {
        trace.endTime = Date.now();
        trace.duration = trace.endTime - trace.startTime;
        if (metrics) {
            trace.tags = { ...trace.tags, ...metrics };
        }
        if (error) {
    ...
    async buildProductionMaturity(data) {
        // This would be implemented based on the existing logic from the original file
        return {
    ...
    resolveExecutionContext(data, options) {
        return {
...
exports.EnterpriseIrisBridge = EnterpriseIrisBridge;
// ============================================================================
...
// Run CLI if this file is executed directly (CommonJS compatible)
