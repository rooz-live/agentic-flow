...
exports.IrisConfigManager = void 0;
exports.createIrisConfigManager = createIrisConfigManager;
exports.loadIrisConfig = loadIrisConfig;
exports.getIrisConfig = getIrisConfig;
exports.updateIrisConfig = updateIrisConfig;
exports.watchIrisConfig = watchIrisConfig;
...
const RetryConfigSchema = zod_1.z.object({
    ...
    baseDelayMs: zod_1.z.number().min(100).max(60000),
    maxDelayMs: zod_1.z.number().min(1000).max(300000),
    backoffMultiplier: zod_1.z.number().min(1.1).max(5),
    jitter: zod_1.z.boolean(),
});
const CircuitBreakerConfigSchema = zod_1.z.object({
    failureThreshold: zod_1.z.number().min(1).max(100),
    recoveryTimeoutMs: zod_1.z.number().min(1000).max(600000),
    monitoringPeriodMs: zod_1.z.number().min(1000).max(300000),
    halfOpenMaxCalls: zod_1.z.number().min(1).max(20),
});
const PerformanceConfigSchema = zod_1.z.object({
    enableMetrics: zod_1.z.boolean(),
    enableTracing: zod_1.z.boolean(),
    memoryThresholdMb: zod_1.z.number().min(64).max(16384),
    executionTimeoutMs: zod_1.z.number().min(1000).max(600000),
    enableResourceMonitoring: zod_1.z.boolean(),
});
const ValidationConfigSchema = zod_1.z.object({
    enableInputSanitization: zod_1.z.boolean(),
    enableCommandWhitelist: zod_1.z.boolean(),
    allowedCommands: zod_1.z.array(zod_1.z.string()),
    maxArgsLength: zod_1.z.number().min(1).max(1000),
    enableOutputValidation: zod_1.z.boolean(),
});
const ConcurrencyConfigSchema = zod_1.z.object({
    maxConcurrentCommands: zod_1.z.number().min(1).max(100),
    resourcePoolSize: zod_1.z.number().min(1).max(200),
    queueTimeoutMs: zod_1.z.number().min(1000).max(300000),
    enablePriorityQueue: zod_1.z.boolean(),
});
const BridgeConfigSchema = zod_1.z.object({
    retry: RetryConfigSchema,
    circuitBreaker: CircuitBreakerConfigSchema,
    performance: PerformanceConfigSchema,
    validation: ValidationConfigSchema,
    concurrency: ConcurrencyConfigSchema,
    enableEnterpriseFeatures: zod_1.z.boolean(),
    logLevel: zod_1.z.enum(['debug', 'info', 'warn', 'error']),
});
const EnvironmentConfigSchema = zod_1.z.object({
    name: zod_1.z.string(),
    type: zod_1.z.enum(['development', 'staging', 'production']),
    features: zod_1.z.object({
        circuitBreaker: zod_1.z.boolean(),
        retry: zod_1.z.boolean(),
        performance: zod_1.z.boolean(),
        validation: zod_1.z.boolean(),
        concurrency: zod_1.z.boolean(),
    }),
    thresholds: zod_1.z.object({
        maxExecutionTime: zod_1.z.number(),
        maxMemoryUsage: zod_1.z.number(),
        maxFailureRate: zod_1.z.number(),
        minSuccessRate: zod_1.z.number(),
    }),
    notifications: zod_1.z.object({
        onFailure: zod_1.z.boolean(),
        OnCircuitBreak: zod_1.z.boolean(),
        OnPerformanceDegradation: zod_1.z.boolean(),
    }),
});
const IntegrationConfigSchema = zod_1.z.object({
    metrics: zod_1.z.object({
        enabled: zod_1.z.boolean(),
        endpoint: zod_1.z.string().url().optional(),
        format: zod_1.z.enum(['json', 'prometheus', 'custom']),
        batchSize: zod_1.z.number().min(1).max(1000),
        flushInterval: zod_1.z.number().min(1000).max(300000),
    }),
    tracing: zod_1.z.object({
        enabled: zod_1.z.boolean(),
        endpoint: zod_1.z.string().url().optional(),
        samplingRate: zod_1.z.number().min(0).max(1),
        includeArgs: zod_1.z.boolean(),
        includeOutput: zod_1.z.boolean(),
    }),
    logging: zod_1.z.object({
        level: zod_1.z.enum(['debug', 'info', 'warn', 'error']),
        format: zod_1.z.enum(['json', 'text']),
        includeStackTrace: zod_1.z.boolean(),
        maxLogSize: zod_1.z.number().min(1000).max(10000000),
    }),
});
// ============================================================================
...
class IrisConfigManager {
    ...
    async loadConfig() {
        try {
            ...
            const raw = await fs_1.promises.readFile(this.configPath, 'utf-8');
...
exports.IrisConfigManager = IrisConfigManager;
// ============================================================================
// Utility Functions
// ============================================================================
function createIrisConfigManager(configPath) {
    return IrisConfigManager.getInstance(configPath);
}
async function loadIrisConfig(configPath) {
...
function updateIrisConfig(updates) {
    const manager = IrisConfigManager.getInstance();
    manager.updateConfig(updates);
}
function watchIrisConfig(callback) {
    const manager = IrisConfigManager.getInstance();
