...
import type {
  IrisBridgeConfig,
  IrisRetryConfig,
  IrisCircuitBreakerConfig,
  IrisPerformanceConfig,
  IrisValidationConfig,
  IrisConcurrencyConfig,
  IrisEnvironmentConfig,
  IrisIntegrationConfig,
  DEFAULT_IRIS_RETRY_CONFIG,
  DEFAULT_IRIS_CIRCUIT_BREAKER_CONFIG,
  DEFAULT_IRIS_PERFORMANCE_CONFIG,
  DEFAULT_IRIS_VALIDATION_CONFIG,
  DEFAULT_IRIS_CONCURRENCY_CONFIG,
} from './iris_types.js';
...
export class IrisConfigManager {
  private static instance: IrisConfigManager;
  private config: IrisBridgeConfig;
  private configPath: string;
  private lastModified: number | null = null;
  private watchers: Array<(config: IrisBridgeConfig) => void> = [];
  ...
  public static getInstance(configPath?: string): IrisConfigManager {
  ...
  public async loadConfig(): Promise<IrisBridgeConfig> {
    try {
  ...
  public async saveConfig(config: Partial<IrisBridgeConfig>): Promise<void> {
    try {
  ...
  public getConfig(): IrisBridgeConfig {
    return { ...this.config };
  }

  public updateConfig(updates: Partial<IrisBridgeConfig>): void {
  ...
  public watchConfig(callback: (config: IrisBridgeConfig) => void): () => void {
  ...
  private loadEnvironmentOverrides(): Partial<IrisBridgeConfig> {
    const overrides: Partial<IrisBridgeConfig> = {};
    ...
    if (process.env.IRIS_LOG_LEVEL) {
      ...
      if (['debug', 'info', 'warn', 'error'].includes(logLevel)) {
        overrides.logLevel = logLevel as 'debug' | 'info' | 'warn' | 'error';
      }
    }
  ...
  private mergeConfigurations(base: IrisBridgeConfig, override: Partial<IrisBridgeConfig>): IrisBridgeConfig {
    return {
  ...
  public async loadEnvironmentConfig(environmentName?: string): Promise<IrisEnvironmentConfig> {
  ...
  private getDefaultEnvironmentConfig(environment: string): IrisEnvironmentConfig {
    return {
      name: environment,
  ...
  public async loadIntegrationConfig(): Promise<IrisIntegrationConfig> {
  ...
  private getIntegrationConfigPath(): string {
    const cwd = process.cwd();
    return path.join(cwd, 'config', 'iris', 'integration.yaml');
  }

  private getDefaultIntegrationConfig(): IrisIntegrationConfig {
    return {
      metrics: {
        enabled: true,
        format: 'json',
  ...
  public validateConfig(config: Partial<IrisBridgeConfig>): { valid: boolean; errors: string[] } {
    try {
      const merged = this.mergeConfigurations(this.config, config);
  ...
  public async exportConfig(format: 'yaml' | 'json' = 'yaml'): Promise<string> {
  ...
  public async importConfig(configString: string, format: 'yaml' | 'json' = 'yaml'): Promise<void> {
    try {
      let parsed: any;
...
export function createIrisConfigManager(configPath?: string): IrisConfigManager {
  return IrisConfigManager.getInstance(configPath);
}

export async function loadIrisConfig(configPath?: string): Promise<IrisBridgeConfig> {
  const manager = IrisConfigManager.getInstance(configPath);
  return manager.loadConfig();
}

export function getIrisConfig(): IrisBridgeConfig {
  const manager = IrisConfigManager.getInstance();
  return manager.getConfig();
}

export function updateIrisConfig(updates: Partial<IrisBridgeConfig>): void {
  const manager = IrisConfigManager.getInstance();
  manager.updateConfig(updates);
}

export function watchIrisConfig(callback: (config: IrisBridgeConfig) => void): () => void {
  const manager = IrisConfigManager.getInstance();
