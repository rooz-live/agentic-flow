...
import { 
  loadRetroCoachContext, 
  applyAutoApplyPolicy,
  generateCodeFixProposals,
  type CodeFixProposal,
  type PatternEvent,
  type EconomicGapJsonRow
} from './governance_agent.js';

// Mock data for testing different system health scenarios
const mockPatterns: PatternEvent[] = [
  {
...
const mockTopEconomicGaps: EconomicGapJsonRow[] = [
  {
    pattern: 'safe-degrade',
    circle: 'Assessor',
    depth: 1,
...
function createMockRetroCoachContext(healthScenario: 'healthy' | 'moderate' | 'poor' | 'regressing') {
  const baseContext = {
    goalieDir: '/tmp/test',
...
function testPolicyScenario(scenario: string, mode: string) {
  ...
  const retroCtx = createMockRetroCoachContext(scenario as any);
  ...
  const enhancedProposals = applyAutoApplyPolicy(
    proposals,
    mockPatterns,
    mockTopEconomicGaps,
    loadedCtx
