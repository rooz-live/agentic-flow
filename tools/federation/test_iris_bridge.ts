...
async function runTest() {
    ...
    try {
        ...
        // Assertions
        if (metrics.type !== 'iris_evaluation') throw new Error(`Expected type 'iris_evaluation', got '${metrics.type}'`);
        if (metrics.iris_command !== 'evaluate') throw new Error(`Expected command 'evaluate', got '${metrics.iris_command}'`);
        if (metrics.execution_context.environment !== 'production_simulation') throw new Error(`Unexpected environment: ${metrics.execution_context.environment}`);
