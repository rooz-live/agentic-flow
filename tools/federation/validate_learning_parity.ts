...
interface CircleStats {
  ...
  behavioralTypes: Map<string, number>;
  avgDepth: number;
...
interface ParityResult {
  ...
  circleRepresentation: {
    [circle: string]: {
      ...
      avgDepth: number;
      iterations: number;
      behavioralTypes: { [type: string]: number };
      lastSeen: string;
    };
  };
  parityScore: number;
...
function normalizeCircleName(circle: string | undefined): string {
...
function analyzeCircleStats(events: PatternEvent[]): Map<string, CircleStats> {
  const statsMap = new Map<string, CircleStats>();
...
function calculateParityScore(statsMap: Map<string, CircleStats>, totalEvents: number): number {
...
function generateIssuesAndRecommendations(
  statsMap: Map<string, CircleStats>,
  totalEvents: number
  ...
  for (const circle of EXPECTED_CIRCLES) {
    ...
    if (stats.totalEvents === 0) {
      ...
      issues.push(
        `Circle ${circle} underrepresented: ${actualPct.toFixed(1)}% (expected ≥${expectedMin.toFixed(1)}%)`
      );
      recommendations.push(`Increase ${circle} circle iterations or add more ${circle}-specific patterns`);
    } else if (actualPct > expectedMax) {
      issues.push(
        `Circle ${circle} overrepresented: ${actualPct.toFixed(1)}% (expected ≤${expectedMax.toFixed(1)}%)`
      );
...
async function validateLearningParity(goalieDir: string): Promise<ParityResult> {
  ...
  const events = await readJsonl<PatternEvent>(patternMetricsPath);
...
function printHumanReadable(result: ParityResult): void {
  ...
  for (const [circle, stats] of sortedCircles) {
    if (stats.eventCount === 0 && circle === 'Unknown') continue;
