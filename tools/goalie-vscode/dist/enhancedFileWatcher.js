...
class EnhancedFileWatcher {
    constructor(workspaceRoot, refreshCallbacks, options = {}) {
        ...
        this.fileWatcherService = new fileWatcherService_1.FileWatcherService(workspaceRoot, watcherOptions);
    ...
    setupEventHandlers() {
        // Handle file changes
        this.fileWatcherService.onDidChangeFile((changes) => {
        ...
        if (this.options.enableVisualIndicators !== false) {
            this.fileWatcherService.onDidDetectNewFiles((newFiles) => {
     ...
     */
    showNewFileNotifications(newFiles) {
        if (newFiles.length === 0) {
            return;
        }
        const config = vscode.workspace.getConfiguration('goalie');
        const enableNotifications = config.get('fileWatcher.enableNotifications', true);
     ...
     */
    showNewFileDetails(newFiles) {
        var _a, _b;
        const workspaceRoot = this.workspaceRoot || ((_b = (_a = vscode.workspace.workspaceFolders) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.uri.fsPath);
        if (!workspaceRoot) {
            return;
        }
        const panel = vscode.window.createWebviewPanel('newFilesDetails', 'New Files Detected', vscode.ViewColumn.One, {
            enableScripts: true,
     ...
     */
    generateNewFilesHtml(newFiles, workspaceRoot) {
...
//# sourceMappingURL=enhancedFileWatcher.js.map