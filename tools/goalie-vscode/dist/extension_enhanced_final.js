...
function updateKanbanEntry(goalieDir, section, entryId, updates) {
    ...
    if (idx >= 0) {
        list[idx] = Object.assign(Object.assign(Object.assign({}, list[idx]), updates), { updatedAt: new Date().toISOString() });
        saveKanbanDoc(goalieDir, doc);
    }
}
function removeKanbanEntry(goalieDir, section, entryId) {
    const doc = loadKanbanDoc(goalieDir);
    if (!doc)
        return;
    const list = doc[section];
...
function addKanbanEntry(goalieDir, section, payload) {
    const doc = loadKanbanDoc(goalieDir);
    if (!doc) {
        throw new Error('Kanban board file not found.');
    }
    const targetEntries = ensureSection(doc, section);
    const newEntry = Object.assign(Object.assign({}, payload), { id: payload.id || Date.now().toString(), createdAt: new Date().toISOString() });
...
class GoalieKanbanProvider {
    constructor(workspaceRoot, logger) {
        ...
        this.dropMimeTypes = ['application/vnd.code.tree.goalieKanbanItem'];
        this.dragMimeTypes = ['application/vnd.code.tree.goalieKanbanItem'];
    }
    handleDrag(source, dataTransfer, token) {
        if (source.length > 0 && source[0].payload && source[0].section) {
            const item = source[0];
            const transferData = Object.assign(Object.assign({}, item.payload), { _fromSection: item.section });
    ...
    handleDrop(target, dataTransfer, token) {
        ...
        try {
            ...
            const fromSection = data._fromSection;
            const { _fromSection } = data, payload = __rest(data, ["_fromSection"]);
            let targetSection;
            if (target) {
...
class PatternItem extends vscode.TreeItem {
    constructor(label, collapsibleState, metric) {
        super(label, collapsibleState);
        this.label = label;
...
class PatternMetricsProvider {
    ...
    loadPersistedFilters() {
        if (this.context) {
            const persisted = this.context.globalState.get('goalie.patternMetrics.filters');
            if (persisted) {
    ...
    startAutoRefresh() {
        ...
        const interval = vscode.workspace.getConfiguration('goalie').get('patternMetrics.refreshInterval') || 30;
        if (interval > 0) {
            ...
            try {
                    ...
                    filteredLines = lines.filter(line => {
                        try {
                            ...
                            for (const filter of this.currentFilters) {
                                if (filter.type === 'gate' && m.gate !== filter.value)
                                    return false;
                ...
                const metricItems = filteredLines.slice(-this.pageSize).reverse().map(line => {
                    ...
                    try {
                        ...
                        if ((_a = metric.economic) === null || _a === void 0 ? void 0 : _a.wsjf_score) {
                            const score = metric.economic.wsjf_score;
                            let rec = 'Low Priority';
                            if (score > 10)
                                rec = '🔥 High Priority - Expedite';
                            else if (score > 5)
                                rec = '⚠️ Medium Priority';
                            tooltipMd += `\n\n**💰 WSJF Score:** ${score}`;
                            tooltipMd += `\n**Recommendation:** ${rec}`;
                ...
                return items.length > 0 ? items : [new GoalieEmptyItem('No Metrics Match Filters', 'Try clearing filters', '**No metrics match your current criteria.**\n\nTry clearing filters or checking the data source.\n\nRight-click for aggregation options.')];
            }
            catch (e) {
    ...
    getPatternDetails(item) {
        ...
        if (m.economic) {
            ...
            if (wsjf_score > 10) {
                rec = 'Expedite (High Priority)';
                icon = 'flame';
            }
            else if (wsjf_score > 5) {
                rec = 'Schedule Soon (Medium)';
                icon = 'alert';
            }
...
class GovernanceEconomicsProvider {
    ...
    getChildren(element) {
        return __awaiter(this, void 0, void 0, function* () {
            return [new GoalieEmptyItem('Governance Data Unavailable', 'Check Logs', '**No governance insights found.**\n\nPlease check `governance_insights.jsonl` or run a governance audit.\n\nRight-click for options.')];
        });
    }
}
class DepthTimelineProvider {
    constructor() {
        this._onDidChangeTreeData = new vscode.EventEmitter();
        this.onDidChangeTreeData = this._onDidChangeTreeData.event;
    }
    ...
    getChildren(element) {
        return __awaiter(this, void 0, void 0, function* () {
            return [new GoalieEmptyItem('No Timeline Events', 'Awaiting Activity', '**No timeline events recorded.**\n\nActivity will appear here as the system processes workloads.\n\nRight-click to configure.')];
        });
    }
}
class ProcessFlowMetricsProvider {
    ...
    getChildren(element) {
        return __awaiter(this, void 0, void 0, function* () {
            return [new GoalieEmptyItem('No Process Metrics', 'Check Configuration', '**No process flow metrics available.**\n\nEnsure process flow monitoring is enabled in your configuration.\n\nRight-click for options.')];
        });
    }
}
const healthProvider_1 = require("./healthProvider");
...
function activate(context) {
    ...
    const dtCalibrationProvider = new dtCalibrationProvider_1.DtCalibrationProvider(workspaceRoot, outputChannel);
    const healthProvider = new healthProvider_1.GoalieHealthProvider(workspaceRoot);
    ...
    const promoteKanbanCommand = vscode.commands.registerCommand('goalie.promoteKanban', () => __awaiter(this, void 0, void 0, function* () {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalie.promoteToKanban', (item) => __awaiter(this, void 0, void 0, function* () {
        var _a, _b, _c;
        if (!(item === null || item === void 0 ? void 0 : item.metric))
            return;
        ...
        const entry = {
            ...
            wsjf_score: (_a = item.metric.economic) === null || _a === void 0 ? void 0 : _a.wsjf_score,
            cost_of_delay: (_b = item.metric.economic) === null || _b === void 0 ? void 0 : _b.cost_of_delay,
            job_size: (_c = item.metric.economic) === null || _c === void 0 ? void 0 : _c.job_size
        };
        // Prompt for section
        const section = yield vscode.window.showQuickPick(['NOW', 'NEXT', 'LATER'], {
            placeHolder: 'Select Kanban section for this pattern'
        });
        if (section) {
            addKanbanEntry(goalieDir, section, entry);
            kanbanProvider.refresh();
    ...
    const openAdminCommand = vscode.commands.registerCommand('goalie.openAdminPanel', () => {
        adminPanel_1.GoalieAdminPanel.createOrShow(context.extensionUri);
    });
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalie.showPatternDetails', (item) => {
        if (!(item === null || item === void 0 ? void 0 : item.metric))
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalie.showWsjfRecommendation', (item) => {
        var _a;
        if (!((_a = item === null || item === void 0 ? void 0 : item.metric) === null || _a === void 0 ? void 0 : _a.economic)) {
        ...
        if (wsjf_score > 10)
            recommendation = 'High Priority - Expedite';
        else if (wsjf_score > 5)
            recommendation = 'Medium Priority - Schedule Soon';
        else
            recommendation = 'Low Priority - Backlog';
        vscode.window.showInformationMessage(`WSJF Score: ${wsjf_score}\nCoD: ${cost_of_delay}, Size: ${job_size}\nRecommendation: ${recommendation}`, 'Open Details').then(selection => {
            if (selection === 'Open Details') {
                vscode.commands.executeCommand('goalie.showPatternDetails', item);
            }
        });
    }));
    context.subscriptions.push(vscode.commands.registerCommand('goalieKanban.editItem', (item) => __awaiter(this, void 0, void 0, function* () {
        var _a;
        if (!((_a = item === null || item === void 0 ? void 0 : item.payload) === null || _a === void 0 ? void 0 : _a.id))
            return;
        const goalieDir = getGoalieDir(workspaceRoot);
        if (!goalieDir)
            return;
        const currentTitle = item.payload.title || item.payload.summary || '';
        const newTitle = yield vscode.window.showInputBox({
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalieKanban.removeItem', (item) => __awaiter(this, void 0, void 0, function* () {
        var _a;
        if (!((_a = item === null || item === void 0 ? void 0 : item.payload) === null || _a === void 0 ? void 0 : _a.id))
            return;
        const confirm = yield vscode.window.showWarningMessage(`Are you sure you want to remove '${item.label}'?`, { modal: true }, 'Yes');
        if (confirm === 'Yes') {
            const goalieDir = getGoalieDir(workspaceRoot);
            if (goalieDir) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalieKanban.moveItem', (item) => __awaiter(this, void 0, void 0, function* () {
        if (!(item === null || item === void 0 ? void 0 : item.payload))
            return;
        const targetSection = yield vscode.window.showQuickPick(['NOW', 'NEXT', 'LATER'], {
            placeHolder: `Move '${item.label}' to...`
        });
        if (targetSection && targetSection !== item.section) {
            const goalieDir = getGoalieDir(workspaceRoot);
            if (goalieDir) {
                moveKanbanEntry(goalieDir, item.payload, item.section, targetSection);
                kanbanProvider.refresh();
            }
        }
    })));
    context.subscriptions.push(vscode.commands.registerCommand('goalie.copyItemValue', (item) => __awaiter(this, void 0, void 0, function* () {
        let valueToCopy = item.label;
        ...
        if (valueToCopy) {
            yield vscode.env.clipboard.writeText(valueToCopy);
            vscode.window.showInformationMessage('Copied to clipboard');
        }
    })));
    context.subscriptions.push(vscode.commands.registerCommand('goalieDashboard.filterByGate', (item) => {
        var _a;
        if ((_a = item === null || item === void 0 ? void 0 : item.metric) === null || _a === void 0 ? void 0 : _a.gate) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalieDashboard.filterByPattern', (item) => {
        var _a;
        if ((_a = item === null || item === void 0 ? void 0 : item.metric) === null || _a === void 0 ? void 0 : _a.pattern) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('goalie.openAggregationOptions', () => __awaiter(this, void 0, void 0, function* () {
        const options = [
...
//# sourceMappingURL=extension_enhanced_final.js.map