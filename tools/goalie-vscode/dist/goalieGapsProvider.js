...
exports.GoalieGapsProvider = exports.GoalieGapItem = void 0;
const fs = require("fs");
const path = require("path");
const vscode = require("vscode");
const yaml = require("yaml");
class GoalieGapItem extends vscode.TreeItem {
    constructor(label, gapContext) {
        super(label, vscode.TreeItemCollapsibleState.None);
        this.gapContext = gapContext;
    }
}
exports.GoalieGapItem = GoalieGapItem;
class GoalieGapsProvider {
    constructor(workspaceRoot, logger, sessionStats) {
    ...
    getChildren(element) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.logger.appendLine(`[Gaps] getChildren. Element: ${(_a = element === null || element === void 0 ? void 0 : element.label) !== null && _a !== void 0 ? _a : 'Root'}. Goalie Dir: ${goalieDir}`);
                ...
                try {
                    ...
                    for (const line of lines) {
                        try {
                            ...
                            const cur = agg.get(key) || { count: 0, codVals: [], wsjfVals: [], tags: new Set(), clusterLike: false, workstationLike: false, tfLike: false, torchLike: false };
                            cur.count += 1;
                            if (cod !== undefined)
                                cur.codVals.push(cod);
                            if (wsjf !== undefined)
                                cur.wsjfVals.push(wsjf);
                            const tagsArrRaw = Array.isArray(obj.tags) ? obj.tags.map((t) => String(t)) : [];
                            ...
                            const mergeConfigEntry = (entry) => {
                                ...
                                if (entry.clusterLike)
                                    cur.clusterLike = true;
                                if (entry.workstationLike)
                                    cur.workstationLike = true;
                                if (entry.tfLike)
                                    cur.tfLike = true;
                                if (entry.torchLike)
                                    cur.torchLike = true;
                            ...
                            const tagsArr = tagsArrRaw.map((t) => t.toLowerCase());
                    ...
                    function workloadTags(pattern, extraTags) {
                        const tags = [];
                        const extras = Array.from(extraTags);
                        const extrasLower = extras.map((t) => t.toLowerCase());
                        // Check for explicit bracketed tags
                        if (extras.some((t) => t.includes('[ML]')))
                            tags.push('ML');
                        if (extras.some((t) => t.includes('[HPC]')))
                            tags.push('HPC');
                        if (extras.some((t) => t.includes('[Stats]')))
                            tags.push('Stats');
                        // Check for plain workload labels
                        if (extrasLower.includes('ml'))
                            tags.push('ML');
                        if (extrasLower.includes('hpc'))
                            tags.push('HPC');
                        if (extrasLower.includes('stats'))
                            tags.push('Stats');
                        if (extrasLower.includes('device/web') ||
                        ...
                        if (mlFailurePatterns.includes(pattern))
                            tags.push('ML');
                        const hpcPatterns = [
                            'hpc-batch-window',
                            'cluster-fragmentation',
                        ...
                        if (hpcPatterns.includes(pattern))
                            tags.push('HPC');
                        const statsPatterns = [
                            'multiple-testing-correction',
                            'cross-validation-fold-failure',
                        ...
                        if (statsPatterns.includes(pattern))
                            tags.push('Stats');
                        ...
                        if (deviceWebPatterns.includes(pattern))
                            tags.push('Device/Web');
                    ...
                    for (const [key, a] of Array.from(agg.entries())) {
                        ...
                        const sessionStats = (_b = this.sessionStats) === null || _b === void 0 ? void 0 : _b.get(key);
                        if (sessionStats && (((_c = sessionStats.expands) !== null && _c !== void 0 ? _c : 0) > 0 || ((_d = sessionStats.quickFixes) !== null && _d !== void 0 ? _d : 0) > 0)) {
                            tooltip += `\n\nSession interactions: expands=${(_e = sessionStats.expands) !== null && _e !== void 0 ? _e : 0}, quick-fixes=${(_f = sessionStats.quickFixes) !== null && _f !== void 0 ? _f : 0}`;
                        }
...
exports.GoalieGapsProvider = GoalieGapsProvider;
//# sourceMappingURL=goalieGapsProvider.js.map