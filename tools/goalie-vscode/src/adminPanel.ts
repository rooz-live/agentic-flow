...
export class GoalieAdminPanel {
  public static currentPanel: GoalieAdminPanel | undefined;
  ...
  private static readonly CIRCLE_TIERS = {
    ...
    tier3: {
      ...
      reason:
        "Qualitative insights, contextual evaluation, relationship mapping",
    },
  };
  ...
  public static createOrShow(extensionUri: vscode.Uri, workspaceRoot?: string) {
    ...
    GoalieAdminPanel.currentPanel = new GoalieAdminPanel(
      panel,
      extensionUri,
      workspaceRoot,
    );
  ...
  private async _handleMessage(message: {
    ...
    switch (message.command) {
      case "saveConfig":
        await this._saveConfig(message.data as Record<string, unknown>);
        vscode.window.showInformationMessage("Goalie configuration saved");
        break;

      case "oauthSignIn":
        await this._handleOAuthSignIn(message.data?.domain as string);
        break;
      ...
      case "openExternalLink":
        if (message.data?.url) {
          await vscode.env.openExternal(
            vscode.Uri.parse(message.data.url as string),
          );
        }
        break;
  ...
  private async _saveConfig(data: Record<string, unknown>) {
    const config = vscode.workspace.getConfiguration("goalie");

    const updateIfPresent = async (
      key: string,
      value: unknown,
    ...
    if (data.mcpServers) {
      try {
        const parsed = JSON.parse(data.mcpServers as string);
  ...
  private async _handleOAuthSignIn(domain?: string) {
    if (domain) {
      const config = vscode.workspace.getConfiguration("goalie");
      await config.update(
        "oauth.domain",
  ...
  private async _runFederationCommand(command: string) {
  ...
  private async _runProdCycle(data?: Record<string, unknown>) {
  ...
  private async _calculateWsjf(data?: Record<string, unknown>) {
    if (!this.workspaceRoot) {
      vscode.window.showErrorMessage("No workspace root found");
      return;
    }

    const wsjfPath = path.join(
      this.workspaceRoot,
      "tools",
      "federation",
      "wsjf_calculator.ts",
  ...
  private async _saveCircleConfig(data?: Record<string, unknown>) {
    if (!this.workspaceRoot || !data) return;
  ...
  private _getGoalieStatus(): {
    hasGoalieDir: boolean;
    hasPatternMetrics: boolean;
    hasKanban: boolean;
    patternCount: number;
  } {
    if (!this.workspaceRoot) {
          ...
          ${domains.business
            ...
            <div class="domain-card ${config.oauthDomain === d.domain ? "selected" : ""}" onclick="selectDomain('${d.domain}')">
              <div class="domain-card-header">
                <span class="domain-icon">${d.icon}</span>
                <div>
                  <div class="domain-name">${d.name} ${(d as { wsjfEnabled?: boolean }).wsjfEnabled ? '<span class="badge info" style="font-size: 0.7em;">WSJF</span>' : ""}</div>
                  <div class="domain-url">${d.domain}</div>
                </div>
              </div>
              <div class="domain-description">${d.description}</div>
        ...
        ${Object.entries(tiers)
          ...
          <div class="tier-card">
            ...
            <div class="tier-circles">
              ${tier.circles.map((c: string) => `<span class="circle-tag">${c}</span>`).join("")}
            </div>
