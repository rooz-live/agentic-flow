...
export class AlertManager {
   ...
   */
  private getThresholdsForCategory(category: 'process' | 'flow' | 'learning' | 'pattern' | 'kanban'): AlertThreshold[] {
    switch (category) {
      case 'process':
   ...
   */
  private evaluateThreshold(threshold: AlertThreshold, value: number): EnhancedAlert | null {
   ...
   * Get previous values for trend analysis
   */
  private getPreviousValues(metric: string): number[] {
   ...
   */
  private calculateTrend(previousValues: number[], currentValue: number, direction: 'higher-is-better' | 'lower-is-better'): 'improving' | 'degrading' | 'stable' {
   ...
   */
  async sendNotification(alert: EnhancedAlert): Promise<void> {
  ...
  getActiveAlerts(): EnhancedAlert[] {
    const activeAlerts: EnhancedAlert[] = [];
