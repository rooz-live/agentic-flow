...
class GoalieKanbanProvider implements vscode.TreeDataProvider<KanbanItem> {
...
class PatternMetricsProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  private setupKeyboardShortcuts(): void {
    const config = vscode.workspace.getConfiguration('goalie');
    const enableShortcuts = config.get<boolean>('patternMetrics.enableKeyboardShortcuts', true);
  ...
  setMultipleFilters(filters: PatternFilter[]): void {
  ...
  private loadPersistedFilters(): void {
    if (this.context) {
      this.currentFilters = this.context.workspaceState.get<PatternFilter[]>('patternMetrics.filters', []);
      this.activePreset = this.context.workspaceState.get<string>('patternMetrics.activePreset', null);
      this.pageSize = this.context.globalState.get<number>('patternMetrics.pageSize', 50);
      this.currentPage = this.context.workspaceState.get<number>('patternMetrics.currentPage', 1);
    }
  }
  ...
  private startAutoRefresh(): void {
    const config = vscode.workspace.getConfiguration('goalie');
    const autoRefreshEnabled = config.get<boolean>('patternMetrics.autoRefresh', false);
    const refreshInterval = config.get<number>('patternMetrics.refreshInterval', 30); // seconds
  ...
  private detectNewPatterns(patterns: any[]): void {
  ...
  private showNewPatternNotification(pattern: any): void {
  ...
  private async loadPatternMetrics(): Promise<any[]> {
    const goalieDir = this.getGoalieDir();
    if (!goalieDir) {
      return [];
    }
  ...
  private getPaginatedResults(patterns: any[]): any[] {
    const startIndex = (this.currentPage - 1) * this.pageSize;
    const endIndex = startIndex + this.pageSize;
    return patterns.slice(startIndex, endIndex);
  }

  private getUniqueValues(patterns: any[], field: string): string[] {
  ...
  async showPatternChart(patterns?: any[], chartType: 'bar' | 'pie' | 'timeline' | 'heatmap' = 'bar'): Promise<void> {
    ...
    const panel = vscode.window.createWebviewPanel(
      panelId,
      `Pattern Distribution - ${chartType.charAt(0).toUpperCase() + chartType.slice(1)}`,
      vscode.ViewColumn.One,
      {
        enableScripts: true,
        retainContextWhenHidden: true
  ...
  private handleChartMessage(message: any, panelId: string): void {
  ...
  private async refreshChart(panelId: string): Promise<void> {
    const chartPanel = this.chartPanels.get(panelId);
    if (!chartPanel) return;
  ...
  private async exportChartData(patterns: any[], format: 'csv' | 'json' | 'png'): Promise<void> {
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    if (!workspaceRoot) {
      vscode.window.showErrorMessage('No workspace folder found.');
      return;
    ...
    try {
      if (format === 'csv' || format === 'json') {
        ...
        if (format === 'csv') {
          ...
          const patternCounts = new Map<string, any>();
          patterns.forEach(p => {
            const key = p.pattern;
  ...
  private detectFramework(pattern: string): string {
    const patternLower = pattern.toLowerCase();
    if (patternLower.includes('tensorflow') || patternLower.includes('tf-')) return 'TensorFlow';
    if (patternLower.includes('pytorch') || patternLower.includes('torch')) return 'PyTorch';
    if (patternLower.includes('keras')) return 'Keras';
    if (patternLower.includes('scikit') || patternLower.includes('sklearn')) return 'Scikit-learn';
    return 'Custom';
  }

  private generateEnhancedChartHtml(patterns: any[], chartType: 'bar' | 'pie' | 'timeline' | 'heatmap'): string {
    const patternCounts = new Map<string, number>();
    const patternTags = new Map<string, string[]>();
    const patternFrameworks = new Map<string, string>();
  ...
  private generateBarChart(sortedPatterns: [string, number][], patternTags: Map<string, string[]>, patternFrameworks: Map<string, string>, maxCount: number): string {
    const chartBars = sortedPatterns.map(([pattern, count]) => {
      ...
      if (workloadTagsList.includes('HPC')) badges.push('<span class="badge hpc">HPC</span>');
      if (workloadTagsList.includes('Stats')) badges.push('<span class="badge stats">Stats</span>');
      if (workloadTagsList.includes('Device/Web')) badges.push('<span class="badge device">Device/Web</span>');
  ...
  private generatePieChart(sortedPatterns: [string, number][], patternTags: Map<string, string[]>, patternFrameworks: Map<string, string>): string {
    ...
    const pieSlices = sortedPatterns.slice(0, 8).map(([pattern, count], index) => {
      ...
      // Create badges
      const badges: string[] = [];
      if (workloadTagsList.includes('ML')) badges.push('ML');
      if (workloadTagsList.includes('HPC')) badges.push('HPC');
      if (workloadTagsList.includes('Stats')) badges.push('Stats');
      if (workloadTagsList.includes('Device/Web')) badges.push('Device/Web');
  ...
  private generateTimelineChart(patterns: any[]): string {
  ...
  private generateHeatmap(patterns: any[]): string {
    ...
    const maxValue = Math.max(...Array.from(heatmapData.values()).flatMap(workloadMap =>
    ...
    const headerRow = frameworks.map(framework =>
      `<div class="heatmap-header">${framework}</div>`
    ).join('');
  ...
  private generatePrintReportHtml(patterns: any[]): string {
    const timestamp = new Date().toLocaleString();
    const counts = new Map<string, number>();
    const patternTagsMap = new Map<string, string[]>();
  ...
  private generateChartHtml(patterns: any[]): string {
  ...
  async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
    ...
    if (!element) {
      ...
      const filterItem = new vscode.TreeItem(
        `Filters: ${filterLabels || 'All Patterns'}`,
        vscode.TreeItemCollapsibleState.None
      );
      ...
      const presetsItem = new vscode.TreeItem(
        '🎯 Filter Presets',
        vscode.TreeItemCollapsibleState.Collapsed
      );
      ...
      const quickFiltersItem = new vscode.TreeItem(
        '⚡ Quick Filters',
        vscode.TreeItemCollapsibleState.Collapsed
      );
      ...
      if (filteredPatterns.length > this.pageSize) {
        const paginationItem = new vscode.TreeItem(
          `📄 Page ${this.currentPage} of ${Math.ceil(filteredPatterns.length / this.pageSize)}`,
          vscode.TreeItemCollapsibleState.Collapsed
        );
      ...
      chartTypes.forEach(chart => {
        const chartItem = new vscode.TreeItem(
          chart.label,
          vscode.TreeItemCollapsibleState.None
        );
      ...
      return items.concat(Array.from(counts.entries())
        ...
        .map(([pattern, count]) => {
          ...
          if (tags.includes('HPC')) badges.push('HPC');
          if (tags.includes('Stats')) badges.push('Stats');
          if (tags.includes('Device/Web')) badges.push('Device/Web');
    ...
    if ((element as any).contextValue === 'filterPresets') {
    ...
    if ((element as any).contextValue === 'quickFilters') {
      const quickFilters = [
    ...
    if ((element as any).contextValue === 'exportControls') {
      return [
    ...
    if ((element as any).contextValue === 'pagination') {
      const patterns = await this.loadPatternMetrics();
      const filteredPatterns = this.filterPatterns(patterns);
      const items = [
    ...
    if ((element as any).contextValue && (element as any).contextValue.startsWith('quickFilter_')) {
      const filterType = (element as any).contextValue.replace('quickFilter_', '');
      const quickFilter = {
...
function workloadTags(pattern: string, explicitTags: string[] = []): string[] {
  ...
  const normalize = (t: string) => {
    ...
    if (low === 'ml' || low === 'machine-learning' || low === 'tensorflow' || low === 'pytorch') return 'ML';
    if (low === 'hpc' || low === 'cluster' || low === 'slurm') return 'HPC';
    if (low === 'stats' || low === 'statistics') return 'Stats';
    if (low === 'device' || low === 'web' || low === 'mobile') return 'Device/Web';
    return t;
  };
...
class GovernanceEconomicsProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  private latestGovernanceJson: any | undefined;
  private latestCodeFixProposals: any[] | undefined;

  constructor(private readonly workspaceRoot?: string) {}

  setGovernanceJson(json: any | undefined): void {
  ...
  async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
    ...
    const envHintsByKey = new Map<string, any>();
    ...
    for (const line of patternLines) {
      try {
        ...
        const current =
          agg.get(key) || ({ pattern, circle, depth, count: 0, codVals: [], wsjfVals: [], gpuVals: [], latencyVals: [], tags: [] } as Agg);
        current.count += 1;
        ...
        if (typeof obj.gpu_util_pct === 'number') current.gpuVals.push(obj.gpu_util_pct);
        if (typeof obj.p99_latency_ms === 'number') current.latencyVals.push(obj.p99_latency_ms);

        const lineTags = Array.isArray(obj.tags) ? obj.tags.map((t: any) => String(t)) : [];
        ...
        const mergeConfigEntry = (entry: any) => {
          ...
          if (entry.clusterLike) existingEnv.clusterLike = true;
          if (entry.workstationLike) existingEnv.workstationLike = true;
          if (entry.tfLike) existingEnv.tfLike = true;
          if (entry.torchLike) existingEnv.torchLike = true;
        };
        ...
        const tagsArr = Array.isArray(obj.tags) ? obj.tags.map((t: any) => String(t).toLowerCase()) : [];
        ...
        if (clusterLike) existingEnv.clusterLike = true;
        if (workstationLike) existingEnv.workstationLike = true;
        if (tfLike) existingEnv.tfLike = true;
        if (torchLike) existingEnv.torchLike = true;
    ...
    let lastSystemLoad: string | undefined;
    let lastCpuIdle: number | undefined;
    if (fs.existsSync(metricsPath)) {
    ...
    for (const row of filteredRows) {
      ...
      if (row.workloads.includes('ML')) item.contextValue += '.ml';
      if (row.workloads.includes('HPC')) item.contextValue += '.hpc';
      if (row.workloads.includes('Stats')) item.contextValue += '.stats';
      if (row.workloads.includes('Device/Web')) item.contextValue += '.device';
      if (row.isGap) item.contextValue += '.gap';
    ...
    if (proposals && proposals.length) {
      for (const proposal of proposals) {
        ...
        const snippetSource =
          (typeof proposal.codeSnippet === 'string' && proposal.codeSnippet) ||
        ...
        const summaryParts: string[] = [proposal.description || '', approvalLabel, modeLabel].filter(Boolean);
...
class DepthLadderTimelineProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<vscode.TreeItem | undefined | void>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  ...
  async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
    ...
    const lines = fs.readFileSync(metricsPath, 'utf8').split(/\r?\n/).filter(Boolean);
    const events: { timestamp: string; depth: number; run?: string; iteration?: number; circle?: string }[] = [];
...
function detectDefaultLensFromMetrics(workspaceRoot?: string): 'ALL' | 'ML' | 'HPC' | 'STATS_DEVICE' {
  ...
  const lines = fs.readFileSync(metricsPath, 'utf8').split(/\r?\n/).filter(Boolean);
  const counts: Record<string, number> = { ML: 0, HPC: 0, STATS_DEVICE: 0 };
  for (const line of lines) {
    try {
      const obj: any = JSON.parse(line);
      const pattern = obj.pattern as string | undefined;
      if (!pattern) continue;
      const rawTags = Array.isArray(obj.tags) ? obj.tags.map((t: any) => String(t)) : [];
      const tags = workloadTags(pattern, rawTags);
      if (tags.includes('ML')) counts.ML++;
      if (tags.includes('HPC')) counts.HPC++;
      if (tags.includes('Stats') || tags.includes('Device/Web')) counts.STATS_DEVICE++;
    } catch {
      // ignore malformed lines
    }
  }
  const entries: Array<['ML' | 'HPC' | 'STATS_DEVICE', number]> = [
    ['ML', counts.ML],
    ['HPC', counts.HPC],
...
import { GapSessionStats, GoalieGapItem, GoalieGapsProvider } from './goalieGapsProvider';
import { ProcessFlowMetricsProvider } from './processFlowMetricsProvider';

async function openKanbanItem(item: KanbanItem) {
...
async function moveKanbanItem(item: KanbanItem) {
  if (!item.payload || !item.section) return;

  const options: KanbanSection[] = ['NOW', 'NEXT', 'LATER', 'DONE' as any]; // DONE is valid target
  ...
  if (target) {
    ...
    if (goalieDir) {
        try {
            moveKanbanEntry(goalieDir, item.payload, item.section, target as KanbanSection);
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const extensionVersion = (context.extension.packageJSON as any)?.version as string | undefined;
  const extensionName = (context.extension.packageJSON as any)?.name as string | undefined;
  ...
  const sessionStats = new Map<string, GapSessionStats>();
  ...
  const config = vscode.workspace.getConfiguration('goalie');
  const autoDetect = config.get<boolean>('autoDetectLens', true);
  const explicitDefault = config.get<'ALL' | 'ML' | 'HPC' | 'STATS_DEVICE'>('defaultLens');
  const persistedLens = context.workspaceState.get<'ALL' | 'ML' | 'HPC' | 'STATS_DEVICE'>('goalie.currentLens');
  const enableRealtimeDashboard =
    config.get<boolean>('enableRealtimeDashboard', false) ||
    process.env.AF_ENABLE_REALTIME_DASHBOARD === '1';

  let liveGapsPanel: vscode.WebviewPanel | undefined;

  const applyLens = (lens: 'ALL' | 'ML' | 'HPC' | 'STATS_DEVICE') => {
  ...
  function persistLensSelection(lens: 'ALL' | 'ML' | 'HPC' | 'STATS_DEVICE') {
    context.workspaceState.update('goalie.currentLens', lens);
  }

  function colorForWorkloads(tags: string[]): string {
    if (tags.includes('ML')) return '#1f77b4'; // blue
    if (tags.includes('HPC')) return '#d62728'; // red
    if (tags.includes('Stats')) return '#2ca02c'; // green
    if (tags.includes('Device/Web')) return '#9467bd'; // purple
    return '#cccccc';
  }

  function renderLiveGapsHtml(governanceJson: any, retroJson: any): string {
    ...
    const pickNumber = (row: any, fields: string[]): number | undefined => {
    ...
    const findObservabilityStatus = (circle: string, depth: number): string => {
      const bucket = observabilityBuckets.find((b: any) => {
      ...
      const tags = Array.isArray(bucket.tags)
        ? bucket.tags.map((t: any) => String(t)).join(', ')
        : '';
    ...
    const govRows: GovRow[] = govGaps.map((row: any) => {
  ...
  <td>${workloadsStr}</td>
    ...
    const retroRows: RetroRow[] = retroGaps.map((row: any) => {
      const pattern = String(row.pattern ?? '<unknown>');
      const circle = String(row.circle ?? '<none>');
      const depth = typeof row.depth === 'number' ? row.depth : 0;
      const workloads = Array.isArray(row.workloadTags)
        ? (row.workloadTags as string[])
  ...
  <td>${workloadsStr}</td>
    ...
    const retroBaselineDetailHtml = retroBaseline
      ? (() => {
          ...
          const tsPart =
            retroBaseline.baselineTimestamp || retroBaseline.currentTimestamp
  ...
  <p><strong>Score:</strong> baseline=${base}, current=${curr}, delta=${deltaHtml}, regression=${hasRegression ? 'yes' : 'no'}, improvement=${hasImprovement ? 'yes' : 'no'}${tsPart}</p>`;
    ...
    const retroRegressionRows = (retroBaseline?.topRegressions ?? []).map((row: any) => {
      const pct =
      ...
      const base =
        typeof row.baselineScore === 'number'
          ...
          : 'n/a';
      const curr =
        typeof row.currentScore === 'number'
  ...
  <td>${workloads}</td>
    ...
    const retroImprovementRows = (retroBaseline?.topImprovements ?? []).map((row: any) => {
      const pct =
        typeof row.deltaPct === 'number'
          ? `${row.deltaPct.toFixed(1)}%`
          : 'n/a';
  ...
  <td>${workloads}</td>
    ...
    const retroRegressionsTableHtml =
      retroBaseline && retroBaseline.topRegressions && retroBaseline.topRegressions.length
  ...
  <table>
    ...
    const retroImprovementsTableHtml =
      retroBaseline && retroBaseline.topImprovements && retroBaseline.topImprovements.length
  ...
  <table>
    ...
    const governanceBaselineSummaryHtml = govBaseline
      ? (() => {
          const delta =
            typeof govBaseline.overallDelta === 'number'
              ? govBaseline.overallDelta
              : undefined;
          const deltaPct =
            typeof govBaseline.overallDeltaPct === 'number'
          ...
          const wrapped =
            cls ? `<span class="${cls}">${deltaStr} (${deltaPctStr})</span>` : `${deltaStr} (${deltaPctStr})`;
          return `<h2>Governance Baseline Comparison</h2>
  <p><strong>Governance CoD Delta:</strong> ${wrapped}</p>`;
        })()
      : '';

    const governanceRegressionRows = (govBaseline?.topRegressions ?? []).map((row: any) => {
      const base =
        typeof row.baselineScore === 'number'
          ? row.baselineScore.toFixed(2)
          : 'n/a';
  ...
  <td>${pctCell}</td>
    ...
    const governanceImprovementRows = (govBaseline?.topImprovements ?? []).map((row: any) => {
      const base =
        typeof row.baselineScore === 'number'
          ? row.baselineScore.toFixed(2)
          : 'n/a';
  ...
  <td>${pctCell}</td>
    ...
    const governanceRegressionsTableHtml =
      govBaseline && govBaseline.topRegressions && govBaseline.topRegressions.length
  ...
  <table>
    ...
    const governanceImprovementsTableHtml =
      govBaseline && govBaseline.topImprovements && govBaseline.topImprovements.length
  ...
  <table>
    ...
    if (insightsSummary) {
      const verifiedCount =
        typeof insightsSummary.verifiedCount === 'number' ? insightsSummary.verifiedCount : 0;
      const totalActions =
        typeof insightsSummary.totalActions === 'number' ? insightsSummary.totalActions : 0;
      const percentage =
        totalActions > 0 ? ((verifiedCount / totalActions) * 100).toFixed(1) : '0.0';

      const avgCodDelta =
        typeof insightsSummary.avgCodDeltaPct === 'number'
          ? insightsSummary.avgCodDeltaPct
          : undefined;
      const medianFreqDelta =
        typeof insightsSummary.medianFreqDeltaPct === 'number'
          ? insightsSummary.medianFreqDeltaPct
          : undefined;
      const highImpact =
        typeof insightsSummary.highImpactActions === 'number'
          ? insightsSummary.highImpactActions
          : undefined;

      const hasEnhanced =
        avgCodDelta !== undefined || medianFreqDelta !== undefined || highImpact !== undefined;
      ...
      if (totalActions === 0) {
        ...
        if (hasEnhanced) {
          ...
          lines.push(
            '<p class="empty-state">Enhanced forensic verification requires pattern mapping in CONSOLIDATED_ACTIONS.yaml and aligned pattern metrics around action completion times.</p>',
          );
        }
      ...
      if (unverified.length > 0) {
        const rows = unverified.slice(0, 5).map((row: any) => {
          const cod =
            typeof row.codAvg === 'number' && !Number.isNaN(row.codAvg)
...
<body>
  ...
  function extractJsonFromOutput(stdout: string): any | undefined {
  ...
  async function runAfJson(subcommand: 'governance-agent' | 'retro-coach'): Promise<any | undefined> {
    if (!root) {
  ...
  const startStreamClient = () => {
    ...
    const socketOverride = vscode.workspace.getConfiguration('goalie').get<string>('streamSocketPath');
      ...
      streamClient = new StreamClient({
        ...
        onEvent: payload => {
          ...
          if (payload?.type === 'dt-dashboard-summary' && payload.data) {
            try {
              const msg = payload.data as DtDashboardSummaryReadyMessage;
              dtCalibrationProvider.handleSummaryMessage(msg);
            } catch (err: any) {
              outputChannel.appendLine(
                `[DT] Error handling dt-dashboard-summary event: ${err?.message ?? String(err)}`
              );
            }
          }
        },
  ...
  context.subscriptions.push(
    kanbanTreeView,
    kanbanTreeView.onDidExpandElement(e => {
  ...
  context.subscriptions.push(
    gapsTreeView,
    gapsTreeView.onDidExpandElement(e => {
      outputChannel.appendLine(`[UX-TRACKING] Expanded Gap Item: ${e.element.label}`);
      const item = e.element as GoalieGapItem;
  ...
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider('patternMetricsView', patternMetricsProvider),
    vscode.window.registerTreeDataProvider('governanceEconomicsView', governanceEconomicsProvider),
    vscode.window.registerTreeDataProvider('depthLadderTimelineView', depthTimelineProvider),
    vscode.window.registerTreeDataProvider('processFlowMetricsView', processFlowMetricsProvider)
  );
  outputChannel.appendLine('Tree Data Providers Registered.');

  context.subscriptions.push(
    vscode.commands.registerCommand('goalieDashboard.refreshKanban', () => kanbanProvider.refresh()),
    vscode.commands.registerCommand('goalieDashboard.refreshPatternMetrics', () => patternMetricsProvider.refresh()),
    vscode.commands.registerCommand('goalieDashboard.exportPatternMetricsCSV', () => patternMetricsProvider.exportData('csv')),
    vscode.commands.registerCommand('goalieDashboard.exportPatternMetricsJSON', () => patternMetricsProvider.exportData('json')),
    vscode.commands.registerCommand('goalieDashboard.printPatternMetricsReport', () => patternMetricsProvider.printReport()),

    // Enhanced pattern metrics commands
    vscode.commands.registerCommand('goalieDashboard.showPatternChart', (patterns?: any[], chartType?: string) =>
      patternMetricsProvider.showPatternChart(patterns, chartType as any)),

    vscode.commands.registerCommand('goalieDashboard.applyFilterPreset', (presetId: string) =>
      patternMetricsProvider.applyFilterPreset(presetId)),

    vscode.commands.registerCommand('goalieDashboard.quickFilterByCircle', (circle: string) =>
      patternMetricsProvider.quickFilterByCircle(circle)),

    vscode.commands.registerCommand('goalieDashboard.quickFilterByRunKind', (runKind: string) =>
      patternMetricsProvider.quickFilterByRunKind(runKind)),

    vscode.commands.registerCommand('goalieDashboard.quickFilterByWorkload', (workload: string) =>
      patternMetricsProvider.quickFilterByWorkload(workload)),

    vscode.commands.registerCommand('goalieDashboard.quickFilterByFramework', (framework: string) =>
      patternMetricsProvider.quickFilterByFramework(framework)),

    vscode.commands.registerCommand('goalieDashboard.quickFilterByDateRange', (range: string) =>
      patternMetricsProvider.quickFilterByDateRange(range)),

    vscode.commands.registerCommand('goalieDashboard.quickFilterByGate', (gate: string) =>
      patternMetricsProvider.quickFilterByGate(gate)),

    vscode.commands.registerCommand('goalieDashboard.clearFilters', () =>
      patternMetricsProvider.clearFilters()),

    vscode.commands.registerCommand('goalieDashboard.exportChartData', (format?: string) =>
      patternMetricsProvider.exportChartData(format as any)),

    vscode.commands.registerCommand('goalieDashboard.patternMetricsPreviousPage', () =>
      patternMetricsProvider.previousPage()),

    vscode.commands.registerCommand('goalieDashboard.patternMetricsNextPage', () =>
      patternMetricsProvider.nextPage()),

    vscode.commands.registerCommand('goalieDashboard.patternMetricsSetPageSize', () => {
    ...
    vscode.commands.registerCommand('goalieDashboard.refreshGovernanceEconomics', () =>
      governanceEconomicsProvider.refresh()
    ),
    vscode.commands.registerCommand('goalieDashboard.refreshDepthLadderTimeline', () =>
      depthTimelineProvider.refresh()
    ),
    vscode.commands.registerCommand('goalieDashboard.refreshGoalieGaps', () => gapsProvider.refresh()),
    vscode.commands.registerCommand('goalieDashboard.openKanbanItem', openKanbanItem),
    vscode.commands.registerCommand('goalieDashboard.moveKanbanItem', moveKanbanItem),
    vscode.commands.registerCommand('goalieDashboard.filterAll', () => {
    ...
    vscode.commands.registerCommand('goalie.openLiveGapsPanel', async () => {
      if (!enableRealtimeDashboard) {
        vscode.window.showInformationMessage(
          "Goalie Gaps (Live) is disabled. Enable 'goalie.enableRealtimeDashboard' or AF_ENABLE_REALTIME_DASHBOARD=1 to use the experimental live dashboard."
        );
        return;
      }
      if (!root) {
      ...
      if (!liveGapsPanel) {
        liveGapsPanel = vscode.window.createWebviewPanel(
          'goalieGapsLive',
          'Goalie Gaps (Live)',
        ...
        async () => {
          ...
          try {
            ...
            if (Array.isArray(retroJson.workloadPrompts) && retroJson.workloadPrompts.length > 0) {
              const prompts = retroJson.workloadPrompts as string[];
            ...
            if (retroJson.baselineComparison) {
              try {
                ...
                outputChannel.appendLine(
                  `[RetroCoach] Baseline comparison: baseline=${bc.baselineScore}, current=${bc.currentScore}, delta=${bc.delta}, deltaPct=${bc.deltaPct}`
                );
              } catch {
  ...
  async function applyCodeFixProposalInternal(
    proposal: any,
    options?: { skipConfirmation?: boolean; logPrefix?: string }
  ): Promise<'applied' | 'skipped' | 'error'> {
    const logPrefix = options?.logPrefix ?? '[CodeFix]';

    if (!proposal) {
      vscode.window.showErrorMessage(
        'No code fix proposal provided. Run this command from a Goalie code fix proposal item.'
      );
      return 'error';
    }
    ...
    if (!filePath) {
      vscode.window.showInformationMessage(
        'This code fix proposal does not specify a filePath; please apply it manually.'
      );
    ...
    if (!snippet.trim()) {
      vscode.window.showInformationMessage(
        'This code fix proposal has no snippet to apply. Opening view instead.'
      );
    ...
    if (!options?.skipConfirmation) {
      ...
      const choice = await vscode.window.showWarningMessage(
        'Apply Goalie code fix proposal?',
        {
          modal: true,
      ...
      if (choice !== 'Apply') {
        outputChannel.appendLine(
          `${logPrefix} Apply canceled for pattern=${patternLabel}, filePath=${filePath}`
        );
        return 'skipped';
      }
    }
    ...
    try {
      ...
      if (!fs.existsSync(absolutePath)) {
        ...
        outputChannel.appendLine(
          `${logPrefix} Apply failed: file not found at ${absolutePath}`
        );
        return 'error';
      }
      ...
      if (!success) {
        vscode.window.showErrorMessage(
          'Failed to apply Goalie code fix proposal. See Output for details.'
        );
        outputChannel.appendLine(
          `${logPrefix} Apply failed: TextEditor.edit() returned false for ${absolutePath}`
        );
        return 'error';
      }
      ...
      outputChannel.appendLine(
        `${logPrefix} Applied proposal for pattern=${patternLabel} to ${absolutePath}`
      );
      return 'applied';
    } catch (err: any) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('goalieDashboard.viewCodeFixProposal', async (proposal: any) => {
      if (!proposal) {
    ...
    vscode.commands.registerCommand('goalieDashboard.applyCodeFixProposal', async (proposal: any) => {
    ...
    vscode.commands.registerCommand('goalieDashboard.applySafeCodeFixesBatch', async () => {
      const config = vscode.workspace.getConfiguration('goalie');
      const enabled = config.get<boolean>('autoApplyFixes.enabled', false);
      if (!enabled) {
        vscode.window.showInformationMessage(
          'Goalie auto-apply of governance code fixes is disabled. Enable it in settings (goalie.autoApplyFixes.enabled).'
        );
        return;
      }
      ...
      if (!governanceJson) {
        vscode.window.showErrorMessage(
          'Governance Audit failed: no JSON output from governance-agent. See Output for details.'
        );
      ...
      const proposals: any[] = Array.isArray(governanceJson.codeFixProposals)
      ...
      const autoEligible = proposals.filter(p => {
        if (!p) return false;
        if (p.approvalRequired === true) return false;
        if (p.mode !== 'apply') return false;
        const filePath = typeof p.filePath === 'string' ? p.filePath.trim() : '';
        if (!filePath) return false;
        const snippet =
          (typeof p.codeSnippet === 'string' && p.codeSnippet) ||
      ...
      if (!autoEligible.length) {
        vscode.window.showInformationMessage(
          'No auto-eligible governance code fixes found. All proposals require approval or are dry-run only.'
        );
        outputChannel.appendLine(
          '[CodeFix][AUTO] Batch apply aborted: no proposals passed auto-apply policy filters.'
        );
        return;
      }

      const confirmBatch = config.get<boolean>('autoApplyFixes.confirmBatch', true);
      if (confirmBatch) {
        ...
        const detail =
          `This will automatically apply ${autoEligible.length} governance-approved code fixes to your workspace.\n\n` +
        ...
        const choice = await vscode.window.showWarningMessage(
          `Apply ${autoEligible.length} auto-eligible Goalie code fixes now?`,
          {
            modal: true,
            detail,
          },
        ...
        if (choice !== 'Apply') {
          outputChannel.appendLine(
            `[CodeFix][AUTO] Batch apply canceled by user for ${autoEligible.length} proposals.`
          );
          return;
        }
      }
      ...
      const parts: string[] = [];
      parts.push(
        `Automatically applied ${applied} Goalie code fix${applied === 1 ? '' : 'es'} based on governance policy.`
      );
      if (manualRequired.length) {
        parts.push(
          `${manualRequired.length} proposal${manualRequired.length === 1 ? '' : 's'} require manual approval. Use the Governance / Economics view context menu to review and apply them.`
        );
      }
      if (errors) {
        parts.push(
          `${errors} proposal${errors === 1 ? '' : 's'} encountered errors during apply. See Goalie Output for details.`
        );
      }
