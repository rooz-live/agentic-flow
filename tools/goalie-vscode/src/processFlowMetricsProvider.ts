...
export interface ProcessMetrics {
  insightToCommitTime: number; // target: < 1 hour
  actionCompletionRate: number; // target: > 80%
  contextSwitchesPerDay: number; // target: < 5
  leadTime: number;
  cycleTime: number;
  throughput: number;
  wipViolations: number; // target: < 5%
  experimentsPerSprint: number; // target: > 3
  retroToFeatureRate: number; // target: > 60%
  learningImplementationTime: number; // target: < 1 week
}
...
export class ProcessFlowMetricsItem extends vscode.TreeItem {
  constructor(
    label: string,
    collapsibleState: vscode.TreeItemCollapsibleState,
    public readonly metric?: ProcessMetrics,
    public readonly alert?: MetricAlert
  ) {
    super(label, collapsibleState);
  }
}

export class ProcessFlowMetricsProvider implements vscode.TreeDataProvider<ProcessFlowMetricsItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<ProcessFlowMetricsItem | undefined | void>();
  ...
  async getChildren(element?: ProcessFlowMetricsItem): Promise<ProcessFlowMetricsItem[]> {
    ...
    if (!element) {
      ...
      const metrics = await this.calculateProcessMetrics(goalieDir);
      const alerts = this.generateAlerts(metrics);
      
      const items: ProcessFlowMetricsItem[] = [];
      
      // Process Metrics Category
      const processItem = new ProcessFlowMetricsItem(
        'Process Metrics',
      ...
      const flowItem = new ProcessFlowMetricsItem(
        'Flow Metrics',
        vscode.TreeItemCollapsibleState.Expanded,
        metrics,
        undefined
      ...
      const learningItem = new ProcessFlowMetricsItem(
        'Learning Metrics',
        vscode.TreeItemCollapsibleState.Expanded,
        metrics,
        undefined
      ...
      // Alerts Category
      if (alerts.length > 0) {
        const alertsItem = new ProcessFlowMetricsItem(
      ...
      switch (category) {
        ...
        case `Alerts (${element.alert ? 1 : 0})`:
          ...
          return alerts.map(alert => {
            const alertItem = new ProcessFlowMetricsItem(
              `${alert.metric}: ${alert.currentValue}${this.getUnitForMetric(alert.metric)}`,
  ...
  private createMetricItem(label: string, value: number, unit: string, target: string): ProcessFlowMetricsItem {
    const item = new ProcessFlowMetricsItem(
      `${label}: ${value}${unit}`,
      vscode.TreeItemCollapsibleState.None
    );
  ...
  private async calculateProcessMetrics(goalieDir: string): Promise<ProcessMetrics> {
    // Default values
  ...
  private generateAlerts(metrics: ProcessMetrics): MetricAlert[] {
